...
proc ::dialog_audio::pdtk_audio_dialog {mytoplevel \
    ...
    # settings
    ttk::labelframe $mytoplevel.settings -text [_ "Settings"]
    pack $mytoplevel.settings -side top -fill x -pady 5

    ttk::frame $mytoplevel.settings.srd
    pack $mytoplevel.settings.srd -side top -fill x
    ttk::label $mytoplevel.settings.srd.sr_label -text [_ "Sample rate:"]
    ttk::entry $mytoplevel.settings.srd.sr_entry -textvariable audio_sr -width 8
    ttk::label $mytoplevel.settings.srd.d_label -text [_ "Delay (msec):"]
    ttk::entry $mytoplevel.settings.srd.d_entry -textvariable audio_advance -width 4
    ...
    ttk::frame $mytoplevel.settings.bsc
    pack $mytoplevel.settings.bsc -side top -fill x
    ttk::button $mytoplevel.settings.bsc.rate1 -text [_ "48k"] \
        -command "set audio_sr 48000"
    ttk::button $mytoplevel.settings.bsc.rate2 -text [_ "44.1k"] \
        -command "set audio_sr 44100"
    ttk::button $mytoplevel.settings.bsc.rate3 -text [_ "96k"] \
    ...
    ttk::label $mytoplevel.settings.bsc.bs_label -text [_ "Block size:"]
    set blocksizes {64 128 256 512 1024 2048}
    set bsmenu \
        [eval tk_optionMenu $mytoplevel.settings.bsc.bs_popup audio_blocksize $blocksizes]
    $mytoplevel.settings.bsc.bs_popup configure -background $::pd_colors::window_background
    ...
    if {$audio_isfixedcallback} {} else {
        ttk::frame $mytoplevel.settings.callback
        pack $mytoplevel.settings.callback -side bottom -fill x
        ttk::checkbutton $mytoplevel.settings.callback.c_button -variable audio_callback \
            -text [_ "Use callbacks"]
        pack $mytoplevel.settings.callback.c_button
    }

    # input devices
    ttk::labelframe $mytoplevel.inputs -text [_ "Input Devices"]
    pack $mytoplevel.inputs -side top -fill x -pady 5

    # input device 1
    ttk::frame $mytoplevel.inputs.in1f
    pack $mytoplevel.inputs.in1f -side top -fill x

    ttk::checkbutton $mytoplevel.inputs.in1f.x0 -variable audio_inenable1 \
        -text "1:"
    ttk::button $mytoplevel.inputs.in1f.x1 -text [fix_win_locale [lindex $audio_indevlist $audio_indev1]] -width 20 \
        -command [list audio_popup $mytoplevel $mytoplevel.inputs.in1f.x1 audio_indev1 $audio_indevlist]
    ttk::label $mytoplevel.inputs.in1f.l2 -text [_ "Channels:"]
    ttk::entry $mytoplevel.inputs.in1f.x2 -textvariable audio_inchan1 -width 3
    ...
    if {$longform && $multi > 1 && [llength $audio_indevlist] > 1} {
        ttk::frame $mytoplevel.inputs.in2f
        pack $mytoplevel.inputs.in2f -side top

        ttk::checkbutton $mytoplevel.inputs.in2f.x0 -variable audio_inenable2 \
            -text "2:"
        ttk::button $mytoplevel.inputs.in2f.x1 -text [fix_win_locale [lindex $audio_indevlist $audio_indev2]] -width 20 \
            -command [list audio_popup $mytoplevel $mytoplevel.inputs.in2f.x1 audio_indev2 \
                $audio_indevlist]
        ttk::label $mytoplevel.inputs.in2f.l2 -text [_ "Channels:"]
        ttk::entry $mytoplevel.inputs.in2f.x2 -textvariable audio_inchan2 -width 3
    ...
    if {$longform && $multi > 1 && [llength $audio_indevlist] > 2} {
        ttk::frame $mytoplevel.inputs.in3f
        pack $mytoplevel.inputs.in3f -side top

        ttk::checkbutton $mytoplevel.inputs.in3f.x0 -variable audio_inenable3 \
            -text "3:"
        ttk::button $mytoplevel.inputs.in3f.x1 -text [fix_win_locale [lindex $audio_indevlist $audio_indev3]] -width 20 \
            -command [list audio_popup $mytoplevel $mytoplevel.inputs.in3f.x1 audio_indev3 \
                $audio_indevlist]
        ttk::label $mytoplevel.inputs.in3f.l2 -text [_ "Channels:"]
    ...
    if {$longform && $multi > 1 && [llength $audio_indevlist] > 3} {
        ttk::frame $mytoplevel.inputs.in4f
        pack $mytoplevel.inputs.in4f -side top

        ttk::checkbutton $mytoplevel.inputs.in4f.x0 -variable audio_inenable4 \
            -text "4:"
        ttk::button $mytoplevel.inputs.in4f.x1 -text [lindex $audio_indevlist $audio_indev4] \
            -command [list audio_popup $mytoplevel $mytoplevel.inputs.in4f.x1 audio_indev4 \
                $audio_indevlist]
        ttk::label $mytoplevel.inputs.in4f.l2 -text [_ "Channels:"]
        ttk::entry $mytoplevel.inputs.in4f.x2 -textvariable audio_inchan4 -width 3
    ...
    # output devices
    ttk::labelframe $mytoplevel.outputs -text [_ "Output Devices"]
    pack $mytoplevel.outputs -side top -fill x -pady 5

    # output device 1
    ttk::frame $mytoplevel.outputs.out1f
    pack $mytoplevel.outputs.out1f -side top -fill x

    ttk::checkbutton $mytoplevel.outputs.out1f.x0 -variable audio_outenable1 \
        -text "1:"
    if {$multi == 0} {
        label $mytoplevel.outputs.out1f.l1 \
            -text [_ "(same as input device)..."]
    } else {
        ttk::button $mytoplevel.outputs.out1f.x1 -text [fix_win_locale [lindex $audio_outdevlist $audio_outdev1]] -width 20 \
            -command  [list audio_popup $mytoplevel $mytoplevel.outputs.out1f.x1 audio_outdev1 \
                $audio_outdevlist]
    }
    ttk::label $mytoplevel.outputs.out1f.l2 -text [_ "Channels:"]
    ttk::entry $mytoplevel.outputs.out1f.x2 -textvariable audio_outchan1 -width 3
    if {$multi == 0} {
    ...
    if {$longform && $multi > 1 && [llength $audio_outdevlist] > 1} {
        ttk::frame $mytoplevel.outputs.out2f
        pack $mytoplevel.outputs.out2f -side top

        ttk::checkbutton $mytoplevel.outputs.out2f.x0 -variable audio_outenable2 \
            -text "2:"
        ttk::button $mytoplevel.outputs.out2f.x1 -text [fix_win_locale [lindex $audio_outdevlist $audio_outdev2]] -width 20 \
            -command \
            [list audio_popup $mytoplevel $mytoplevel.outputs.out2f.x1 audio_outdev2 $audio_outdevlist]
        ttk::label $mytoplevel.outputs.out2f.l2 -text [_ "Channels:"]
    ...
    if {$longform && $multi > 1 && [llength $audio_outdevlist] > 2} {
        ttk::frame $mytoplevel.outputs.out3f
        pack $mytoplevel.outputs.out3f -side top

        ttk::checkbutton $mytoplevel.outputs.out3f.x0 -variable audio_outenable3 \
            -text "3:" -anchor e
        ttk::button $mytoplevel.outputs.out3f.x1 -text [fix_win_locale [lindex $audio_outdevlist $audio_outdev3]] -width 20 \
            -command \
            [list audio_popup $mytoplevel $mytoplevel.outputs.out3f.x1 audio_outdev3 $audio_outdevlist]
        ttk::label $mytoplevel.outputs.out3f.l2 -text [_ "Channels:"]
        ttk::entry $mytoplevel.outputs.out3f.x2 -textvariable audio_outchan3 -width 3
    ...
    if {$longform && $multi > 1 && [llength $audio_outdevlist] > 3} {
        ttk::frame $mytoplevel.outputs.out4f
        pack $mytoplevel.outputs.out4f -side top

        ttk::checkbutton $mytoplevel.outputs.out4f.x0 -variable audio_outenable4 \
            -text "4:" -anchor e
        ttk::button $mytoplevel.outputs.out4f.x1 -text [fix_win_locale [lindex $audio_outdevlist $audio_outdev4]] \
            -command \
            [list audio_popup $mytoplevel $mytoplevel.outputs.out4f.x1 audio_outdev4 $audio_outdevlist]
        ttk::label $mytoplevel.outputs.out4f.l2 -text [_ "Channels:"]
        ttk::entry $mytoplevel.outputs.out4f.x2 -textvariable audio_outchan4 -width 3
    ...
    if {$longform == 0 && $multi > 1} {
        ttk::frame $mytoplevel.longbutton
        pack $mytoplevel.longbutton -side top -fill x
        ttk::button $mytoplevel.longbutton.b -text [_ "Use Multiple Devices"] \
            -command  {pdsend "pd audio-properties 1"}
        pack $mytoplevel.longbutton.b -expand 1 -ipadx 10 -pady 5
    }

    # save all settings button
    ttk::button $mytoplevel.saveall -text [_ "Save All Settings"] \
    ...
    if {$::windowingsystem eq "aqua"} {
        ...
        $mytoplevel.settings.srd.sr_entry config -validate focusin -validatecommand "::dialog_audio::unbind_return $mytoplevel"
        $mytoplevel.settings.srd.d_entry config -validate focusin -validatecommand "::dialog_audio::unbind_return $mytoplevel"
        $mytoplevel.outputs.out1f.x2 config -validate focusin -validatecommand "::dialog_audio::unbind_return $mytoplevel"
        ...
        if {[winfo exists $mytoplevel.longbutton.b]} {
            $mytoplevel.longbutton.b config
        }
