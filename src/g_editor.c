...
void glist_deselectline(t_glist *x)
{
    if (x->gl_editor)
    {
        ...
        g_cord_deselect(x, x->gl_editor->e_selectline_tag); // ceammc
    }
}
...
static void canvas_rightclick(t_canvas *x, int xpos, int ypos, t_gobj *y)
{
    ...
    // ceammc: open dialog at specified position
    t_canvas* cnv = glist_getcanvas(x);
    int absx = cnv->gl_screenx1 + xpos;
    int absy = cnv->gl_screeny1 + ypos;
    pdgui_vmess("pdtk_canvas_popup", "^ ii ii ii",
        x,
        xpos, ypos,
        canprop, canopen,
        absx, absy);
    ...
    int mod, int doit)
{
                ...
                ypos >= y2 - (OHEIGHT*x->gl_zoom) + x->gl_zoom)
            {
                    ...
                    xpos <= hotspot + (iow + x->gl_zoom))
                {
                    if (doit)
                    {
                        ...
                        g_connection_draw(x, xpos, ypos, xpos, ypos, issignal);
                    }
    ...
    if (doit)
    {
        ...
        g_selection_draw(x, xpos, ypos, xpos, ypos); // ceammc
        x->gl_editor->e_xwas = xpos;
        x->gl_editor->e_ywas = ypos;
    ...
    if(canconnect(x, src, nout, sink, nin))
    {
        ...
        if(oc)
        {
            ...
            lx2 = x21 + (ninlets > 1 ?
                ...
                + iom;
            ly2 = y21;
            g_cord_draw(glist_getcanvas(x), src, nout, oc, lx1, ly1, lx2, ly2, x->gl_zoom); // ceammc
            canvas_undo_add(x, UNDO_CONNECT, "connect", canvas_undo_set_connect(x,
...
#endif
    if (doit)
        g_selection_clear(x); // ceammc
    else
        g_selection_move(x, xwas, ywas, xpos, ypos); // ceammc
    ...
    t_floatarg fwhoin, t_floatarg finno)
{
    ...
    {
        g_cord_draw(glist_getcanvas(x), objsrc, outno, oc, 0, 0, 0, 0, x->gl_zoom); // ceammc
...
void canvas_editmode(t_canvas *x, t_floatarg state)
{
    ...
    else
    {
        ...
        {
            canvas_setcursor(x, CURSOR_RUNMODE_NOTHING);
            g_commentbar_erase_all(glist_getcanvas(x)); // ceammc
        }
    }
    if (glist_isvisible(x) && x->gl_havewindow)
    {
