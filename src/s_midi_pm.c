...
void strcpy_safe_tcl(char* dest, const char* src) {
    int i = 0;
    while (1) {
        char c = src[i];
    ...
    char *outdevlist, int *noutdevs, int maxndev, int devdescsize)
{
    ...
    for (i = 0; i < Pm_CountDevices(); i++)
    {
        ...
        {
            strcpy_safe_tcl(indevlist + nindev * devdescsize, utf8device);
            nindev++;
        }
        if (info->output && noutdev < maxndev)
        {
            strcpy_safe_tcl(outdevlist + noutdev * devdescsize, utf8device);
            noutdev++;
        }
    }
    *nindevs = nindev;
