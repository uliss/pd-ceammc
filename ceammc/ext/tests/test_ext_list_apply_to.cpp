...
TEST_CASE("list.apply_to", "[externals]")
{
    ...
    SECTION("create")
    {
        TestListApplyTo t("list.apply_to");
    ...
    SECTION("mlist")
    {
        ...
        REQUIRE(t.outputDataAt(0) == DataPtr(new DataTypeMList("(2)")));

        t.send(DataTypeMList("(1 3)"));
        REQUIRE(t.outputDataAt(0) == DataPtr(new DataTypeMList("(2 6)")));

        t.send(DataTypeMList("(1 3 5)"));
        REQUIRE(t.outputDataAt(0) == DataPtr(new DataTypeMList("(2 3 10)")));
    }
}
