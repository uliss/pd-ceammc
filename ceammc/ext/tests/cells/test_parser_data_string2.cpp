...
#include "lex/data_string_parser2.h"
#include "lex/lexer_data_string.h"
#include "test_base.h"
...
#define REQUIRE_PARSE_STR(str, res)            \
    {                                          \
        AtomList out;                          \
        parser::DataStringLexer lex(str);      \
        parser::DataStringParser2 p(lex, out); \
        REQUIRE(p.parse() == 0);               \
        REQUIRE(out == res);                   \
    }
...
TEST_CASE("datastring2", "[ceammc::data]")
{
