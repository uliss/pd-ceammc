...
PD_COMPLETE_TEST_SETUP(ListRunningSum, list, runsum)

TEST_CASE("list.runsum", "[externals]")
{
    pd_test_init();

    SECTION("init")
    {
        TObj t("list.runsum", L());
    ...
    SECTION("do")
    {
        ...
        t << LF(1, 10);
        REQUIRE(t.outputListAt(0) == LF(1, 11));
        ...
        t << LA("S", 10, 20);
        REQUIRE(t.outputListAt(0) == LF(0, 10, 30));

        t << LA("S", "S");
        REQUIRE(t.outputListAt(0) == LF(0, 0));

        t << LF(1, 2, 3, 4, 5.5, -10.5);
        REQUIRE(t.outputListAt(0) == LF(1, 3, 6, 10, 15.5, 5));
    }
}
