...
static float hpf12_faustpower2_f(float value) {
	return (value * value);
	
}

#ifndef FAUSTCLASS 
#define FAUSTCLASS hpf12
#endif
#ifdef __APPLE__ 
...
class hpf12 : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt_hpf12");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp2 = hpf12_faustpower2_f(fTemp0);
			float fTemp3 = (1.0f / fTemp2);
			float fTemp4 = (((fTemp1 + 1.41421354f) / fTemp0) + 1.0f);
			fRec0[0] = (float(input0[i]) - (((fRec0[2] * (((fTemp1 + -1.41421354f) / fTemp0) + 1.0f)) + (2.0f * (fRec0[1] * (1.0f - fTemp3)))) / fTemp4));
			output0[i] = FAUSTFLOAT((((fRec0[2] / fTemp2) + ((fRec0[0] / fTemp2) + (2.0f * (fRec0[1] * (0.0f - fTemp3))))) / fTemp4));
			fRec1[1] = fRec1[0];
			fRec0[2] = fRec0[1];
			fRec0[1] = fRec0[0];
...
static t_class* hpf12_faust_class;
#define FAUST_EXT t_faust_hpf12
#define FAUST_EXT_CLASS hpf12_faust_class
// clang-format on

template <class T>
class _hpf12_UI : public UI {
};
typedef _hpf12_UI<hpf12> hpf12_UI;

struct t_faust_hpf12 {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_hpf12* x = reinterpret_cast<t_faust_hpf12*>(w[1]);
...
static void hpf12_faust_dsp(t_faust_hpf12* x, t_signal** sp)
{
...
static void hpf12_dump_to_console(t_faust_hpf12* x)
{
...
static void hpf12_faust_any(t_faust_hpf12* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_hpf12* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_hpf12* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_hpf12* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_hpf12* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_hpf12* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void hpf12_faust_free(t_faust_hpf12* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_hpf12* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(hpf12));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_hpf12* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(hpf12));
            return false;
        }
...
static bool faust_new_internal(t_faust_hpf12* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new hpf12();
    x->ui = new PdUI<hpf12_UI>(sym(hpf12), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_hpf12* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_hpf12* pd_obj()
    {
        return this->x_;
    }
};

static void* hpf12_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    hpf12_faust_class = class_new(s, reinterpret_cast<t_newmethod>(hpf12_faust_new),
        reinterpret_cast<t_method>(hpf12_faust_free),
        sizeof(t_faust_hpf12),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(hpf12_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(hpf12_faust_class, t_faust_hpf12, f);
    }

    class_addmethod(hpf12_faust_class, reinterpret_cast<t_method>(hpf12_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(hpf12_faust_class, reinterpret_cast<t_method>(hpf12_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(hpf12_faust_class, hpf12_faust_any);
    ceammc::register_faust_external(hpf12_faust_class);
}

#define EXTERNAL_NEW void* hpf12_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* hpf12_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_hpf12* x = reinterpret_cast<t_faust_hpf12*>(pd_new(hpf12_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ehpf12_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".hpf12~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ehpf12_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".hpf12~"), false); \
    }
