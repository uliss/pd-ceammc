...
#ifndef  __eq_peak_cq_H__
#define  __eq_peak_cq_H__
...
static float eq_peak_cq_faustpower2_f(float value) {
	return (value * value);
	
}

#ifndef FAUSTCLASS 
#define FAUSTCLASS eq_peak_cq
#endif
#ifdef __APPLE__ 
...
class eq_peak_cq : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "flt_eq_peak_cq");
	...
	virtual eq_peak_cq* clone() {
		return new eq_peak_cq();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt_eq_peak_cq");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp1 = (2.0f * (fRec0[1] * (1.0f - (1.0f / eq_peak_cq_faustpower2_f(fTemp0)))));
			float fTemp2 = (1.0f / fTemp0);
			fRec2[0] = (fSlow1 + (0.999000013f * fRec2[1]));
			int iTemp3 = (fRec2[0] > 0.0f);
			fRec3[0] = (fSlow2 + (0.999000013f * fRec3[1]));
			float fTemp4 = (fRec3[0] * std::sin((fConst4 * fRec1[0])));
			float fTemp5 = (fConst3 * ((fRec1[0] * std::pow(10.0f, (0.0500000007f * std::fabs(fRec2[0])))) / fTemp4));
			float fTemp6 = (fConst3 * (fRec1[0] / fTemp4));
			...
			output0[i] = FAUSTFLOAT((((fTemp1 + ((((fTemp9 + fTemp2) / fTemp0) + 1.0f) * fRec0[0])) + (fRec0[2] * (1.0f - ((fTemp9 - fTemp2) / fTemp0)))) / fTemp8));
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
			fRec3[1] = fRec3[0];
			fRec0[2] = fRec0[1];
...
static t_class* eq_peak_cq_faust_class;
#define FAUST_EXT t_faust_eq_peak_cq
#define FAUST_EXT_CLASS eq_peak_cq_faust_class
// clang-format on

template <class T>
class _eq_peak_cq_UI : public UI {
};
typedef _eq_peak_cq_UI<eq_peak_cq> eq_peak_cq_UI;

struct t_faust_eq_peak_cq {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_eq_peak_cq* x = reinterpret_cast<t_faust_eq_peak_cq*>(w[1]);
...
static void eq_peak_cq_faust_dsp(t_faust_eq_peak_cq* x, t_signal** sp)
{
...
static void eq_peak_cq_dump_to_console(t_faust_eq_peak_cq* x)
{
...
static void eq_peak_cq_faust_any(t_faust_eq_peak_cq* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_eq_peak_cq* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_eq_peak_cq* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_eq_peak_cq* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_eq_peak_cq* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_eq_peak_cq* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void eq_peak_cq_faust_free(t_faust_eq_peak_cq* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_eq_peak_cq* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(eq_peak_cq));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_eq_peak_cq* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(eq_peak_cq));
            return false;
        }
...
static bool faust_new_internal(t_faust_eq_peak_cq* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new eq_peak_cq();
    x->ui = new PdUI<eq_peak_cq_UI>(sym(eq_peak_cq), objId);

    if (!faust_init_inputs(x)) {
        eq_peak_cq_faust_free(x);
        return false;
    }
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_eq_peak_cq* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_eq_peak_cq* pd_obj()
    {
        return this->x_;
    }
};

static void* eq_peak_cq_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    eq_peak_cq_faust_class = class_new(s, reinterpret_cast<t_newmethod>(eq_peak_cq_faust_new),
        reinterpret_cast<t_method>(eq_peak_cq_faust_free),
        sizeof(t_faust_eq_peak_cq),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(eq_peak_cq_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(eq_peak_cq_faust_class, t_faust_eq_peak_cq, f);
    }

    class_addmethod(eq_peak_cq_faust_class, reinterpret_cast<t_method>(eq_peak_cq_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(eq_peak_cq_faust_class, reinterpret_cast<t_method>(eq_peak_cq_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(eq_peak_cq_faust_class, eq_peak_cq_faust_any);
    ceammc::register_faust_external(eq_peak_cq_faust_class);
}

#define EXTERNAL_NEW void* eq_peak_cq_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* eq_peak_cq_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_eq_peak_cq* x = reinterpret_cast<t_faust_eq_peak_cq*>(pd_new(eq_peak_cq_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2eeq_peak_cq_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".eq_peak_cq~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2eeq_peak_cq_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".eq_peak_cq~"), false); \
    }
