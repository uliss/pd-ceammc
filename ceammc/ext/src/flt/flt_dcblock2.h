...
#ifndef  __dcblock2_H__
#define  __dcblock2_H__
...
class dcblock2 : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "flt_dcblock2.dsp");
...
static t_class* dcblock2_faust_class;
#define FAUST_EXT t_faust_dcblock2
#define FAUST_EXT_CLASS dcblock2_faust_class
// clang-format on

template <class T>
class _dcblock2_UI : public UI {
};
typedef _dcblock2_UI<dcblock2> dcblock2_UI;

struct t_faust_dcblock2 {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_dcblock2* x = reinterpret_cast<t_faust_dcblock2*>(w[1]);
    if (!x->dsp || !x->buf)
        return (w + 3);
...
static void dcblock2_faust_dsp(t_faust_dcblock2* x, t_signal** sp)
{
    // blocksize
...
static void dcblock2_dump_to_console(t_faust_dcblock2* x)
{
...
static void dcblock2_faust_any(t_faust_dcblock2* x, t_symbol* s, int argc, t_atom* argv)
{
...
static void faust_free_dsp(t_faust_dcblock2* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_dcblock2* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_dcblock2* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_dcblock2* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_dcblock2* x)
{
    if (x->buf) {
...
static void dcblock2_faust_free(t_faust_dcblock2* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_dcblock2* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(dcblock2));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_dcblock2* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(dcblock2));
            return false;
        }
...
static bool faust_new_internal(t_faust_dcblock2* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = true;
    x->xfade = 0;
    ...
    x->dsp = new dcblock2();
    x->ui = new PdUI<dcblock2_UI>(sym(dcblock2), objId);

    if (!faust_init_inputs(x)) {
        dcblock2_faust_free(x);
        return false;
    }
...
public:
    ...
    t_faust_dcblock2* pd_obj()
    {
        return this->x_;
    }
};

static void* dcblock2_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    dcblock2_faust_class = class_new(s, reinterpret_cast<t_newmethod>(dcblock2_faust_new),
        reinterpret_cast<t_method>(dcblock2_faust_free),
        sizeof(t_faust_dcblock2),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(dcblock2_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(dcblock2_faust_class, t_faust_dcblock2, f);
    }

    class_addmethod(dcblock2_faust_class, reinterpret_cast<t_method>(dcblock2_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(dcblock2_faust_class, reinterpret_cast<t_method>(dcblock2_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(dcblock2_faust_class, reinterpret_cast<t_method>(dcblock2_faust_any));
    ceammc::register_faust_external(dcblock2_faust_class);
}

#define EXTERNAL_NEW void* dcblock2_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* dcblock2_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_dcblock2* x = reinterpret_cast<t_faust_dcblock2*>(pd_new(dcblock2_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2edcblock2_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".dcblock2~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2edcblock2_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".dcblock2~"), false); \
    }
