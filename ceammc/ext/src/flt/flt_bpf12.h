...
static float bpf12_faustpower2_f(float value) {
	return (value * value);
	
}

#ifndef FAUSTCLASS 
#define FAUSTCLASS bpf12
#endif
#ifdef __APPLE__ 
 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp1 = std::tan((fConst3 * std::min((fRec1[0] * (fTemp0 + 1.0f)), fConst4)));
			float fTemp2 = bpf12_faustpower2_f(std::sqrt((fConst2 * (fTemp1 * std::tan((fConst3 * std::max((fRec1[0] * (1.0f - fTemp0)), 0.0f)))))));
			float fTemp3 = (fConst1 * fTemp2);
			float fTemp4 = ((fConst6 * fTemp1) - (fConst7 * (fTemp2 / fTemp1)));
			float fTemp5 = (fConst5 * fTemp4);
			float fTemp6 = ((fTemp3 + fTemp5) + 4.0f);
			fRec0[0] = (float(input0[i]) - (((fRec0[2] * (fTemp3 + (4.0f - fTemp5))) + (fRec0[1] * ((fConst8 * fTemp2) + -8.0f))) / fTemp6));
			output0[i] = FAUSTFLOAT((((fRec0[2] * (0.0f - fTemp5)) + (fConst5 * (fTemp4 * fRec0[0]))) / fTemp6));
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
			fRec0[2] = fRec0[1];
			fRec0[1] = fRec0[0];
...
static t_class* bpf12_faust_class;
#define FAUST_EXT t_faust_bpf12
#define FAUST_EXT_CLASS bpf12_faust_class
// clang-format on

template <class T>
class _bpf12_UI : public UI {
};
typedef _bpf12_UI<bpf12> bpf12_UI;

struct t_faust_bpf12 {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_bpf12* x = reinterpret_cast<t_faust_bpf12*>(w[1]);
...
static void bpf12_faust_dsp(t_faust_bpf12* x, t_signal** sp)
{
...
static void bpf12_dump_to_console(t_faust_bpf12* x)
{
...
static void bpf12_faust_any(t_faust_bpf12* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_bpf12* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_bpf12* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_bpf12* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_bpf12* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_bpf12* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void bpf12_faust_free(t_faust_bpf12* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_bpf12* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(bpf12));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_bpf12* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(bpf12));
            return false;
        }
...
static bool faust_new_internal(t_faust_bpf12* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new bpf12();
    x->ui = new PdUI<bpf12_UI>(sym(bpf12), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_bpf12* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_bpf12* pd_obj()
    {
        return this->x_;
    }
};

static void* bpf12_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    bpf12_faust_class = class_new(s, reinterpret_cast<t_newmethod>(bpf12_faust_new),
        reinterpret_cast<t_method>(bpf12_faust_free),
        sizeof(t_faust_bpf12),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(bpf12_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(bpf12_faust_class, t_faust_bpf12, f);
    }

    class_addmethod(bpf12_faust_class, reinterpret_cast<t_method>(bpf12_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(bpf12_faust_class, reinterpret_cast<t_method>(bpf12_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(bpf12_faust_class, bpf12_faust_any);
    ceammc::register_faust_external(bpf12_faust_class);
}

#define EXTERNAL_NEW void* bpf12_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* bpf12_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_bpf12* x = reinterpret_cast<t_faust_bpf12*>(pd_new(bpf12_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ebpf12_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".bpf12~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ebpf12_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".bpf12~"), false); \
    }
