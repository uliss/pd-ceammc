 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp5 = ((sqrtf(fTemp0) * sinf(fTemp1)) / max(0.00100000005f, fRec2[0]));
			float fTemp6 = ((fTemp0 + fTemp5) + (1.0f - fTemp3));
			output0[i] = FAUSTFLOAT(((fTemp0 * ((fTemp4 + fTemp5) + 1.0f)) / fTemp6));
			float fTemp7 = ((fTemp0 + 1.0f) * fTemp2);
			output1[i] = FAUSTFLOAT((((0.0f - (2.0f * fTemp0)) * ((fTemp0 + fTemp7) + -1.0f)) / fTemp6));
			output2[i] = FAUSTFLOAT(((fTemp0 * (fTemp4 + (1.0f - fTemp5))) / fTemp6));
			output3[i] = FAUSTFLOAT((2.0f * ((fTemp0 + (-1.0f - fTemp7)) / fTemp6)));
			output4[i] = FAUSTFLOAT(((fTemp0 + (1.0f - (fTemp3 + fTemp5))) / fTemp6));
			fRec0[1] = fRec0[0];
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
...
static bool faust_new_internal(t_faust_c_highshelf* x, const std::string& objId = "", bool info_outlet = true)
{
    ...
    x->ui = new PdUI<UI>(sym(c_highshelf), objId);
