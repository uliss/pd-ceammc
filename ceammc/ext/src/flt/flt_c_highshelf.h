/* ------------------------------------------------------------
name: "flt_c_highshelf"
Code generated with Faust 2.8.5 (https://faust.grame.fr)
Compilation options: cpp, -scal -ftz 0
------------------------------------------------------------ */

#ifndef  __c_highshelf_H__
#define  __c_highshelf_H__
...
class c_highshelf : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "flt_c_highshelf");
		...
		m->declare("name", "flt_c_highshelf");
	...
	virtual c_highshelf* clone() {
		return new c_highshelf();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt_c_highshelf");
		ui_interface->declare(&fVslider1, "unit", "Hz");
		ui_interface->addVerticalSlider("freq", &fVslider1, 10000.0f, 20.0f, 20000.0f, 0.100000001f);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp4 = ((std::sqrt(fTemp0) * std::sin(fTemp1)) / std::max(0.00100000005f, fRec2[0]));
			float fTemp5 = (fTemp0 + fTemp4);
			float fTemp6 = (fTemp5 + (1.0f - fTemp3));
			output0[i] = FAUSTFLOAT(((fTemp0 * ((fTemp3 + fTemp5) + 1.0f)) / fTemp6));
			float fTemp7 = (fTemp2 * (fTemp0 + 1.0f));
			output1[i] = FAUSTFLOAT((((0.0f - (2.0f * fTemp0)) * ((fTemp0 + fTemp7) + -1.0f)) / fTemp6));
			output2[i] = FAUSTFLOAT(((fTemp0 * ((fTemp0 + fTemp3) + (1.0f - fTemp4))) / fTemp6));
			output3[i] = FAUSTFLOAT((2.0f * ((fTemp0 + (-1.0f - fTemp7)) / fTemp6)));
			output4[i] = FAUSTFLOAT(((fTemp0 + (1.0f - (fTemp4 + fTemp3))) / fTemp6));
			fRec0[1] = fRec0[0];
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
...
static t_class* c_highshelf_faust_class;
#define FAUST_EXT t_faust_c_highshelf
#define FAUST_EXT_CLASS c_highshelf_faust_class
// clang-format on

template <class T>
class _c_highshelf_UI : public UI {
};
typedef _c_highshelf_UI<c_highshelf> c_highshelf_UI;

struct t_faust_c_highshelf {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_c_highshelf* x = reinterpret_cast<t_faust_c_highshelf*>(w[1]);
...
static void c_highshelf_faust_dsp(t_faust_c_highshelf* x, t_signal** sp)
{
...
static void c_highshelf_dump_to_console(t_faust_c_highshelf* x)
{
...
static void c_highshelf_faust_any(t_faust_c_highshelf* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_c_highshelf* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_c_highshelf* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_c_highshelf* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_c_highshelf* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_c_highshelf* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void c_highshelf_faust_free(t_faust_c_highshelf* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_c_highshelf* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(c_highshelf));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_c_highshelf* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(c_highshelf));
            return false;
        }
...
static bool faust_new_internal(t_faust_c_highshelf* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new c_highshelf();
    x->ui = new PdUI<c_highshelf_UI>(sym(c_highshelf), objId);

    if (!faust_init_inputs(x)) {
        c_highshelf_faust_free(x);
        return false;
    }
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_c_highshelf* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_c_highshelf* pd_obj()
    {
        return this->x_;
    }
};

static void* c_highshelf_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    c_highshelf_faust_class = class_new(s, reinterpret_cast<t_newmethod>(c_highshelf_faust_new),
        reinterpret_cast<t_method>(c_highshelf_faust_free),
        sizeof(t_faust_c_highshelf),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(c_highshelf_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(c_highshelf_faust_class, t_faust_c_highshelf, f);
    }

    class_addmethod(c_highshelf_faust_class, reinterpret_cast<t_method>(c_highshelf_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(c_highshelf_faust_class, reinterpret_cast<t_method>(c_highshelf_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(c_highshelf_faust_class, c_highshelf_faust_any);
    ceammc::register_faust_external(c_highshelf_faust_class);
}

#define EXTERNAL_NEW void* c_highshelf_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* c_highshelf_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_c_highshelf* x = reinterpret_cast<t_faust_c_highshelf*>(pd_new(c_highshelf_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ec_highshelf_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".c_highshelf~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ec_highshelf_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".c_highshelf~"), false); \
    }
