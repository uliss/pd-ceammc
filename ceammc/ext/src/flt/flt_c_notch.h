...
#ifndef  __c_notch_H__
#define  __c_notch_H__
...
class c_notch : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "flt_c_notch");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt_c_notch");
		ui_interface->declare(&fVslider0, "unit", "Hz");
		ui_interface->addVerticalSlider("freq", &fVslider0, 1000.0f, 20.0f, 20000.0f, 0.100000001f);
		ui_interface->addVerticalSlider("q", &fVslider1, 1.0f, 0.00999999978f, 100.0f, 0.100000001f);
...
static t_class* c_notch_faust_class;
#define FAUST_EXT t_faust_c_notch
#define FAUST_EXT_CLASS c_notch_faust_class
// clang-format on

template <class T>
class _c_notch_UI : public UI {
};
typedef _c_notch_UI<c_notch> c_notch_UI;

struct t_faust_c_notch {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_c_notch* x = reinterpret_cast<t_faust_c_notch*>(w[1]);
...
static void c_notch_faust_dsp(t_faust_c_notch* x, t_signal** sp)
{
...
static void c_notch_dump_to_console(t_faust_c_notch* x)
{
...
static void c_notch_faust_any(t_faust_c_notch* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_c_notch* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_c_notch* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_c_notch* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_c_notch* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_c_notch* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void c_notch_faust_free(t_faust_c_notch* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_c_notch* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(c_notch));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_c_notch* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(c_notch));
            return false;
        }
...
static bool faust_new_internal(t_faust_c_notch* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new c_notch();
    x->ui = new PdUI<c_notch_UI>(sym(c_notch), objId);

    if (!faust_init_inputs(x)) {
        c_notch_faust_free(x);
        return false;
    }
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_c_notch* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_c_notch* pd_obj()
    {
        return this->x_;
    }
};

static void* c_notch_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    c_notch_faust_class = class_new(s, reinterpret_cast<t_newmethod>(c_notch_faust_new),
        reinterpret_cast<t_method>(c_notch_faust_free),
        sizeof(t_faust_c_notch),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(c_notch_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(c_notch_faust_class, t_faust_c_notch, f);
    }

    class_addmethod(c_notch_faust_class, reinterpret_cast<t_method>(c_notch_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(c_notch_faust_class, reinterpret_cast<t_method>(c_notch_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(c_notch_faust_class, c_notch_faust_any);
    ceammc::register_faust_external(c_notch_faust_class);
}

#define EXTERNAL_NEW void* c_notch_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* c_notch_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_c_notch* x = reinterpret_cast<t_faust_c_notch*>(pd_new(c_notch_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ec_notch_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".c_notch~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ec_notch_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".c_notch~"), false); \
    }
