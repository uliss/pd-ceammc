...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn flt_c_notch -scn flt_c_notch_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __flt_c_notch_H__
#define  __flt_c_notch_H__
...
class FAUST_API flt_c_notch_dsp {

    public:

        flt_c_notch_dsp() = default;
        virtual ~flt_c_notch_dsp() = default;
         ...
         */
        virtual ::flt_c_notch_dsp* clone() = 0;
...
class FAUST_API decorator_dsp : public ::flt_c_notch_dsp {

    protected:

        ::flt_c_notch_dsp* fDSP;

    public:

        decorator_dsp(::flt_c_notch_dsp* flt_c_notch_dsp = nullptr):fDSP(flt_c_notch_dsp) {}
...
class FAUST_API dsp_factory {
    ...
    public:
        ...
        virtual ::flt_c_notch_dsp* createDSPInstance() = 0;
...
/**
 The base class of Meta handler to be used in flt_c_notch_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
// clang-format off
#ifndef FAUST_MACRO
struct flt_c_notch : public flt_c_notch_dsp {
};
#endif
// clang-format on
...
class flt_c_notch : public flt_c_notch_dsp {
 ...
 public:
	...
	flt_c_notch(const flt_c_notch&) = default;
	
	virtual ~flt_c_notch() = default;
	
	flt_c_notch& operator=(const flt_c_notch&) = default;
	
	void metadata(Meta* m) { 
		m->declare("ceammc_ui.lib/name", "CEAMMC faust default UI elements");
		m->declare("ceammc_ui.lib/version", "0.1.2");
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn flt_c_notch -scn flt_c_notch_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
		m->declare("filename", "flt_c_notch.dsp");
	...
	virtual flt_c_notch* clone() {
		return new flt_c_notch(*this);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("q", &fVslider1, FAUSTFLOAT(1.0f), FAUSTFLOAT(0.01f), FAUSTFLOAT(1e+02f), FAUSTFLOAT(0.1f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp0 = fConst1 * std::max<float>(0.0f, fRec0[0]);
			fRec1[0] = fSlow1 + fConst3 * fRec1[1];
			float fTemp1 = 0.5f * (std::sin(fTemp0) / std::max<float>(0.001f, fRec1[0]));
			float fTemp2 = fTemp1 + 1.0f;
			float fTemp3 = 1.0f / fTemp2;
			output0[i0] = static_cast<FAUSTFLOAT>(fTemp3);
			float fTemp4 = 2.0f * (std::cos(fTemp0) / fTemp2);
			output1[i0] = static_cast<FAUSTFLOAT>(-fTemp4);
			output2[i0] = static_cast<FAUSTFLOAT>(fTemp3);
			output3[i0] = static_cast<FAUSTFLOAT>(-fTemp4);
			output4[i0] = static_cast<FAUSTFLOAT>((1.0f - fTemp1) / fTemp2);
			fRec0[1] = fRec0[0];
			fRec1[1] = fRec1[0];
		}
	}
...
class faust_flt_c_notch_tilde : public FaustExternal<flt_c_notch> {
public:
    faust_flt_c_notch_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(flt_c_notch))
    {
    }
};
