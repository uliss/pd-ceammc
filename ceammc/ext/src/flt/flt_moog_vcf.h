...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn flt_moog_vcf -scn flt_moog_vcf_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0
------------------------------------------------------------ */
...
class FAUST_API flt_moog_vcf_dsp {
    ...
    public:
        ...
        flt_moog_vcf_dsp() = default;
        virtual ~flt_moog_vcf_dsp() = default;
...
class flt_moog_vcf : public flt_moog_vcf_dsp {
 ...
 public:
	...
	flt_moog_vcf(const flt_moog_vcf&) = default;
	
	virtual ~flt_moog_vcf() = default;
	
	flt_moog_vcf& operator=(const flt_moog_vcf&) = default;
	
	void metadata(Meta* m) { 
		m->declare("basics.lib/name", "Faust Basic Element Library");
		m->declare("basics.lib/version", "1.22.0");
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn flt_moog_vcf -scn flt_moog_vcf_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0");
	...
	virtual void instanceResetUserInterface() {
		fVslider0 = static_cast<FAUSTFLOAT>(0.4);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("res", &fVslider0, FAUSTFLOAT(0.4), FAUSTFLOAT(0.0), FAUSTFLOAT(1.0), FAUSTFLOAT(0.001));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			double fTemp5 = std::tan(fConst3 * std::max<double>(static_cast<double>(input1[i0]), 2e+01));
			...
			double fTemp15 = std::max<double>(-0.9999999999999998, std::min<double>(0.9999999999999998, 2.0 * (fTemp14 / (fTemp7 * (fTemp8 + 1.0)))));
			...
			double fTemp25 = std::max<double>(-0.9999999999999998, std::min<double>(0.9999999999999998, fTemp24));
			...
			double fTemp30 = std::max<double>(-0.9999999999999998, std::min<double>(0.9999999999999998, 2.0 * (fTemp29 / (fTemp23 * (fTemp24 + 1.0)))));
			...
			output0[i0] = static_cast<FAUSTFLOAT>((fTemp20 * fTemp25 / fTemp7 + fRec4[1] * fTemp27 + 2.0 * (fRec4[0] * fTemp33 / fTemp34) + fRec5 * (1.0 - fTemp24 - 2.0 * fTemp30 * fTemp33) / (fTemp34 * std::sqrt(fTemp31))) / fTemp23);
			fRec0[1] = fRec0[0];
			fRec3[1] = fRec3[0];
			fRec1[1] = fRec1[0];
			fRec6[1] = fRec6[0];
...
class faust_flt_moog_vcf_tilde : public FaustExternal<flt_moog_vcf> {
public:
    faust_flt_moog_vcf_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(flt_moog_vcf))
    {
    }
};
