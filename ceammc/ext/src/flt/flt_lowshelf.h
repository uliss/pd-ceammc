...
class lowshelf : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt_lowshelf");
...
static t_class* lowshelf_faust_class;
#define FAUST_EXT t_faust_lowshelf
...
{
    t_faust_lowshelf* x = reinterpret_cast<t_faust_lowshelf*>(w[1]);
...
static void lowshelf_faust_dsp(t_faust_lowshelf* x, t_signal** sp)
{
...
static void lowshelf_dump_to_console(t_faust_lowshelf* x)
{
...
static void lowshelf_faust_any(t_faust_lowshelf* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_lowshelf* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_lowshelf* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_lowshelf* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_lowshelf* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_lowshelf* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void lowshelf_faust_free(t_faust_lowshelf* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_lowshelf* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(lowshelf));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_lowshelf* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(lowshelf));
            return false;
        }
...
static bool faust_new_internal(t_faust_lowshelf* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
    ...
    t_faust_lowshelf* pd_obj()
    {
        return this->x_;
    }
};

static void* lowshelf_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    lowshelf_faust_class = class_new(s, reinterpret_cast<t_newmethod>(lowshelf_faust_new),
        reinterpret_cast<t_method>(lowshelf_faust_free),
        sizeof(t_faust_lowshelf),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(lowshelf_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(lowshelf_faust_class, t_faust_lowshelf, f);
    }
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* lowshelf_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_lowshelf* x = reinterpret_cast<t_faust_lowshelf*>(pd_new(lowshelf_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2elowshelf_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".lowshelf~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2elowshelf_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".lowshelf~"), false); \
    }
