...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn flt_lowshelf -scn flt_lowshelf_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __flt_lowshelf_H__
#define  __flt_lowshelf_H__
...
class FAUST_API flt_lowshelf_dsp {

    public:

        flt_lowshelf_dsp() = default;
        virtual ~flt_lowshelf_dsp() = default;
         ...
         */
        virtual ::flt_lowshelf_dsp* clone() = 0;
...
class FAUST_API decorator_dsp : public ::flt_lowshelf_dsp {

    protected:

        ::flt_lowshelf_dsp* fDSP;

    public:

        decorator_dsp(::flt_lowshelf_dsp* flt_lowshelf_dsp = nullptr):fDSP(flt_lowshelf_dsp) {}
...
class FAUST_API dsp_factory {
    ...
    public:
        ...
        virtual ::flt_lowshelf_dsp* createDSPInstance() = 0;
...
/**
 The base class of Meta handler to be used in flt_lowshelf_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
// clang-format off
#ifndef FAUST_MACRO
struct flt_lowshelf : public flt_lowshelf_dsp {
};
#endif
// clang-format on
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS flt_lowshelf
#endif
...
class flt_lowshelf : public flt_lowshelf_dsp {
 ...
 public:
	...
	flt_lowshelf(const flt_lowshelf&) = default;
	
	virtual ~flt_lowshelf() = default;
	
	flt_lowshelf& operator=(const flt_lowshelf&) = default;
	
	void metadata(Meta* m) { 
		m->declare("ceammc_ui.lib/name", "CEAMMC faust default UI elements");
		m->declare("ceammc_ui.lib/version", "0.1.2");
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn flt_lowshelf -scn flt_lowshelf_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual flt_lowshelf* clone() {
		return new flt_lowshelf(*this);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt.lowshelf");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp6 = (fTemp0 + 1.0f) * fTemp2;
			float fTemp7 = fTemp5 + fTemp4 + 1.0f;
			fRec1[0] = static_cast<float>(input0[i0]) - ((fTemp4 + (1.0f - fTemp5)) * fRec1[2] - 2.0f * (fTemp0 + fTemp6 + -1.0f) * fRec1[1]) / fTemp7;
			output0[i0] = static_cast<FAUSTFLOAT>(fTemp0 * (fRec1[0] * (fTemp0 + fTemp5 + (1.0f - fTemp3)) + 2.0f * fRec1[1] * (fTemp0 + (-1.0f - fTemp6)) + fRec1[2] * (fTemp0 + (1.0f - (fTemp3 + fTemp5)))) / fTemp7);
			fRec0[1] = fRec0[0];
			fRec2[1] = fRec2[0];
			fRec1[2] = fRec1[1];
			fRec1[1] = fRec1[0];
...
class faust_flt_lowshelf_tilde : public FaustExternal<flt_lowshelf> {
public:
    faust_flt_lowshelf_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(flt_lowshelf))
    {
    }
};
