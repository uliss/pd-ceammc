...
template <class T>
class _c_lpf_UI : public UI {
};
typedef _c_lpf_UI<c_lpf> c_lpf_UI;
...
static bool faust_init_inputs(t_faust_c_lpf* x)
{
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(c_lpf));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_c_lpf* x, bool info_outlet)
{
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(c_lpf));
            return false;
        }
...
static bool faust_new_internal(t_faust_c_lpf* x, const std::string& objId = "", bool info_outlet = true)
{
    ...
    x->dsp = new c_lpf();
    x->ui = new PdUI<c_lpf_UI>(sym(c_lpf), objId);
...
static void internal_setup(t_symbol* s, bool soundIn = true)
{
    ...
    ceammc::register_faust_external(c_lpf_faust_class);
}
...
#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ec_lpf_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".c_lpf~"), false); \
    }
