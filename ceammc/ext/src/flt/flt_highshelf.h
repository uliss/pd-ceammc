...
class highshelf : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "flt_highshelf");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt_highshelf");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp5 = (fTemp2 * (fTemp0 + 1.0f));
			float fTemp6 = ((fTemp0 + fTemp4) + (1.0f - fTemp3));
			fRec0[0] = (float(input0[i]) - (((fRec0[2] * (fTemp0 + (1.0f - (fTemp3 + fTemp4)))) + (2.0f * (fRec0[1] * (fTemp0 + (-1.0f - fTemp5))))) / fTemp6));
			float fTemp7 = (fTemp0 + fTemp3);
			output0[i] = FAUSTFLOAT(((((fRec0[1] * (0.0f - (2.0f * fTemp0))) * ((fTemp0 + fTemp5) + -1.0f)) + (fTemp0 * ((fRec0[2] * (fTemp7 + (1.0f - fTemp4))) + (fRec0[0] * ((fTemp4 + fTemp7) + 1.0f))))) / fTemp6));
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
			fRec0[2] = fRec0[1];
			fRec0[1] = fRec0[0];
...
static t_class* highshelf_faust_class;
#define FAUST_EXT t_faust_highshelf
#define FAUST_EXT_CLASS highshelf_faust_class
// clang-format on

template <class T>
class _highshelf_UI : public UI {
};
typedef _highshelf_UI<highshelf> highshelf_UI;

struct t_faust_highshelf {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_highshelf* x = reinterpret_cast<t_faust_highshelf*>(w[1]);
...
static void highshelf_faust_dsp(t_faust_highshelf* x, t_signal** sp)
{
...
static void highshelf_dump_to_console(t_faust_highshelf* x)
{
...
static void highshelf_faust_any(t_faust_highshelf* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_highshelf* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_highshelf* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_highshelf* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_highshelf* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_highshelf* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void highshelf_faust_free(t_faust_highshelf* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_highshelf* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(highshelf));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_highshelf* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(highshelf));
            return false;
        }
...
static bool faust_new_internal(t_faust_highshelf* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new highshelf();
    x->ui = new PdUI<highshelf_UI>(sym(highshelf), objId);

    if (!faust_init_inputs(x)) {
        highshelf_faust_free(x);
        return false;
    }
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_highshelf* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_highshelf* pd_obj()
    {
        return this->x_;
    }
};

static void* highshelf_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    highshelf_faust_class = class_new(s, reinterpret_cast<t_newmethod>(highshelf_faust_new),
        reinterpret_cast<t_method>(highshelf_faust_free),
        sizeof(t_faust_highshelf),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(highshelf_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(highshelf_faust_class, t_faust_highshelf, f);
    }

    class_addmethod(highshelf_faust_class, reinterpret_cast<t_method>(highshelf_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(highshelf_faust_class, reinterpret_cast<t_method>(highshelf_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(highshelf_faust_class, highshelf_faust_any);
    ceammc::register_faust_external(highshelf_faust_class);
}

#define EXTERNAL_NEW void* highshelf_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* highshelf_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_highshelf* x = reinterpret_cast<t_faust_highshelf*>(pd_new(highshelf_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ehighshelf_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".highshelf~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ehighshelf_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".highshelf~"), false); \
    }
