...
#ifndef FAUSTCLASS 
#define FAUSTCLASS c_lowshelf
#endif

class c_lowshelf : public dsp {
  private:
	FAUSTFLOAT 	fslider0;
	float 	fRec0[2];
	float 	fConst0;
  ...
  public:
	...
	virtual c_lowshelf* clone() {
		return new c_lowshelf();
	}
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp2 = ((sqrtf(fTemp0) * sinf(fTemp1)) / max(0.001f, fRec2[0]));
			float fTemp3 = (fTemp0 + fTemp2);
			float fTemp4 = cosf(fTemp1);
			float fTemp5 = ((fTemp0 + -1) * fTemp4);
			float fTemp6 = ((fTemp3 + fTemp5) + 1);
			output0[i] = (FAUSTFLOAT)((fTemp0 * (fTemp3 + (1 - fTemp5))) / fTemp6);
			float fTemp7 = (fTemp4 * (fTemp0 + 1));
			output1[i] = (FAUSTFLOAT)(2 * ((fTemp0 * (fTemp0 + (-1 - fTemp7))) / fTemp6));
			output2[i] = (FAUSTFLOAT)((fTemp0 * (fTemp0 + (1 - (fTemp2 + fTemp5)))) / fTemp6);
			output3[i] = (FAUSTFLOAT)((0 - (2 * ((fTemp0 + fTemp7) + -1))) / fTemp6);
			output4[i] = (FAUSTFLOAT)(((fTemp0 + fTemp5) + (1 - fTemp2)) / fTemp6);
			// post processing
			fRec2[1] = fRec2[0];
			fRec1[1] = fRec1[0];
			fRec0[1] = fRec0[0];
...
static t_class* c_lowshelf_faust_class;
#define FAUST_EXT t_faust_c_lowshelf
...
{
    t_faust_c_lowshelf* x = reinterpret_cast<t_faust_c_lowshelf*>(w[1]);
...
static void c_lowshelf_faust_dsp(t_faust_c_lowshelf* x, t_signal** sp)
{
...
static void c_lowshelf_dump_to_console(t_faust_c_lowshelf* x)
{
...
static void c_lowshelf_faust_any(t_faust_c_lowshelf* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_c_lowshelf* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_c_lowshelf* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_c_lowshelf* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_c_lowshelf* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_c_lowshelf* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void c_lowshelf_faust_free(t_faust_c_lowshelf* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_c_lowshelf* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(c_lowshelf));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_c_lowshelf* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(c_lowshelf));
            return false;
        }
...
static bool faust_new_internal(t_faust_c_lowshelf* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_c_lowshelf* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
    ...
    t_faust_c_lowshelf* pd_obj()
    {
        return this->x_;
    }
};

static void* c_lowshelf_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s)
{
    c_lowshelf_faust_class = class_new(s, reinterpret_cast<t_newmethod>(c_lowshelf_faust_new),
        reinterpret_cast<t_method>(c_lowshelf_faust_free),
        sizeof(t_faust_c_lowshelf),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);
    class_addmethod(c_lowshelf_faust_class, nullfn, &s_signal, A_NULL);
    ...
    CLASS_MAINSIGNALIN(c_lowshelf_faust_class, t_faust_c_lowshelf, f);
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* c_lowshelf_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_c_lowshelf* x = reinterpret_cast<t_faust_c_lowshelf*>(pd_new(c_lowshelf_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ec_lowshelf_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".c_lowshelf~"));    \
    }
