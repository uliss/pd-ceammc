...
#ifndef FAUSTCLASS 
#define FAUSTCLASS resonbp
#endif

class resonbp : public dsp {
  private:
	float 	fConst0;
	FAUSTFLOAT 	fslider0;
	float 	fRec1[2];
  ...
  public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("q", &fslider1, 8e+01f, 0.1f, 1.5e+02f, 0.1f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			fRec0[0] = ((float)input0[i] - (((fRec0[2] * (((fTemp1 - fTemp2) / fTemp0) + 1)) + (2 * (fRec0[1] * (1 - (1.0f / faustpower<2>(fTemp0)))))) / fTemp3));
			output0[i] = (FAUSTFLOAT)(((fRec0[2] * (0 - fTemp1)) + (fRec0[0] / fTemp0)) / fTemp3);
			// post processing
...
static t_class* resonbp_faust_class;
#define FAUST_EXT t_faust_resonbp
...
{
    t_faust_resonbp* x = reinterpret_cast<t_faust_resonbp*>(w[1]);
...
static void resonbp_faust_dsp(t_faust_resonbp* x, t_signal** sp)
{
...
static void resonbp_dump_to_console(t_faust_resonbp* x)
{
...
static void resonbp_faust_any(t_faust_resonbp* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_resonbp* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_resonbp* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_resonbp* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_resonbp* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_resonbp* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void resonbp_faust_free(t_faust_resonbp* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_resonbp* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(resonbp));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_resonbp* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(resonbp));
            return false;
        }
...
static bool faust_new_internal(t_faust_resonbp* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_resonbp* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* resonbp_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_resonbp* x = reinterpret_cast<t_faust_resonbp*>(pd_new(resonbp_faust_class)); \
