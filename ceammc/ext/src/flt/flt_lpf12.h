...
static float lpf12_faustpower2_f(float value) {
	return (value * value);
	
}

#ifndef FAUSTCLASS 
#define FAUSTCLASS lpf12
#endif
#ifdef __APPLE__ 
...
class lpf12 : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt_lpf12");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			fRec0[0] = (float(input0[i]) - (((fRec0[2] * (((fTemp1 + -1.41421354f) / fTemp0) + 1.0f)) + (2.0f * (fRec0[1] * (1.0f - (1.0f / lpf12_faustpower2_f(fTemp0)))))) / fTemp2));
			output0[i] = FAUSTFLOAT(((fRec0[2] + (fRec0[0] + (2.0f * fRec0[1]))) / fTemp2));
			fRec1[1] = fRec1[0];
			fRec0[2] = fRec0[1];
			fRec0[1] = fRec0[0];
...
static t_class* lpf12_faust_class;
#define FAUST_EXT t_faust_lpf12
#define FAUST_EXT_CLASS lpf12_faust_class
// clang-format on

template <class T>
class _lpf12_UI : public UI {
};
typedef _lpf12_UI<lpf12> lpf12_UI;

struct t_faust_lpf12 {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_lpf12* x = reinterpret_cast<t_faust_lpf12*>(w[1]);
...
static void lpf12_faust_dsp(t_faust_lpf12* x, t_signal** sp)
{
...
static void lpf12_dump_to_console(t_faust_lpf12* x)
{
...
static void lpf12_faust_any(t_faust_lpf12* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_lpf12* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_lpf12* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_lpf12* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_lpf12* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_lpf12* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void lpf12_faust_free(t_faust_lpf12* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_lpf12* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(lpf12));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_lpf12* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(lpf12));
            return false;
        }
...
static bool faust_new_internal(t_faust_lpf12* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new lpf12();
    x->ui = new PdUI<lpf12_UI>(sym(lpf12), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_lpf12* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_lpf12* pd_obj()
    {
        return this->x_;
    }
};

static void* lpf12_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    lpf12_faust_class = class_new(s, reinterpret_cast<t_newmethod>(lpf12_faust_new),
        reinterpret_cast<t_method>(lpf12_faust_free),
        sizeof(t_faust_lpf12),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(lpf12_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(lpf12_faust_class, t_faust_lpf12, f);
    }

    class_addmethod(lpf12_faust_class, reinterpret_cast<t_method>(lpf12_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(lpf12_faust_class, reinterpret_cast<t_method>(lpf12_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(lpf12_faust_class, lpf12_faust_any);
    ceammc::register_faust_external(lpf12_faust_class);
}

#define EXTERNAL_NEW void* lpf12_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* lpf12_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_lpf12* x = reinterpret_cast<t_faust_lpf12*>(pd_new(lpf12_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2elpf12_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".lpf12~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2elpf12_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".lpf12~"), false); \
    }
