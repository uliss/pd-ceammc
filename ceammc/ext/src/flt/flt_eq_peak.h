...
class eq_peak : public dsp {
  ...
  public:
	...
	virtual void instanceConstants(int samplingFreq) {
		...
		fConst2 = float((1.0f / fConst0));
		fConst3 = (3.1415927f * fConst2);
		fConst4 = (6.2831855f * fConst2);
	}
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			output0[i] = (FAUSTFLOAT)(((fTemp1 + (fRec0[0] * (((fTemp2 + fTemp9) / fTemp0) + 1))) + (fRec0[2] * (1 - ((fTemp9 - fTemp2) / fTemp0)))) / fTemp8);
			// post processing
...
static bool faust_init_inputs(t_faust_eq_peak* x)
{
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(eq_peak));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_eq_peak* x, bool info_outlet)
{
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(eq_peak));
            return false;
        }
