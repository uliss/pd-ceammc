...
class biquad : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filters.lib/tf21:author", "Julius O. Smith III");
		m->declare("filters.lib/tf21:copyright", "Copyright (C) 2003-2019 by Julius O. Smith III <jos@ccrma.stanford.edu>");
		m->declare("filters.lib/tf21:license", "MIT-style STK-4.3 license");
...
static t_class* biquad_faust_class;
#define FAUST_EXT t_faust_biquad
...
{
    t_faust_biquad* x = reinterpret_cast<t_faust_biquad*>(w[1]);
    if (!x->dsp || !x->buf)
        return (w + 3);
...
static void biquad_faust_dsp(t_faust_biquad* x, t_signal** sp)
{
    // blocksize
...
static void biquad_dump_to_console(t_faust_biquad* x)
{
...
static void biquad_faust_any(t_faust_biquad* x, t_symbol* s, int argc, t_atom* argv)
{
...
static void faust_free_dsp(t_faust_biquad* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_biquad* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_biquad* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_biquad* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_biquad* x)
{
    if (x->buf) {
...
static void biquad_faust_free(t_faust_biquad* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_biquad* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(biquad));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_biquad* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(biquad));
            return false;
        }
...
static bool faust_new_internal(t_faust_biquad* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = true;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_biquad* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_biquad* pd_obj()
    {
        return this->x_;
    }
};

static void* biquad_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    biquad_faust_class = class_new(s, reinterpret_cast<t_newmethod>(biquad_faust_new),
        reinterpret_cast<t_method>(biquad_faust_free),
        sizeof(t_faust_biquad),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(biquad_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(biquad_faust_class, t_faust_biquad, f);
    }
    ...
    class_addanything(biquad_faust_class, reinterpret_cast<t_method>(biquad_faust_any));
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* biquad_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_biquad* x = reinterpret_cast<t_faust_biquad*>(pd_new(biquad_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ebiquad_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".biquad~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ebiquad_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".biquad~"), false); \
    }
