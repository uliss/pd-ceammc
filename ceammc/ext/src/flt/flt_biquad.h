...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn flt_biquad -scn flt_biquad_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
class FAUST_API flt_biquad_dsp {

    public:

        flt_biquad_dsp() = default;
        virtual ~flt_biquad_dsp() = default;
...
class FAUST_API decorator_dsp : public ::flt_biquad_dsp {
...
/**
 The base class of Meta handler to be used in flt_biquad_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
class flt_biquad : public flt_biquad_dsp {
 ...
 public:
	...
	flt_biquad(const flt_biquad&) = default;
	
	virtual ~flt_biquad() = default;
	
	flt_biquad& operator=(const flt_biquad&) = default;
	
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn flt_biquad -scn flt_biquad_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			fRec0[0] = static_cast<float>(input0[i0]) - (static_cast<float>(input4[i0]) * fRec0[1] + static_cast<float>(input5[i0]) * fRec0[2]);
			output0[i0] = static_cast<FAUSTFLOAT>(static_cast<float>(input1[i0]) * fRec0[0] + static_cast<float>(input2[i0]) * fRec0[1] + static_cast<float>(input3[i0]) * fRec0[2]);
			fRec0[2] = fRec0[1];
			fRec0[1] = fRec0[0];
		}
	}
...
class faust_flt_biquad_tilde : public FaustExternal<flt_biquad> {
public:
    faust_flt_biquad_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(flt_biquad))
    {
    }
};
