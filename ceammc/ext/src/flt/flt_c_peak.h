...
#ifndef FAUSTCLASS 
#define FAUSTCLASS c_peak
#endif

class c_peak : public dsp {
  private:
	float 	fConst0;
	FAUSTFLOAT 	fslider0;
	float 	fRec0[2];
  ...
  public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("gain", &fslider1, 0.0f, -15.0f, 15.0f, 0.1f);
		ui_interface->addVerticalSlider("q", &fslider2, 1.0f, 0.01f, 1e+02f, 0.1f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp2 = powf(10,(0.025f * fRec1[0]));
			fRec2[0] = (fSlow2 + (0.999f * fRec2[1]));
			float fTemp3 = max(0.001f, fRec2[0]);
			float fTemp4 = (0.5f * ((fTemp1 * fTemp2) / fTemp3));
			float fTemp5 = (0.5f * (fTemp1 / (fTemp2 * fTemp3)));
			float fTemp6 = (fTemp5 + 1);
			output0[i] = (FAUSTFLOAT)((fTemp4 + 1) / fTemp6);
			float fTemp7 = ((0 - (2 * cosf(fTemp0))) / fTemp6);
			output1[i] = (FAUSTFLOAT)fTemp7;
			output2[i] = (FAUSTFLOAT)((1 - fTemp4) / fTemp6);
			output3[i] = (FAUSTFLOAT)fTemp7;
			output4[i] = (FAUSTFLOAT)((1 - fTemp5) / fTemp6);
			// post processing
			fRec2[1] = fRec2[0];
			fRec1[1] = fRec1[0];
			fRec0[1] = fRec0[0];
...
static t_class* c_peak_faust_class;
#define FAUST_EXT t_faust_c_peak
...
{
    t_faust_c_peak* x = reinterpret_cast<t_faust_c_peak*>(w[1]);
...
static void c_peak_faust_dsp(t_faust_c_peak* x, t_signal** sp)
{
...
static void c_peak_dump_to_console(t_faust_c_peak* x)
{
...
static void c_peak_faust_any(t_faust_c_peak* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_c_peak* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_c_peak* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_c_peak* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_c_peak* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_c_peak* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void c_peak_faust_free(t_faust_c_peak* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_c_peak* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(c_peak));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_c_peak* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(c_peak));
            return false;
        }
...
static bool faust_new_internal(t_faust_c_peak* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_c_peak* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
    ...
    t_faust_c_peak* pd_obj()
    {
        return this->x_;
    }
};

static void* c_peak_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s)
{
    c_peak_faust_class = class_new(s, reinterpret_cast<t_newmethod>(c_peak_faust_new),
        reinterpret_cast<t_method>(c_peak_faust_free),
        sizeof(t_faust_c_peak),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);
    class_addmethod(c_peak_faust_class, nullfn, &s_signal, A_NULL);
    ...
    CLASS_MAINSIGNALIN(c_peak_faust_class, t_faust_c_peak, f);
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* c_peak_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_c_peak* x = reinterpret_cast<t_faust_c_peak*>(pd_new(c_peak_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ec_peak_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".c_peak~"));    \
    }
