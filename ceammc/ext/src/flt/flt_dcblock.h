...
class dcblock : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("filename", "flt_dcblock.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt_dcblock");
...
static t_class* dcblock_faust_class;
#define FAUST_EXT t_faust_dcblock
...
{
    t_faust_dcblock* x = reinterpret_cast<t_faust_dcblock*>(w[1]);
    if (!x->dsp || !x->buf)
        return (w + 3);
...
static void dcblock_faust_dsp(t_faust_dcblock* x, t_signal** sp)
{
    // blocksize
...
static void dcblock_dump_to_console(t_faust_dcblock* x)
{
...
static void dcblock_faust_any(t_faust_dcblock* x, t_symbol* s, int argc, t_atom* argv)
{
...
static void faust_free_dsp(t_faust_dcblock* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_dcblock* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_dcblock* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_dcblock* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_dcblock* x)
{
    if (x->buf) {
...
static void dcblock_faust_free(t_faust_dcblock* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_dcblock* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(dcblock));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_dcblock* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(dcblock));
            return false;
        }
...
static bool faust_new_internal(t_faust_dcblock* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = true;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_dcblock* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_dcblock* pd_obj()
    {
        return this->x_;
    }
};

static void* dcblock_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    dcblock_faust_class = class_new(s, reinterpret_cast<t_newmethod>(dcblock_faust_new),
        reinterpret_cast<t_method>(dcblock_faust_free),
        sizeof(t_faust_dcblock),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(dcblock_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(dcblock_faust_class, t_faust_dcblock, f);
    }
    ...
    class_addanything(dcblock_faust_class, reinterpret_cast<t_method>(dcblock_faust_any));
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* dcblock_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_dcblock* x = reinterpret_cast<t_faust_dcblock*>(pd_new(dcblock_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2edcblock_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".dcblock~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2edcblock_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".dcblock~"), false); \
    }
