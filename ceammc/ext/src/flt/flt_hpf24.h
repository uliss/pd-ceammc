...
static float hpf24_faustpower2_f(float value) {
	return (value * value);
...
class hpf24 : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt_hpf24");
		ui_interface->declare(&fVslider0, "unit", "Hz");
		ui_interface->addVerticalSlider("freq", &fVslider0, 10000.0f, 20.0f, 20000.0f, 0.100000001f);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp1 = hpf24_faustpower2_f(fTemp0);
			float fTemp2 = (1.0f / fTemp1);
			float fTemp3 = (0.0f - fTemp2);
			float fTemp4 = (1.0f - fTemp2);
			float fTemp5 = (1.0f / fTemp0);
			float fTemp6 = (((fTemp5 + 1.84775901f) / fTemp0) + 1.0f);
			fRec2[0] = (float(input0[i]) - (((2.0f * (fRec2[1] * fTemp4)) + (fRec2[2] * (((fTemp5 + -1.84775901f) / fTemp0) + 1.0f))) / fTemp6));
			float fTemp7 = (((fTemp5 + 0.765366852f) / fTemp0) + 1.0f);
			fRec0[0] = ((((2.0f * (fTemp3 * fRec2[1])) + ((fRec2[2] + fRec2[0]) / fTemp1)) / fTemp6) - (((2.0f * (fRec0[1] * fTemp4)) + (fRec0[2] * (((fTemp5 + -0.765366852f) / fTemp0) + 1.0f))) / fTemp7));
			output0[i] = FAUSTFLOAT(((((fRec0[0] / fTemp1) + (2.0f * (fRec0[1] * fTemp3))) + (fRec0[2] / fTemp1)) / fTemp7));
			fRec1[1] = fRec1[0];
			fRec2[2] = fRec2[1];
			fRec2[1] = fRec2[0];
			fRec0[2] = fRec0[1];
...
static t_class* hpf24_faust_class;
#define FAUST_EXT t_faust_hpf24
#define FAUST_EXT_CLASS hpf24_faust_class
// clang-format on

template <class T>
class _hpf24_UI : public UI {
};
typedef _hpf24_UI<hpf24> hpf24_UI;

struct t_faust_hpf24 {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_hpf24* x = reinterpret_cast<t_faust_hpf24*>(w[1]);
...
static void hpf24_faust_dsp(t_faust_hpf24* x, t_signal** sp)
{
...
static void hpf24_dump_to_console(t_faust_hpf24* x)
{
...
static void hpf24_faust_any(t_faust_hpf24* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_hpf24* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_hpf24* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_hpf24* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_hpf24* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_hpf24* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void hpf24_faust_free(t_faust_hpf24* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_hpf24* x)
{
    x->inputs = NULL;
...
static bool faust_init_outputs(t_faust_hpf24* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
...
static bool faust_new_internal(t_faust_hpf24* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new hpf24();
    x->ui = new PdUI<hpf24_UI>(sym(hpf24), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_hpf24* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_hpf24* pd_obj()
    {
        return this->x_;
    }
};

static void* hpf24_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    hpf24_faust_class = class_new(s, reinterpret_cast<t_newmethod>(hpf24_faust_new),
        reinterpret_cast<t_method>(hpf24_faust_free),
        sizeof(t_faust_hpf24),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(hpf24_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(hpf24_faust_class, t_faust_hpf24, f);
    }

    class_addmethod(hpf24_faust_class, reinterpret_cast<t_method>(hpf24_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(hpf24_faust_class, reinterpret_cast<t_method>(hpf24_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(hpf24_faust_class, hpf24_faust_any);
    ceammc::register_faust_external(hpf24_faust_class);
}

#define EXTERNAL_NEW void* hpf24_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* hpf24_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_hpf24* x = reinterpret_cast<t_faust_hpf24*>(pd_new(hpf24_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ehpf24_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".hpf24~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ehpf24_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".hpf24~"), false); \
    }
