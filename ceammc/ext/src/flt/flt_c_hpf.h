...
class c_hpf : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("flt_c_hpf");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			output1[i] = FAUSTFLOAT(((-1.0f - fTemp1) / fTemp3));
			output2[i] = FAUSTFLOAT(fTemp4);
			output3[i] = FAUSTFLOAT(((0.0f - (2.0f * fTemp1)) / fTemp3));
			output4[i] = FAUSTFLOAT(((1.0f - fTemp2) / fTemp3));
			fRec0[1] = fRec0[0];
			fRec1[1] = fRec1[0];
...
static t_class* c_hpf_faust_class;
#define FAUST_EXT t_faust_c_hpf
...
{
    t_faust_c_hpf* x = reinterpret_cast<t_faust_c_hpf*>(w[1]);
...
static void c_hpf_faust_dsp(t_faust_c_hpf* x, t_signal** sp)
{
...
static void c_hpf_dump_to_console(t_faust_c_hpf* x)
{
...
static void c_hpf_faust_any(t_faust_c_hpf* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_c_hpf* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_c_hpf* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_c_hpf* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_c_hpf* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_c_hpf* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void c_hpf_faust_free(t_faust_c_hpf* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_c_hpf* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(c_hpf));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_c_hpf* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(c_hpf));
            return false;
        }
...
static bool faust_new_internal(t_faust_c_hpf* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_c_hpf* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
    ...
    t_faust_c_hpf* pd_obj()
    {
        return this->x_;
    }
};

static void* c_hpf_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    c_hpf_faust_class = class_new(s, reinterpret_cast<t_newmethod>(c_hpf_faust_new),
        reinterpret_cast<t_method>(c_hpf_faust_free),
        sizeof(t_faust_c_hpf),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(c_hpf_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(c_hpf_faust_class, t_faust_c_hpf, f);
    }
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* c_hpf_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_c_hpf* x = reinterpret_cast<t_faust_c_hpf*>(pd_new(c_hpf_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ec_hpf_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".c_hpf~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ec_hpf_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".c_hpf~"), false); \
    }
