...
float notch_faustpower2_f(float value) {
	return (value * value);
	
}

#ifndef FAUSTCLASS 
#define FAUSTCLASS notch
#endif
#ifdef __APPLE__ 
...
class notch : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("width", &fVslider0, 50.0f, 1.0f, 10000.0f, 0.100000001f);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp1 = notch_faustpower2_f((1.0f - fTemp0));
			float fTemp2 = notch_faustpower2_f((fTemp0 + 1.0f));
			float fTemp3 = (fTemp1 / fTemp2);
			fRec2[0] = (fSlow1 + (0.999000013f * fRec2[1]));
			float fTemp4 = ((fRec0[1] * (-1.0f - fTemp3)) * cosf((fConst2 * fRec2[0])));
			fRec0[0] = (float(input0[i]) - (fTemp4 + ((fTemp1 * fRec0[2]) / fTemp2)));
			output0[i] = FAUSTFLOAT((fTemp4 + (0.5f * ((fTemp3 + 1.0f) * (fRec0[0] + fRec0[2])))));
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
			fRec0[2] = fRec0[1];
			fRec0[1] = fRec0[0];
...
static t_class* notch_faust_class;
#define FAUST_EXT t_faust_notch
#define FAUST_EXT_CLASS notch_faust_class
// clang-format on

struct t_faust_notch {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_notch* x = reinterpret_cast<t_faust_notch*>(w[1]);
...
static void notch_faust_dsp(t_faust_notch* x, t_signal** sp)
{
...
static void notch_dump_to_console(t_faust_notch* x)
{
...
static void notch_faust_any(t_faust_notch* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_notch* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_notch* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_notch* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_notch* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_notch* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void notch_faust_free(t_faust_notch* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_notch* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(notch));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_notch* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(notch));
            return false;
        }
...
static bool faust_new_internal(t_faust_notch* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new notch();
    x->ui = new PdUI<UI>(sym(notch), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_notch* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_notch* pd_obj()
    {
        return this->x_;
    }
};

static void* notch_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    notch_faust_class = class_new(s, reinterpret_cast<t_newmethod>(notch_faust_new),
        reinterpret_cast<t_method>(notch_faust_free),
        sizeof(t_faust_notch),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(notch_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(notch_faust_class, t_faust_notch, f);
    }

    class_addmethod(notch_faust_class, reinterpret_cast<t_method>(notch_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(notch_faust_class, reinterpret_cast<t_method>(notch_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(notch_faust_class, notch_faust_any);
}

#define EXTERNAL_NEW void* notch_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* notch_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_notch* x = reinterpret_cast<t_faust_notch*>(pd_new(notch_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2enotch_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".notch~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2enotch_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".notch~"), false); \
    }
