 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp2 = std::max(0.00100000005f, fRec1[0]);
			...
			output2[i] = FAUSTFLOAT(((0.0f - fTemp3) / fTemp4));
...
static t_class* c_bpf_faust_class;
#define FAUST_EXT t_faust_c_bpf
#define FAUST_EXT_CLASS c_bpf_faust_class
// clang-format on

template <class T>
class _c_bpf_UI : public UI {
};
typedef _c_bpf_UI<c_bpf> c_bpf_UI;

struct t_faust_c_bpf {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_c_bpf* x = reinterpret_cast<t_faust_c_bpf*>(w[1]);
...
static void c_bpf_faust_dsp(t_faust_c_bpf* x, t_signal** sp)
{
...
static void c_bpf_dump_to_console(t_faust_c_bpf* x)
{
...
static void c_bpf_faust_any(t_faust_c_bpf* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_c_bpf* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_c_bpf* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_c_bpf* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_c_bpf* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_c_bpf* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void c_bpf_faust_free(t_faust_c_bpf* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_c_bpf* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(c_bpf));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_c_bpf* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(c_bpf));
            return false;
        }
...
static bool faust_new_internal(t_faust_c_bpf* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new c_bpf();
    x->ui = new PdUI<c_bpf_UI>(sym(c_bpf), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_c_bpf* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_c_bpf* pd_obj()
    {
        return this->x_;
    }
};

static void* c_bpf_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    c_bpf_faust_class = class_new(s, reinterpret_cast<t_newmethod>(c_bpf_faust_new),
        reinterpret_cast<t_method>(c_bpf_faust_free),
        sizeof(t_faust_c_bpf),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(c_bpf_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(c_bpf_faust_class, t_faust_c_bpf, f);
    }

    class_addmethod(c_bpf_faust_class, reinterpret_cast<t_method>(c_bpf_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(c_bpf_faust_class, reinterpret_cast<t_method>(c_bpf_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(c_bpf_faust_class, c_bpf_faust_any);
    ceammc::register_faust_external(c_bpf_faust_class);
}

#define EXTERNAL_NEW void* c_bpf_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* c_bpf_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_c_bpf* x = reinterpret_cast<t_faust_c_bpf*>(pd_new(c_bpf_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ec_bpf_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".c_bpf~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ec_bpf_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".c_bpf~"), false); \
    }
