...
static float bpf24_faustpower2_f(float value) {
	return (value * value);
	
}
static float bpf24_faustpower3_f(float value) {
	return ((value * value) * value);
	
}

#ifndef FAUSTCLASS 
#define FAUSTCLASS bpf24
#endif
#ifdef __APPLE__ 
 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp1 = std::tan((fConst5 * std::min((fRec1[0] * (fTemp0 + 1.0f)), fConst6)));
			float fTemp2 = bpf24_faustpower2_f(std::sqrt((fConst4 * (fTemp1 * std::tan((fConst5 * std::max((fRec1[0] * (1.0f - fTemp0)), 0.0f)))))));
			float fTemp3 = (fConst7 * fTemp2);
			float fTemp4 = ((fConst8 * fTemp1) - (fConst9 * (fTemp2 / fTemp1)));
			float fTemp5 = (5.65685415f * fTemp4);
			float fTemp6 = (fConst10 * fTemp4);
			float fTemp7 = bpf24_faustpower2_f(fTemp4);
			float fTemp8 = (8.0f * fTemp7);
			float fTemp9 = (4.0f * fTemp7);
			float fTemp10 = (fConst1 * fTemp2);
			float fTemp11 = (2.82842708f * fTemp4);
			float fTemp12 = (fConst12 * fTemp4);
			float fTemp13 = ((fTemp12 + (fConst2 * (fTemp9 + (fTemp2 * ((fConst1 * (fTemp10 + fTemp11)) + 8.0f))))) + 16.0f);
			fRec0[0] = (float(input0[i]) - (((fRec0[1] * ((fConst3 * (fTemp2 * (fTemp3 + fTemp5))) + (-64.0f - fTemp6))) + ((fRec0[2] * ((fConst2 * ((fTemp2 * ((fConst11 * fTemp2) + -16.0f)) - fTemp8)) + 96.0f)) + ((fRec0[4] * ((fConst2 * (fTemp9 + (fTemp2 * ((fConst1 * (fTemp10 - fTemp11)) + 8.0f)))) + (16.0f - fTemp12))) + (fRec0[3] * ((fTemp6 + (fConst3 * (fTemp2 * (fTemp3 - fTemp5)))) + -64.0f))))) / fTemp13));
			output0[i] = FAUSTFLOAT((fConst2 * ((((fRec0[2] * (0.0f - fTemp8)) + (4.0f * (fTemp7 * fRec0[0]))) + (4.0f * (fTemp7 * fRec0[4]))) / fTemp13)));
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
...
static t_class* bpf24_faust_class;
#define FAUST_EXT t_faust_bpf24
#define FAUST_EXT_CLASS bpf24_faust_class
// clang-format on

template <class T>
class _bpf24_UI : public UI {
};
typedef _bpf24_UI<bpf24> bpf24_UI;

struct t_faust_bpf24 {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_bpf24* x = reinterpret_cast<t_faust_bpf24*>(w[1]);
...
static void bpf24_faust_dsp(t_faust_bpf24* x, t_signal** sp)
{
...
static void bpf24_dump_to_console(t_faust_bpf24* x)
{
...
static void bpf24_faust_any(t_faust_bpf24* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_bpf24* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_bpf24* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_bpf24* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_bpf24* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_bpf24* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void bpf24_faust_free(t_faust_bpf24* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_bpf24* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(bpf24));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_bpf24* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(bpf24));
            return false;
        }
...
static bool faust_new_internal(t_faust_bpf24* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new bpf24();
    x->ui = new PdUI<bpf24_UI>(sym(bpf24), objId);
...
public:
    ...
    t_faust_bpf24* pd_obj()
    {
        return this->x_;
    }
};

static void* bpf24_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    bpf24_faust_class = class_new(s, reinterpret_cast<t_newmethod>(bpf24_faust_new),
        reinterpret_cast<t_method>(bpf24_faust_free),
        sizeof(t_faust_bpf24),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(bpf24_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(bpf24_faust_class, t_faust_bpf24, f);
    }

    class_addmethod(bpf24_faust_class, reinterpret_cast<t_method>(bpf24_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(bpf24_faust_class, reinterpret_cast<t_method>(bpf24_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(bpf24_faust_class, bpf24_faust_any);
    ceammc::register_faust_external(bpf24_faust_class);
}

#define EXTERNAL_NEW void* bpf24_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* bpf24_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_bpf24* x = reinterpret_cast<t_faust_bpf24*>(pd_new(bpf24_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ebpf24_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".bpf24~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ebpf24_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".bpf24~"), false); \
    }
