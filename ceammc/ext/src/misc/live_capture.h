...
#ifndef FAUSTCLASS 
#define FAUSTCLASS capture
#endif

class capture : public dsp {
  private:
  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			fVec0[IOTA&524287] = ((fSlow0 * (float)input0[i]) + (fSlow1 * fRec0[1]));
			iVec1[0] = iSlow2;
...
static t_class* capture_faust_class;
#define FAUST_EXT t_faust_capture
#define FAUST_EXT_CLASS capture_faust_class
// clang-format on

struct t_faust_capture {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_capture* x = reinterpret_cast<t_faust_capture*>(w[1]);
...
static void capture_faust_dsp(t_faust_capture* x, t_signal** sp)
{
...
static void capture_dump_to_console(t_faust_capture* x)
{
...
static void capture_faust_any(t_faust_capture* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_capture* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_capture* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_capture* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_capture* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_capture* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void capture_faust_free(t_faust_capture* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_capture* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(capture));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_capture* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(capture));
            return false;
        }
...
static bool faust_new_internal(t_faust_capture* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new capture();
    x->ui = new PdUI<UI>(sym(capture), objId);

    if (!faust_init_inputs(x)) {
        capture_faust_free(x);
        return false;
    }
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_capture* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
...
#define EXTERNAL_NEW void* capture_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* capture_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_capture* x = reinterpret_cast<t_faust_capture*>(pd_new(capture_faust_class)); \
