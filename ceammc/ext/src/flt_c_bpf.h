...
class c_bpf : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("q", &fslider1, 1.0f, 0.0f, 1.0f, 0.1f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow5 = (0.5f * (fSlow1 / (fSlow4 * fSlow2)));
...
static void faust_init_label(t_faust* x, const char* obj_id) {
    x->label = new std::string(sym(c_bpf) "~");
...
static bool faust_new_internal(t_faust* x, const char* obj_id = NULL, bool info_outlet = true) {
    ...
    x->dsp = new c_bpf();
    x->ui = new PdUI(sym(c_bpf), obj_id);
