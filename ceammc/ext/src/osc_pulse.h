  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (index = 0; index <= fullcount - 64; index += 64) {
			...
			// exec code
			for (int i=0; i<count; i++) {
				fZec2[i] = ((fConst1 * fZec1[i]) + fRec0[i-1]);
				fRec0[i] = (fZec2[i] - floorf(fZec2[i]));
			}
			// post processing
			...
			// exec code
			for (int i=0; i<count; i++) {
				fZec3[i] = max((float)0, min((float)2047, (fConst2 * (fRec1[i] / fZec0[i]))));
			}
			...
			// exec code
			for (int i=0; i<count; i++) {
				output0[i] = (FAUSTFLOAT)(fConst3 * (fYec1[(fYec1_idx+i) & 4095] - ((fYec1[(fYec1_idx+i-iZec4[i])&4095] * (fZec5[i] + (1 - fZec3[i]))) + ((fZec3[i] - fZec5[i]) * fYec1[(fYec1_idx+i-int((iZec4[i] + 1)))&4095]))));
			}
...
static void faust_init_label(t_faust* x, const char* obj_id) {
    x->label = new std::string(sym(pulse) "~");
...
static bool faust_new_internal(t_faust* x, const char* obj_id = NULL, bool info_outlet = true) {
    ...
    x->dsp = new pulse();
    x->ui = new PdUI(sym(pulse), obj_id);
