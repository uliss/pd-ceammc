...
class smooth : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "env_smooth");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.1");
		m->declare("name", "env_smooth");
...
static t_class* smooth_faust_class;
#define FAUST_EXT t_faust_smooth
#define FAUST_EXT_CLASS smooth_faust_class
// clang-format on

struct t_faust_smooth {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_smooth* x = reinterpret_cast<t_faust_smooth*>(w[1]);
...
static void smooth_faust_dsp(t_faust_smooth* x, t_signal** sp)
{
...
static void smooth_dump_to_console(t_faust_smooth* x)
{
...
static void smooth_faust_any(t_faust_smooth* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_smooth* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_smooth* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_smooth* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_smooth* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_smooth* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void smooth_faust_free(t_faust_smooth* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_smooth* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(smooth));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_smooth* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(smooth));
            return false;
        }
...
static bool faust_new_internal(t_faust_smooth* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new smooth();
    x->ui = new PdUI<UI>(sym(smooth), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_smooth* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
...
#define EXTERNAL_NEW void* smooth_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* smooth_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_smooth* x = reinterpret_cast<t_faust_smooth*>(pd_new(smooth_faust_class)); \
