...
class follow : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "env_follow");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.1");
		m->declare("name", "env_follow");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("env_follow");
		ui_interface->addHorizontalSlider("attack", &fHslider0, 200.0f, 1.0f, 1000.0f, 0.100000001f);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			fRec1[0] = ((fTemp0 * (1.0f - fTemp1)) + (fTemp1 * fRec1[1]));
			fRec0[0] = fRec1[0];
...
static t_class* follow_faust_class;
#define FAUST_EXT t_faust_follow
#define FAUST_EXT_CLASS follow_faust_class
// clang-format on

template <class T>
class _follow_UI : public UI {
};
typedef _follow_UI<follow> follow_UI;

struct t_faust_follow {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_follow* x = reinterpret_cast<t_faust_follow*>(w[1]);
...
static void follow_faust_dsp(t_faust_follow* x, t_signal** sp)
{
...
static void follow_dump_to_console(t_faust_follow* x)
{
...
static void follow_faust_any(t_faust_follow* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_follow* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_follow* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_follow* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_follow* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_follow* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void follow_faust_free(t_faust_follow* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_follow* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(follow));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_follow* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(follow));
            return false;
        }
...
static bool faust_new_internal(t_faust_follow* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new follow();
    x->ui = new PdUI<follow_UI>(sym(follow), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_follow* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_follow* pd_obj()
    {
        return this->x_;
    }
};

static void* follow_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    follow_faust_class = class_new(s, reinterpret_cast<t_newmethod>(follow_faust_new),
        reinterpret_cast<t_method>(follow_faust_free),
        sizeof(t_faust_follow),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(follow_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(follow_faust_class, t_faust_follow, f);
    }

    class_addmethod(follow_faust_class, reinterpret_cast<t_method>(follow_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(follow_faust_class, reinterpret_cast<t_method>(follow_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(follow_faust_class, follow_faust_any);
    ceammc::register_faust_external(follow_faust_class);
}

#define EXTERNAL_NEW void* follow_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* follow_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_follow* x = reinterpret_cast<t_faust_follow*>(pd_new(follow_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2efollow_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".follow~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2efollow_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".follow~"), false); \
    }
