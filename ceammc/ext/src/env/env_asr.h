...
static t_class* asr_faust_class;
#define FAUST_EXT t_faust_asr
...
{
    t_faust_asr* x = reinterpret_cast<t_faust_asr*>(w[1]);
...
static void asr_faust_dsp(t_faust_asr* x, t_signal** sp)
{
...
static void asr_dump_to_console(t_faust_asr* x)
{
...
static void asr_faust_any(t_faust_asr* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_asr* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_asr* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_asr* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_asr* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_asr* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void asr_faust_free(t_faust_asr* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_asr* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(asr));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_asr* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(asr));
            return false;
        }
...
static bool faust_new_internal(t_faust_asr* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_asr* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* asr_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_asr* x = reinterpret_cast<t_faust_asr*>(pd_new(asr_faust_class)); \
