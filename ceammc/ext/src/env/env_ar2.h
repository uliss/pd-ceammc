...
class ar2 : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "env_ar2");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.1");
		m->declare("name", "env_ar2");
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("env_ar2");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			fVec0[0] = fSlow0;
			fVec1[0] = fSlow3;
			fRec0[0] = ((((fSlow0 - fVec0[1]) > 0.0f) > 0)?0.0f:std::min(fSlow4, (fRec0[1] + (1.0f - (fConst1 * (fVec1[1] - fSlow3))))));
...
// clang-format on
    ...
    template <class T>
    struct _ar2_UI : public UI {
    static std::string name;
};

template <class T>
std::string _ar2_UI<T>::name(sym(ar2));

typedef _ar2_UI<ar2> ar2_UI;

class faust_ar2_tilde : public FaustExternal<ar2, ar2_UI> {
public:
    faust_ar2_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args)
    {
    }
};
