...
bool XMidiEvent::parse(const AtomList& l)
{
    ...
    if (valid_) {
        ...
        event_ = MidiEvent(l[0].asInt(), 0, v);
        track_ = l[1].asInt();
...
MidiEventNote::MidiEventNote(const PdArgs& args)
    : BaseObject(args)
    , ev_(AtomList())
{
    createOutlet();
...
void MidiEventNote::onAny(t_symbol* s, const AtomList& args)
{
    if (s != SYM_MIDI_EVENT) {
        OBJ_ERR << "MidiEvent expected: " << s->s_name;
        return floatTo(0, 0);
...
void setup_midi_event()
{
    ...
    ObjectFactory<MidiEventNote> note("midievent.note");
}
