...
static t_symbol** pg_instr_names()
{
    ...
    static_assert(sizeof(PG_INSTR_NAMES) / sizeof(t_symbol*) == 128, "invalid table of instruments size");
    return PG_INSTR_NAMES;
}
...
t_symbol* ceammc::midi::key_to_name(size_t key, const music::Tonality& t, bool up)
{
    using namespace ceammc::music;
    PitchClass p = Tonality::correctAlteration(key, t, up ? ALTERATE_UP : ALTERATE_DOWN);
    return gensym(to_string(p, NAMING_SCHEME_SPN).c_str());
}
