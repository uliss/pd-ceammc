...
class capture : public dsp {
  ...
  public:
	...
	virtual void instanceClear() {
		...
		for (int i=0; i<4; i++) iRec1_perm[i]=0;
		for (int i=0; i<1048576; i++) fYec0[i]=0;
		fYec0_idx = 0;
		fYec0_idx_save = 0;
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (index = 0; index <= fullcount - 64; index += 64) {
			...
			// LOOP 0x7f97a3d2f450
			// pre processing
			for (int i=0; i<4; i++) iRec1_tmp[i]=iRec1_perm[i];
			// exec code
			for (int i=0; i<count; i++) {
				iRec1[i] = (((iSlow1 - iVec0[i-1]) <= 0) * (iSlow1 + iRec1[i-1]));
			}
			// post processing
			for (int i=0; i<4; i++) iRec1_perm[i]=iRec1_tmp[count+i];
			...
			// exec code
			for (int i=0; i<count; i++) {
				fYec0[(fYec0_idx+i)&1048575] = ((fSlow0 * (float)input0[i]) + (fSlow2 * fRec0[i-1]));
				fRec0[i] = fYec0[(fYec0_idx+i-int((int((iRec1[i] + -1)) & 524287)))&1048575];
			}
			// post processing
...
static void faust_init_label(t_faust* x, const char* obj_id) {
    x->label = new std::string(sym(capture) "~");
...
static bool faust_new_internal(t_faust* x, const char* obj_id = NULL, bool info_outlet = true) {
    ...
    x->dsp = new capture();
    x->ui = new PdUI(sym(capture), obj_id);
