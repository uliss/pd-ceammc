...
#ifndef FAUSTCLASS 
#define FAUSTCLASS lorenz
#endif

class lorenz : public dsp {
  private:
	float 	fRec0_perm[4];
	float 	fRec1_perm[4];
	float 	fRec2_perm[4];
  ...
  public:
	...
	virtual void instanceClear() {
		for (int i=0; i<4; i++) fRec0_perm[i]=0.1;
		for (int i=0; i<4; i++) fRec1_perm[i]=0.1;
		for (int i=0; i<4; i++) fRec2_perm[i]=0.1;
	}
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (index = 0; index <= fullcount - 64; index += 64) {
			...
			for (int i=0; i<count; i++) {
				fRec0[i] = (fRec0[i-1] + ((float)input0[i] * ((fRec2[i-1] * fRec1[i-1]) - (2.67f * fRec0[i-1]))));
				fRec1[i] = (fRec1[i-1] + ((float)input0[i] * ((fRec2[i-1] * ((float)input1[i] - fRec0[i-1])) - fRec1[i-1])));
				fRec2[i] = (fRec2[i-1] + ((float)input0[i] * (0 - (10 * (fRec2[i-1] - fRec1[i-1])))));
			}
			// post processing
...
static bool faust_init_inputs(t_faust* x)
{
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(lorenz));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust* x, bool info_outlet)
{
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(lorenz));
            return false;
        }
...
static bool faust_new_internal(t_faust* x, const std::string& objId = "", bool info_outlet = true)
{
    ...
    x->dsp = new lorenz();
    x->ui = new PdUI<UI>(sym(lorenz), objId);
