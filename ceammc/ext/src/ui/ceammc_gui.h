...
t_pd* as_pd(T* x)
{
    return &x->j_box.b_obj.o_obj.te_g.g_pd;
}

template <class T>
t_symbol* preset_id(T* x)
{
    return x->j_box.b_objpreset_id;
}

template <class T>
t_symbol* old_preset_id(T* x)
{
    return x->b_old_preset_id;
}

template <class T>
void bind_preset(T* x, t_symbol* name)
{
    if (!name || name == s_null)
        return;
    ...
    pd_bind(as_pd<T>(x), ceammc::Preset::SYM_PRESET_ALL);
    ceammc::PresetStorage::instance().bindPreset(name);
    ceammc_gui::GuiFactory<T>::acquire_preset_name(name);
}

template <class T>
void unbind_preset(T* x, t_symbol* name)
{
    if (!name || name == s_null)
        return;
    ...
    pd_unbind(as_pd(x), ceammc::Preset::SYM_PRESET_ALL);
    ceammc::PresetStorage::instance().unbindPreset(name);
    ceammc_gui::GuiFactory<T>::release_preset_name(name);
}

template <class T>
void rebind_preset(T* x, t_symbol* from, t_symbol* to)
{
    if (!from || !to)
        return;
    ...
    ceammc_gui::GuiFactory<T>::release_preset_name(from);
    ceammc::PresetStorage::instance().bindPreset(to);
    ceammc_gui::GuiFactory<T>::acquire_preset_name(to);
}

template <class T>
void handle_preset_change(T* x)
{
    if (old_preset_id(x) != preset_id(x)) {
        if (old_preset_id(x) == s_null)
            bind_preset(x, preset_id(x));
        else if (old_preset_id(x) == s_null)
            unbind_preset(x, old_preset_id(x));
        else
            rebind_preset(x, old_preset_id(x), preset_id(x));
    }

    x->b_old_preset_id = x->j_box.b_objpreset_id;
}

template <class T>
void preset_init(T* x, const char* prefix)
{
    x->b_old_preset_id = s_null;
    if (!preset_id(x) || preset_id(x) == s_null) {
        t_symbol* name = ceammc_gui::GuiFactory<T>::generate_preset_name(prefix);
        x->j_box.b_objpreset_id = name;
        bind_preset(x, preset_id(x));
    }
...
template <class T>
void preset_free(T* x)
{
    unbind_preset(x, preset_id(x));
}
...
struct BaseGuiObject {
    ...
    // track @presetname change
    t_symbol* b_old_preset_id;
    const char* preset_name_template;
    // flag fot quick check presets support
    bool use_presets;
    bool init_presets;
    ...
    t_rect rect() const { return j_box.b_rect; }
    float x() const { return j_box.b_rect.x; }
    float y() const { return j_box.b_rect.y; }
    float width() const { return j_box.b_rect.width; }
    float height() const { return j_box.b_rect.height; }

    void send(t_float f)
    {
        t_pd* send = ebox_getsender(&j_box);
        if (send) {
            pd_float(send, f);
        }
    }
    ...
    {
        assert(r);
        ebox_get_rect_for_view(&j_box, r);
    }

    t_object* asObject() { return &j_box.b_obj.o_obj; }
    t_pd* asPd() { return as_pd(this); }
    t_symbol* presetId() { return preset_id(this); }
};
...
class GuiFactory {
    bool use_presets_;
    typedef std::map<t_symbol*, int> PresetNameMap;
...
#pragma mark method 'extension' stubs
    ...
    static void wx_load(U* z, t_symbol* s, int argc, t_atom* argv)
    {
        post("load");
    }

    static void wx_store(U* z, t_symbol* s, int argc, t_atom* argv)
    {
        post("store");
    }
...
#pragma mark pd object instance
    ...
    static void* new_method(t_symbol* s, int argc, t_atom* argv)
    {
        ...
        if (z && d) {
            ...
            // init presets
            if (zgetfn(&z->te_g.g_pd, gensym("preset"))) {
                U* ext = asStruct(z);
                ext->use_presets = true;
                preset_init<U>(ext, ext->preset_name_template);
            }
    ...
    static void free_method(t_object* z)
    {
        ...
        if (ext->use_presets)
            unbind_preset(ext, preset_id(ext));
...
public:
    void setup_methods(t_eclass* cl)
    {
        ...
        if(use_presets_) {
            ...
            eclass_addmethod(cl, UI_METHOD_PTR(wx_load),    "load",   A_GIMME, 0);
            eclass_addmethod(cl, UI_METHOD_PTR(wx_store),   "store",   A_GIMME, 0);
        }
    ...
    static t_symbol* generate_preset_name(const char* prefix)
    {
        char buf[32];

        for (int i = 0; i < 1024; i++) {
            snprintf(buf, 30, "%s.%d", prefix, i);
    ...
    static void release_preset_name(t_symbol* s)
    {
    ...
    static void acquire_preset_name(t_symbol* s)
    {
        presets_[s] += 1;
...
template <typename U>
typename GuiFactory<U>::PresetNameMap GuiFactory<U>::presets_;
