...
static t_pd_err slider_notify(t_slider* x, t_symbol* s, t_symbol* msg, void* sender, void* data)
{
    if (msg == cream_sym_attr_modified) {
        ...
        ceammc_gui::handle_preset_change<t_slider>(x);
    }
    return 0;
}
...
static void slider_load(t_slider* x, t_symbol*, int ac, t_atom* av)
{
...
static void slider_store(t_slider* x, t_symbol*, int ac, t_atom* av)
{
    size_t idx = ac > 0 ? atom_getfloat(av) : 0;
...
static void* slider_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    if (x && d) {
        ...
        ceammc_gui::preset_init<t_slider>(x, "ui_slider");
...
{
    ceammc_gui::unbind_preset(x, ceammc_gui::preset_id(x));
...
{
    t_eclass* c = eclass_new("ui.slider", (method)slider_new, (method)slider_free, sizeof(t_slider), 0L, A_GIMME, 0);
    if (c) {
        eclass_guiinit(c, 0);
        // clang-format off
        ...
        eclass_addmethod(c, (method)slider_modify,        "--",            A_GIMME, 0);
        eclass_addmethod(c, (method)slider_load,          "load",          A_GIMME, 0);
        eclass_addmethod(c, (method)slider_store,         "store",         A_GIMME, 0);
