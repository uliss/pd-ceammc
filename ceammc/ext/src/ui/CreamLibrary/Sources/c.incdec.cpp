...
static void incdec_preset(t_incdec*, t_binbuf*) {}

static void incdec_load(t_incdec* x, t_symbol*, int ac, t_atom* av)
{
...
static void incdec_store(t_incdec* x, t_symbol*, int ac, t_atom* av)
{
    size_t idx = ac > 0 ? atom_getfloat(av) : 0;
    ceammc::PresetStorage::instance().setFloatValueAt(ceammc_gui::preset_id(x), idx, x->f_value);
}
...
extern "C" void setup_ui0x2eincdec(void)
{
    ...
    eclass_addmethod(c, (method)incdec_preset, "preset", A_NULL, 0);
    eclass_addmethod(c, (method)incdec_load, "load", A_GIMME, 0);
    eclass_addmethod(c, (method)incdec_store, "store", A_GIMME, 0);
...
void* incdec_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    x->f_setted = 0;
    ebox_ready((t_ebox*)x);
    ceammc_gui::preset_init(x, "ui_incdec");
...
t_pd_err incdec_notify(t_incdec* x, t_symbol* s, t_symbol* msg, void* sender, void* data)
{
    if (msg == cream_sym_attr_modified) {
        if (s == cream_sym_bgcolor || s == cream_sym_bdcolor || s == gensym("arrow_color")) {
