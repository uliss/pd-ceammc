...
static void matrixctrl_load(t_matrixctrl* x, t_symbol*, int ac, t_atom* av)
{
...
static void matrixctrl_store(t_matrixctrl* x, t_symbol*, int ac, t_atom* av)
{
    size_t idx = ac > 0 ? atom_getfloat(av) : 0;

    ceammc::AtomList lst;
    lst.reserve(x->numRows() * x->numCols() * 3);
    for (size_t row = 0; row < x->numRows(); row++) {
        for (size_t col = 0; col < x->numCols(); col++) {
            lst.append(row);
            lst.append(col);
            lst.append(x->value(row, col));
        }
    }
...
static void* matrixctrl_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    if (x && d) {
        ...
        ceammc_gui::preset_init(x, "ui_matrix");
...
extern "C" void setup_ui0x2ematrix(void)
{
    ...
    if (c) {
        ...
        eclass_addmethod(c, (method) matrixctrl_load,            "load",             A_GIMME, 0);
        eclass_addmethod(c, (method) matrixctrl_store,           "store",            A_GIMME, 0);
