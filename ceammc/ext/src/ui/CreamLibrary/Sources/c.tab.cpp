...
static void tab_load(t_tab* x, t_symbol*, int ac, t_atom* av)
{
    using namespace ceammc_gui;
    size_t idx = ac > 0 ? atom_getfloat(av) : 0;
    t_float v = ceammc::PresetStorage::instance().floatValueAt(preset_id(x), idx);
    pd_float(as_pd(x), v);
}

static void tab_store(t_tab* x, t_symbol*, int ac, t_atom* av)
{
    size_t idx = ac > 0 ? atom_getfloat(av) : 0;
    ceammc::PresetStorage::instance().setFloatValueAt(ceammc_gui::preset_id(x), idx, x->f_item_selected);
}
...
static t_pd_err tab_notify(t_tab* x, t_symbol* s, t_symbol* msg, void* sender, void* data)
{
    if (msg == cream_sym_attr_modified) {
        if (s == cream_sym_bgcolor || s == cream_sym_bdcolor || s == cream_sym_textcolor || s == cream_sym_hocolor || s == cream_sym_secolor || s == cream_sym_orientation || s == cream_sym_fontsize || s == cream_sym_fontname || s == cream_sym_fontweight || s == cream_sym_fontslant) {
            ebox_invalidate_layer((t_ebox*)x, cream_sym_selection_layer);
            ebox_invalidate_layer((t_ebox*)x, cream_sym_text_layer);
            ebox_invalidate_layer((t_ebox*)x, cream_sym_background_layer);
        } else if (s == cream_sym_fontsize || s == cream_sym_orientation || s == cream_sym_items) {
            ebox_notify((t_ebox*)x, s_size, cream_sym_attr_modified, NULL, NULL);
        }

        ceammc_gui::handle_preset_change(x);
    }
    return 0;
}
...
static void* tab_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    if (x && d) {
        ...
        ceammc_gui::preset_init<t_tab>(x, "ui_tab");
    }
...
{
    ceammc_gui::preset_free(x);

    outlet_free(x->f_out_index);
    outlet_free(x->f_out_item);
    outlet_free(x->f_out_hover);
    ebox_free((t_ebox*)x);
}

extern "C" void setup_ui0x2etab(void)
{
    t_eclass* c = eclass_new("c.tab", (method)tab_new, (method)tab_free, sizeof(t_tab), 0L, A_GIMME, 0);
    ...
    if(c) {
        ...
        eclass_addmethod(c, (method) tab_load,             "load",            A_GIMME, 0);
        eclass_addmethod(c, (method) tab_store,            "store",           A_GIMME, 0);
        ...
        CLASS_ATTR_LABEL                (c, "toggle", 0, _("Toggle Mode"));
