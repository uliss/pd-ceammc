...
static void* rslider_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    if (x && d) {
        ...
        ceammc_gui::preset_init(x, "ui_slider");
...
static void rslider_free(t_rslider* x)
{
    ceammc_gui::preset_free(x);

    outlet_free(x->f_out_left);
    ebox_free((t_ebox*)x);
}
...
static t_pd_err rslider_notify(t_rslider* x, t_symbol* s, t_symbol* msg, void* sender, void* data)
{
    if (msg == cream_sym_attr_modified) {
        ...
        ceammc_gui::handle_preset_change<t_rslider>(x);
    }
    return 0;
}
...
static void rslider_load(t_rslider* x, t_symbol*, int ac, t_atom* av)
{
    using namespace ceammc_gui;
    size_t idx = ac > 0 ? atom_getfloat(av) : 0;
    ceammc::AtomList lst = ceammc::PresetStorage::instance().listValueAt(preset_id(x), idx);
    pd_list(as_pd(x), &s_list, lst.size(), lst.toPdData());
}

static void rslider_store(t_rslider* x, t_symbol*, int ac, t_atom* av)
{
    size_t idx = ac > 0 ? atom_getfloat(av) : 0;
    ceammc::AtomList lst(2, x->out_list);
    ceammc::PresetStorage::instance().setListValueAt(ceammc_gui::preset_id(x), idx, lst);
}

extern "C" void setup_ui0x2erslider(void)
{
    t_eclass* c = eclass_new("c.rslider", (method)rslider_new, (method)rslider_free, sizeof(t_rslider), 0L, A_GIMME, 0);
    ...
    if (c) {
        ...
        eclass_addmethod(c, (method) rslider_load,            "load",             A_GIMME, 0);
        eclass_addmethod(c, (method) rslider_store,           "store",            A_GIMME, 0);
