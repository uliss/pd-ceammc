...
static t_pd_err toggle_notify(t_toggle* x, t_symbol* s, t_symbol* msg, void* sender, void* data)
{
    if (msg == cream_sym_attr_modified) {
        ...
        ceammc_gui::handle_preset_change<t_toggle>(x);
    }
    return 0;
}
...
static void* toggle_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    if (x && d) {
        ...
        ceammc_gui::preset_init<t_toggle>(x, "ui_toggle");
...
static void toggle_load(t_toggle* x, t_symbol*, int ac, t_atom* av)
{
...
static void toggle_store(t_toggle* x, t_symbol*, int ac, t_atom* av)
{
    size_t idx = ac > 0 ? atom_getfloat(av) : 0;
    ceammc::PresetStorage::instance().setFloatValueAt(ceammc_gui::preset_id(x), idx, x->f_active);
}
...
{
    t_eclass* c = eclass_new("ui.toggle", (method)toggle_new, (method)toggle_free, sizeof(t_toggle), 0L, A_GIMME, 0);
    if (c) {
        // clang-format off
        eclass_guiinit(c, 0);
        ...
        eclass_addmethod(c, (method) toggle_load,            "load",             A_GIMME, 0);
        eclass_addmethod(c, (method) toggle_store,           "store",            A_GIMME, 0);
