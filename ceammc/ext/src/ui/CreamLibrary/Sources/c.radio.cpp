...
static bool isChecked(const ceammc::Atom& a)
{
    return (a.asFloat() > 0);
}

static void radio_load(t_radio* x, t_symbol*, int ac, t_atom* av)
{
    ...
    AtomList lst = PresetStorage::instance().listValueAt(preset_id(x), idx);
    if (x->f_mode) {
        pd_list(as_pd(x), &s_list, lst.size(), lst.toPdData());
    } else {
        pd_float(as_pd(x), lst.findPos(isChecked));
    }
}

static void radio_store(t_radio* x, t_symbol*, int ac, t_atom* av)
{
    using namespace ceammc;
    ...
    AtomList lst;
    for (int i = 0; i < x->f_nitems; i++)
        lst.append(x->f_items[i]);

    PresetStorage::instance().setListValueAt(ceammc_gui::preset_id(x), idx, lst);
}
...
static void* radio_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    if (x && d) {
        ...
        ceammc_gui::preset_init(x, "ui_radio");
...
{
    t_eclass* c = eclass_new("ui.radio", (method)radio_new, (method)radio_free, sizeof(t_radio), 0L, A_GIMME, 0);
    if (c) {
        // clang-format off
        eclass_guiinit(c, 0);
        ...
        eclass_addmethod(c, (method) radio_load,            "load",             A_GIMME, 0);
        eclass_addmethod(c, (method) radio_store,           "store",            A_GIMME, 0);
