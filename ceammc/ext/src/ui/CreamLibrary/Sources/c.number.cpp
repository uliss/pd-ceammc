...
static void number_output(t_number* x)
{
    ...
    outlet_float(x->f_out, x->f_value);
    if (send) {
...
static void number_load(t_number* x, t_symbol*, int ac, t_atom* av)
{
...
static void number_store(t_number* x, t_symbol*, int ac, t_atom* av)
{
    size_t idx = ac > 0 ? atom_getfloat(av) : 0;
...
{
    t_eclass* c = eclass_new("ui.number", (method)number_new, (method)number_free, sizeof(t_number), 0L, A_GIMME, 0);
    ...
    if (c) {
        ...
        eclass_addmethod(c, (method) number_modify,           "--",              A_GIMME, 0);
        eclass_addmethod(c, (method) number_load,             "load",            A_GIMME, 0);
        eclass_addmethod(c, (method) number_store,            "store",           A_GIMME, 0);
