...
static t_symbol* SYM_LIST_LAYER = gensym("list_layer");
...
static void menu_load(t_menu* x, t_symbol*, int ac, t_atom* av);
static void menu_store(t_menu* x, t_symbol*, int ac, t_atom* av);

extern "C" void setup_ui0x2emenu(void)
{
    t_eclass* c = eclass_new("c.menu", (method)menu_new, (method)menu_free, sizeof(t_menu), 0L, A_GIMME, 0);
    // clang-format off
    ...
    eclass_addmethod(c, (method) menu_load,             "load",            A_GIMME, 0);
    eclass_addmethod(c, (method) menu_store,            "store",           A_GIMME, 0);
...
static void* menu_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    if (x && d) {
        ...
        ceammc_gui::preset_init(x, "ui_menu");
...
void menu_append(t_menu* x, t_symbol* s, int argc, t_atom* argv)
{
    ...
    if (argc && argv && menu_symbol_exist(x, item) == -1) {
        ...
        ebox_invalidate_layer((t_ebox*)x, SYM_LIST_LAYER);
...
{
    t_elayer* g = ebox_start_layer((t_ebox*)x, SYM_LIST_LAYER, rect->width, rect->height);
    t_etext* jtl = etext_layout_create();
    if (g && jtl) {
        egraphics_set_color_rgba(g, &x->color_border);
        ...
        ebox_end_layer((t_ebox*)x, SYM_LIST_LAYER);
    }
    ebox_paint_layer((t_ebox*)x, SYM_LIST_LAYER, 0., 0.);
}
...
void menu_load(t_menu* x, t_symbol*, int ac, t_atom* av)
{
...
void menu_store(t_menu* x, t_symbol*, int ac, t_atom* av)
{
    size_t idx = ac > 0 ? atom_getfloat(av) : 0;
