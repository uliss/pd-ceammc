...
{
    if (prop_mouse_events)
        anyTo(0, gensym("@mouseup"), AtomList({ 1 }));
    ...
    if (prop_mouse_events)
        anyTo(0, gensym("@mouseup"), AtomList({ 0 }));
}
...
void UIRSlider::setup()
{
    ...
    obj.addProperty("mouse", _("Mouse events"), false, &UIRSlider::prop_mouse_events, _("Main"));
