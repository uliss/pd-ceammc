...
class crackle : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "noise_crackle");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.1");
		m->declare("name", "noise_crackle");
...
static t_class* crackle_faust_class;
#define FAUST_EXT t_faust_crackle
...
{
    t_faust_crackle* x = reinterpret_cast<t_faust_crackle*>(w[1]);
...
static void crackle_faust_dsp(t_faust_crackle* x, t_signal** sp)
{
...
static void crackle_dump_to_console(t_faust_crackle* x)
{
...
static void crackle_faust_any(t_faust_crackle* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_crackle* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_crackle* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_crackle* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_crackle* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_crackle* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void crackle_faust_free(t_faust_crackle* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_crackle* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(crackle));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_crackle* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(crackle));
            return false;
        }
...
static bool faust_new_internal(t_faust_crackle* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_crackle* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* crackle_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_crackle* x = reinterpret_cast<t_faust_crackle*>(pd_new(crackle_faust_class)); \
