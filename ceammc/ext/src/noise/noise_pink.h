...
class pink : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("filename", "noise_pink.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("noise_pink");
...
static t_class* pink_faust_class;
#define FAUST_EXT t_faust_pink
#define FAUST_EXT_CLASS pink_faust_class
// clang-format on

template <class T>
class _pink_UI : public UI {
};
typedef _pink_UI<pink> pink_UI;
...
{
    t_faust_pink* x = reinterpret_cast<t_faust_pink*>(w[1]);
    if (!x->dsp || !x->buf)
        return (w + 3);
...
static void pink_faust_dsp(t_faust_pink* x, t_signal** sp)
{
    // blocksize
...
static void pink_dump_to_console(t_faust_pink* x)
{
...
static void pink_faust_any(t_faust_pink* x, t_symbol* s, int argc, t_atom* argv)
{
...
static void faust_free_dsp(t_faust_pink* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_pink* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_pink* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_pink* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_pink* x)
{
    if (x->buf) {
...
static void pink_faust_free(t_faust_pink* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_pink* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(pink));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_pink* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(pink));
            return false;
        }
...
static bool faust_new_internal(t_faust_pink* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = true;
    x->xfade = 0;
    ...
    x->dsp = new pink();
    x->ui = new PdUI<pink_UI>(sym(pink), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_pink* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
...
static void* pink_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    pink_faust_class = class_new(s, reinterpret_cast<t_newmethod>(pink_faust_new),
        reinterpret_cast<t_method>(pink_faust_free),
        sizeof(t_faust_pink),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(pink_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(pink_faust_class, t_faust_pink, f);
    }

    class_addmethod(pink_faust_class, reinterpret_cast<t_method>(pink_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(pink_faust_class, reinterpret_cast<t_method>(pink_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(pink_faust_class, reinterpret_cast<t_method>(pink_faust_any));
    ceammc::register_faust_external(pink_faust_class);
}

#define EXTERNAL_NEW void* pink_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* pink_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_pink* x = reinterpret_cast<t_faust_pink*>(pd_new(pink_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2epink_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".pink~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2epink_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".pink~"), false); \
    }
