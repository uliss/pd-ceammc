...
class lfreq0 : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "noise_lfreq0");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("noise_lfreq0");
...
static t_class* lfreq0_faust_class;
#define FAUST_EXT t_faust_lfreq0
#define FAUST_EXT_CLASS lfreq0_faust_class
// clang-format on

template <class T>
class _lfreq0_UI : public UI {
};
typedef _lfreq0_UI<lfreq0> lfreq0_UI;

struct t_faust_lfreq0 {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_lfreq0* x = reinterpret_cast<t_faust_lfreq0*>(w[1]);
...
static void lfreq0_faust_dsp(t_faust_lfreq0* x, t_signal** sp)
{
...
static void lfreq0_dump_to_console(t_faust_lfreq0* x)
{
...
static void lfreq0_faust_any(t_faust_lfreq0* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_lfreq0* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_lfreq0* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_lfreq0* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_lfreq0* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_lfreq0* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void lfreq0_faust_free(t_faust_lfreq0* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_lfreq0* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(lfreq0));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_lfreq0* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(lfreq0));
            return false;
        }
...
static bool faust_new_internal(t_faust_lfreq0* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new lfreq0();
    x->ui = new PdUI<lfreq0_UI>(sym(lfreq0), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_lfreq0* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_lfreq0* pd_obj()
    {
        return this->x_;
    }
};

static void* lfreq0_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    lfreq0_faust_class = class_new(s, reinterpret_cast<t_newmethod>(lfreq0_faust_new),
        reinterpret_cast<t_method>(lfreq0_faust_free),
        sizeof(t_faust_lfreq0),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(lfreq0_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(lfreq0_faust_class, t_faust_lfreq0, f);
    }

    class_addmethod(lfreq0_faust_class, reinterpret_cast<t_method>(lfreq0_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(lfreq0_faust_class, reinterpret_cast<t_method>(lfreq0_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(lfreq0_faust_class, lfreq0_faust_any);
    ceammc::register_faust_external(lfreq0_faust_class);
}

#define EXTERNAL_NEW void* lfreq0_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* lfreq0_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_lfreq0* x = reinterpret_cast<t_faust_lfreq0*>(pd_new(lfreq0_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2elfreq0_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".lfreq0~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2elfreq0_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".lfreq0~"), false); \
    }
