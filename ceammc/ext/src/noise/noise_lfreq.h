...
class lfreq : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "noise_lfreq");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp2 = (0.0f - ((1.0f - fTemp0) / fTemp1));
			float fTemp3 = (fConst2 * fRec1[0]);
			float fTemp4 = sinf(fTemp3);
			float fTemp5 = cosf(fTemp3);
			fRec7[0] = ((fRec8[1] * fTemp4) + (fRec7[1] * fTemp5));
			fRec8[0] = (((fRec8[1] * fTemp5) + (fRec7[1] * (0.0f - fTemp4))) + float((1 - iVec0[1])));
			int iTemp6 = ((fRec7[1] <= 0.0f) & (fRec7[0] > 0.0f));
			iRec9[0] = ((1103515245 * iRec9[1]) + 12345);
			fRec6[0] = ((fRec6[1] * float((1 - iTemp6))) + (4.65661287e-10f * float((iRec9[0] * iTemp6))));
			fRec5[0] = ((fTemp2 * fRec5[1]) + ((fRec6[0] + fRec6[1]) / fTemp1));
			fRec4[0] = ((fTemp2 * fRec4[1]) + ((fRec5[0] + fRec5[1]) / fTemp1));
			fRec3[0] = ((fTemp2 * fRec3[1]) + ((fRec4[0] + fRec4[1]) / fTemp1));
			fRec2[0] = ((fTemp2 * fRec2[1]) + ((fRec3[0] + fRec3[1]) / fTemp1));
			fRec0[0] = ((fRec0[1] * fTemp2) + ((fRec2[0] + fRec2[1]) / fTemp1));
...
static t_class* lfreq_faust_class;
#define FAUST_EXT t_faust_lfreq
...
{
    t_faust_lfreq* x = reinterpret_cast<t_faust_lfreq*>(w[1]);
...
static void lfreq_faust_dsp(t_faust_lfreq* x, t_signal** sp)
{
...
static void lfreq_dump_to_console(t_faust_lfreq* x)
{
...
static void lfreq_faust_any(t_faust_lfreq* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_lfreq* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_lfreq* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_lfreq* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_lfreq* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_lfreq* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void lfreq_faust_free(t_faust_lfreq* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_lfreq* x)
{
    x->inputs = NULL;
...
static bool faust_init_outputs(t_faust_lfreq* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
...
static bool faust_new_internal(t_faust_lfreq* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_lfreq* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* lfreq_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_lfreq* x = reinterpret_cast<t_faust_lfreq*>(pd_new(lfreq_faust_class)); \
