...
class SynthHat : public faust_synth_hat_tilde {
    UIProperty* gate_;
    UIProperty* attack_;
    UIProperty* release_;
...
public:
    SynthHat(const PdArgs& args)
        : faust_synth_hat_tilde(args)
...
{
    SoundExternalFactory<SynthHat> obj("synth.hat~", OBJECT_FACTORY_DEFAULT);
    obj.addMethod("reset", &SynthHat::m_reset);

    obj.setXletsInfo({ "bang: play hat\n"
