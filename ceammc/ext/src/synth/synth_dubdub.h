...
class dubdub : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "synth_dubdub");
...
static t_class* dubdub_faust_class;
#define FAUST_EXT t_faust_dubdub
...
{
    t_faust_dubdub* x = reinterpret_cast<t_faust_dubdub*>(w[1]);
...
static void dubdub_faust_dsp(t_faust_dubdub* x, t_signal** sp)
{
...
static void dubdub_dump_to_console(t_faust_dubdub* x)
{
...
static void dubdub_faust_any(t_faust_dubdub* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_dubdub* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_dubdub* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_dubdub* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_dubdub* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_dubdub* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void dubdub_faust_free(t_faust_dubdub* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_dubdub* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(dubdub));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_dubdub* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(dubdub));
            return false;
        }
...
static bool faust_new_internal(t_faust_dubdub* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_dubdub* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_dubdub* pd_obj()
    {
        return this->x_;
    }
};

static void* dubdub_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    dubdub_faust_class = class_new(s, reinterpret_cast<t_newmethod>(dubdub_faust_new),
        reinterpret_cast<t_method>(dubdub_faust_free),
        sizeof(t_faust_dubdub),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(dubdub_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(dubdub_faust_class, t_faust_dubdub, f);
    }
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* dubdub_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_dubdub* x = reinterpret_cast<t_faust_dubdub*>(pd_new(dubdub_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2edubdub_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".dubdub~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2edubdub_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".dubdub~"), false); \
    }
