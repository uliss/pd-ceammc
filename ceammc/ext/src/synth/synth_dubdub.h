...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn synth_dubdub -scn synth_dubdub_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
class FAUST_API synth_dubdub_dsp {
    ...
    public:
        ...
        synth_dubdub_dsp() = default;
        virtual ~synth_dubdub_dsp() = default;
...
class synth_dubdub : public synth_dubdub_dsp {
 ...
 public:
	...
	synth_dubdub(const synth_dubdub&) = default;
	
	virtual ~synth_dubdub() = default;
	
	synth_dubdub& operator=(const synth_dubdub&) = default;
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn synth_dubdub -scn synth_dubdub_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = fConst2 * static_cast<float>(fButton0);
		float fSlow1 = std::max<float>(1.1920929e-07f, std::fabs(4.4e+02f * std::pow(2.0f, 0.083333336f * (static_cast<float>(fHslider0) + -69.0f))));
		float fSlow2 = fConst3 * fSlow1;
		float fSlow3 = 1.0f - fConst0 / fSlow1;
		float fSlow4 = std::tan(fConst4 * static_cast<float>(fHslider1));
		float fSlow5 = 1.0f / fSlow4;
		float fSlow6 = fConst5 * static_cast<float>(fVslider0);
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			output0[i0] = static_cast<FAUSTFLOAT>((fRec0[2] + fRec0[0] + 2.0f * fRec0[1]) / fTemp4);
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
			fRec4[1] = fRec4[0];
			fRec0[2] = fRec0[1];
