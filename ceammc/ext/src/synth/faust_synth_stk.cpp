...
const double marmstk1[faust::synth::TABLE_SIZE / 4] = {
...
namespace synth {
    ...
    double LookupTable::getValue(double x) const
    {
        ...
        if (i >= size_)
            return points_[(size_ - 1) * 2 + 1];

        double ratio = (x - points_[(i - 1) * 2])
            / (points_[i * 2] - points_[(i - 1) * 2]);

        return points_[(i - 1) * 2 + 1] * (1 - ratio) + points_[i * 2 + 1] * (ratio);
    }
