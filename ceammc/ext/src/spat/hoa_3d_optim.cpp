...
HoaOptim3D::HoaOptim3D(const PdArgs& args)
    : HoaBase(args)
    , mode_(nullptr)
{
...
void HoaOptim3D::initDone()
{
    optim_.reset(new Optim3d(order()));
    adjustMode();
...
void HoaOptim3D::processBlock(const t_sample** in, t_sample** out)
{
...
void HoaOptim3D::adjustMode()
{
    ...
    case hash_basic:
        optim_->setMode(Optim3d::Basic);
        break;
    case hash_maxre:
        optim_->setMode(Optim3d::MaxRe);
        break;
    case hash_inphase:
        optim_->setMode(Optim3d::InPhase);
        break;
    default:
...
{
    SoundExternalFactory<HoaOptim3D> obj("hoa.3d.optim~");
}
