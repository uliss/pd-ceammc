...
#ifndef FAUSTCLASS 
#define FAUSTCLASS pan4
#endif

class pan4 : public dsp {
  private:
	FAUSTFLOAT 	fslider0;
	FAUSTFLOAT 	fslider1;
	float 	fRec0[2];
  ...
  public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("angle", &fslider1, 0.0f, 3.6e+02f, 1e+01f, 0.1f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow2 = (0.0027777778f * float(fslider1));
		float 	fSlow3 = (5e-05f * (fSlow1 * sqrtf(max(0.0f, (1.0f - (4 * (fSlow0 * fabsf((fmodf((fSlow2 + 1.5f),1.0f) + -0.5f)))))))));
		float 	fSlow4 = (5e-05f * (fSlow1 * sqrtf(max(0.0f, (1.0f - (4 * (fSlow0 * fabsf((fmodf((fSlow2 + 1.25f),1.0f) + -0.5f)))))))));
		float 	fSlow5 = (5e-05f * (fSlow1 * sqrtf(max(0.0f, (1.0f - (4 * (fSlow0 * fabsf((fmodf((fSlow2 + 1.0f),1.0f) + -0.5f)))))))));
		float 	fSlow6 = (5e-05f * (fSlow1 * sqrtf(max(0.0f, (1.0f - (4 * (fSlow0 * fabsf((fmodf((fSlow2 + 0.75f),1.0f) + -0.5f)))))))));
...
static t_class* pan4_faust_class;
#define FAUST_EXT t_faust_pan4
...
{
    t_faust_pan4* x = reinterpret_cast<t_faust_pan4*>(w[1]);
...
static void pan4_faust_dsp(t_faust_pan4* x, t_signal** sp)
{
...
static void pan4_dump_to_console(t_faust_pan4* x)
{
...
static void pan4_faust_any(t_faust_pan4* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_pan4* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_pan4* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_pan4* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_pan4* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_pan4* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void pan4_faust_free(t_faust_pan4* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_pan4* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(pan4));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_pan4* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(pan4));
            return false;
        }
...
static bool faust_new_internal(t_faust_pan4* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_pan4* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* pan4_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_pan4* x = reinterpret_cast<t_faust_pan4*>(pd_new(pan4_faust_class)); \
