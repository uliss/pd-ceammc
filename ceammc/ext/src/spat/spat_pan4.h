...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn spat_pan4 -scn spat_pan4_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __spat_pan4_H__
#define  __spat_pan4_H__
...
class FAUST_API spat_pan4_dsp {

    public:

        spat_pan4_dsp() = default;
        virtual ~spat_pan4_dsp() = default;
...
class FAUST_API decorator_dsp : public ::spat_pan4_dsp {
    ...
    public:
        ...
        decorator_dsp(::spat_pan4_dsp* spat_pan4_dsp = nullptr):fDSP(spat_pan4_dsp) {}
...
class FAUST_API dsp_factory {
    ...
    public:
        ...
        virtual ::spat_pan4_dsp* createDSPInstance() = 0;
...
/**
 The base class of Meta handler to be used in spat_pan4_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
// clang-format off
#ifndef FAUST_MACRO
struct spat_pan4 : public spat_pan4_dsp {
};
#endif
// clang-format on
...
class spat_pan4 : public spat_pan4_dsp {
 ...
 public:
	...
	spat_pan4(const spat_pan4&) = default;
	
	virtual ~spat_pan4() = default;
	
	spat_pan4& operator=(const spat_pan4&) = default;
	
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn spat_pan4 -scn spat_pan4_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
		m->declare("filename", "spat_pan4.dsp");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.9.0");
		m->declare("name", "spat.pan4");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow3 = 5e-05f * fSlow1 * std::sqrt(std::max<float>(0.0f, 1.0f - 4.0f * fSlow0 * std::fabs(std::fmod(fSlow2 + 1.5f, 1.0f) + -0.5f)));
		float fSlow4 = 5e-05f * fSlow1 * std::sqrt(std::max<float>(0.0f, 1.0f - 4.0f * fSlow0 * std::fabs(std::fmod(fSlow2 + 1.25f, 1.0f) + -0.5f)));
		float fSlow5 = 5e-05f * fSlow1 * std::sqrt(std::max<float>(0.0f, 1.0f - 4.0f * fSlow0 * std::fabs(std::fmod(fSlow2 + 1.0f, 1.0f) + -0.5f)));
		float fSlow6 = 5e-05f * fSlow1 * std::sqrt(std::max<float>(0.0f, 1.0f - 4.0f * fSlow0 * std::fabs(std::fmod(fSlow2 + 0.75f, 1.0f) + -0.5f)));
...
class faust_spat_pan4_tilde : public FaustExternal<spat_pan4> {
public:
    faust_spat_pan4_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(spat_pan4))
    {
    }
};
