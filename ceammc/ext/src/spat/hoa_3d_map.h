...
class Hoa3dMap : public HoaBase<hoa::Hoa3d> {
    IntProperty* nins_;
    FloatProperty* ramp_;
    SymbolEnumProperty* mode_;

    std::unique_ptr<MultiEncoder3d> map_;
    std::unique_ptr<PolarLines3d> lines_;
...
private:
    ...
    {
        auto ext = reinterpret_cast<Hoa3dMap*>(w[1]);
        ext->processMultiSource();
        return (w + 2);
    }

    static t_int* dspPerformIn1In2In3(t_int* w)
    {
        auto ext = reinterpret_cast<Hoa3dMap*>(w[1]);
        ext->processIn1In2In3();
        return (w + 2);
    }

    bool checkSourceIdx(int idx) const;
    void setPolar(int idx, t_float r, t_float azimuth, t_float elevation);
    void setCartesian(int idx, t_float x, t_float y, t_float z);
