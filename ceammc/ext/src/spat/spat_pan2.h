...
class pan2 : public dsp {
  ...
  public:
	virtual void metadata(Meta* m) { 
		m->declare("spat.lib/name", "Faust Spatialization Library");
		m->declare("spat.lib/version", "0.0");
...
static t_class* pan2_faust_class;
#define FAUST_EXT t_faust_pan2
#define FAUST_EXT_CLASS pan2_faust_class
// clang-format on
...
{
    t_faust_pan2* x = reinterpret_cast<t_faust_pan2*>(w[1]);
...
static void pan2_faust_dsp(t_faust_pan2* x, t_signal** sp)
{
...
static void pan2_dump_to_console(t_faust_pan2* x)
{
...
static void pan2_faust_any(t_faust_pan2* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_pan2* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_pan2* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_pan2* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_pan2* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_pan2* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void pan2_faust_free(t_faust_pan2* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_pan2* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(pan2));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_pan2* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(pan2));
            return false;
        }
...
static bool faust_new_internal(t_faust_pan2* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new pan2();
    x->ui = new PdUI<UI>(sym(pan2), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_pan2* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
...
static void* pan2_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s)
{
    pan2_faust_class = class_new(s, reinterpret_cast<t_newmethod>(pan2_faust_new),
        reinterpret_cast<t_method>(pan2_faust_free),
        sizeof(t_faust_pan2),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);
    class_addmethod(pan2_faust_class, nullfn, &s_signal, A_NULL);
    class_addmethod(pan2_faust_class, reinterpret_cast<t_method>(pan2_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(pan2_faust_class, reinterpret_cast<t_method>(pan2_dump_to_console), gensym("dump"), A_NULL);
    CLASS_MAINSIGNALIN(pan2_faust_class, t_faust_pan2, f);
    class_addanything(pan2_faust_class, pan2_faust_any);
}

#define EXTERNAL_NEW void* pan2_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* pan2_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_pan2* x = reinterpret_cast<t_faust_pan2*>(pd_new(pan2_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2epan2_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".pan2~"));    \
    }
