...
// clang-format off
#ifndef FAUST_MACRO
struct navierstokes : public dsp {
};
#endif
// clang-format on
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS navierstokes
#endif

class navierstokes : public dsp {
  private:
	float 	fRec0_perm[4];
	float 	fRec1_perm[4];
	float 	fRec2_perm[4];
  ...
  public:
	...
	virtual void instanceClear() {
		for (int i=0; i<4; i++) fRec0_perm[i]=1;
		for (int i=0; i<4; i++) fRec1_perm[i]=1;
		for (int i=0; i<4; i++) fRec2_perm[i]=1;
		for (int i=0; i<4; i++) fRec3_perm[i]=1;
		for (int i=0; i<4; i++) fRec4_perm[i]=1;
	}
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (index = 0; index <= fullcount - 64; index += 64) {
			...
			for (int i=0; i<count; i++) {
				fRec0[i] = (fRec0[i-1] + ((float)input0[i] * (0 - (fRec0[i-1] + (3 * (fRec4[i-1] * fRec1[i-1]))))));
				fRec1[i] = (fRec1[i-1] + ((float)input0[i] * (0 - ((5 * fRec1[i-1]) + (fRec0[i-1] * fRec4[i-1])))));
				fRec2[i] = (fRec2[i-1] + ((float)input0[i] * ((float)input1[i] - ((5 * fRec2[i-1]) + (7 * (fRec4[i-1] * fRec3[i-1]))))));
				fRec3[i] = (fRec3[i-1] + ((float)input0[i] * ((3 * (fRec4[i-1] * fRec2[i-1])) - (9 * fRec3[i-1]))));
				fRec4[i] = (fRec4[i-1] + ((float)input0[i] * ((4 * ((fRec2[i-1] * fRec3[i-1]) + (fRec0[i-1] * fRec1[i-1]))) - (2 * fRec4[i-1]))));
			}
			// post processing
			...
			// exec code
			for (int i=0; i<count; i++) {
				output0[i] = (FAUSTFLOAT)fRec4[i];
			}
			
			// LOOP 0x7fed40d38040
			// exec code
			for (int i=0; i<count; i++) {
				output1[i] = (FAUSTFLOAT)fRec3[i];
			}
			
			// LOOP 0x7fed40d382d0
			// exec code
			for (int i=0; i<count; i++) {
				output2[i] = (FAUSTFLOAT)fRec2[i];
			}
			
			// LOOP 0x7fed40d385e0
			// exec code
			for (int i=0; i<count; i++) {
				output3[i] = (FAUSTFLOAT)fRec1[i];
			}
			
			// LOOP 0x7fed40d38940
			// exec code
			for (int i=0; i<count; i++) {
				output4[i] = (FAUSTFLOAT)fRec0[i];
			}
...
static bool faust_init_inputs(t_faust* x)
{
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(navierstokes));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust* x, bool info_outlet)
{
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(navierstokes));
            return false;
        }
...
static bool faust_new_internal(t_faust* x, const std::string& objId = "", bool info_outlet = true)
{
    ...
    x->dsp = new navierstokes();
    x->ui = new PdUI<UI>(sym(navierstokes), objId);
