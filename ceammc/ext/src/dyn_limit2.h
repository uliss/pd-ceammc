...
class limit2 : public dsp {
  ...
  public:
	...
	virtual void instanceConstants(int samplingFreq) {
		...
		fConst1 = expf((0 - (2.0f / fConst0)));
		fConst2 = expf((0 - (1.25e+03f / fConst0)));
		fConst3 = expf((0 - (2.5e+03f / fConst0)));
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (index = 0; index <= fullcount - 64; index += 64) {
			...
			// exec code
			for (int i=0; i<count; i++) {
				fZec1[i] = ((int((fRec1[i-1] > fZec0[i])))?fConst1:fConst2);
			...
			// exec code
			for (int i=0; i<count; i++) {
				fRec0[i] = ((fConst3 * fRec0[i-1]) + (fConst4 * (0 - (0.75f * max(((20 * log10f(fRec1[i])) + 6), 0.0f)))));
			}
			// post processing
...
static void faust_init_label(t_faust* x, const char* obj_id) {
    x->label = new std::string(sym(limit2) "~");
...
static bool faust_new_internal(t_faust* x, const char* obj_id = NULL, bool info_outlet = true) {
    ...
    x->dsp = new limit2();
    x->ui = new PdUI(sym(limit2), obj_id);
