...
class limit : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("dyn_limit");
...
static t_class* limit_faust_class;
#define FAUST_EXT t_faust_limit
...
{
    t_faust_limit* x = reinterpret_cast<t_faust_limit*>(w[1]);
...
static void limit_faust_dsp(t_faust_limit* x, t_signal** sp)
{
...
static void limit_dump_to_console(t_faust_limit* x)
{
...
static void limit_faust_any(t_faust_limit* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_limit* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_limit* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_limit* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_limit* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_limit* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void limit_faust_free(t_faust_limit* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_limit* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(limit));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_limit* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(limit));
            return false;
        }
...
static bool faust_new_internal(t_faust_limit* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_limit* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* limit_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_limit* x = reinterpret_cast<t_faust_limit*>(pd_new(limit_faust_class)); \
