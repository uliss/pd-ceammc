...
class comp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("attack", &fslider0, 1e+01f, 1.0f, 1e+02f, 0.1f);
		ui_interface->addVerticalSlider("ratio", &fslider1, 1.0f, 1.0f, 1e+01f, 0.001f);
		ui_interface->addVerticalSlider("release", &fslider2, 5e+01f, 1.0f, 5e+02f, 0.1f);
		ui_interface->addVerticalSlider("threshold", &fslider3, 1e+02f, 0.0f, 1e+02f, 0.1f);
		ui_interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		float 	fSlow0 = float(fslider0);
		float 	fSlow1 = expf((0 - (fConst1 / fSlow0)));
		float 	fSlow2 = (((1.0f / float(float(fslider1))) + -1.0f) * (1.0f - fSlow1));
		float 	fSlow3 = expf((0 - (fConst2 / float(fslider2))));
		float 	fSlow4 = expf((0 - (fConst2 / fSlow0)));
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp2 = ((int((fRec1[1] > fTemp1)))?fSlow3:fSlow4);
			fRec2[0] = (((1.0f - fTemp2) * fTemp1) + (fTemp2 * fRec2[1]));
			fRec1[0] = fRec2[0];
			fRec0[0] = ((fSlow1 * fRec0[1]) + (fSlow2 * max((((20 * log10f(fRec1[0])) + 100) - fSlow5), 0.0f)));
...
static t_class* comp_faust_class;
#define FAUST_EXT t_faust_comp
#define FAUST_EXT_CLASS comp_faust_class
// clang-format on
...
{
    t_faust_comp* x = reinterpret_cast<t_faust_comp*>(w[1]);
...
static void comp_faust_dsp(t_faust_comp* x, t_signal** sp)
{
...
static void comp_dump_to_console(t_faust_comp* x)
{
...
static void comp_faust_any(t_faust_comp* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_comp* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_comp* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_comp* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_comp* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_comp* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void comp_faust_free(t_faust_comp* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_comp* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(comp));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_comp* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(comp));
            return false;
        }
...
static bool faust_new_internal(t_faust_comp* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new comp();
    x->ui = new PdUI<UI>(sym(comp), objId);
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_comp* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
...
static void* comp_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s)
{
    comp_faust_class = class_new(s, reinterpret_cast<t_newmethod>(comp_faust_new),
        reinterpret_cast<t_method>(comp_faust_free),
        sizeof(t_faust_comp),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);
    class_addmethod(comp_faust_class, nullfn, &s_signal, A_NULL);
    class_addmethod(comp_faust_class, reinterpret_cast<t_method>(comp_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(comp_faust_class, reinterpret_cast<t_method>(comp_dump_to_console), gensym("dump"), A_NULL);
    CLASS_MAINSIGNALIN(comp_faust_class, t_faust_comp, f);
    class_addanything(comp_faust_class, comp_faust_any);
}

#define EXTERNAL_NEW void* comp_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* comp_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_comp* x = reinterpret_cast<t_faust_comp*>(pd_new(comp_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ecomp_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".comp~"));    \
    }
