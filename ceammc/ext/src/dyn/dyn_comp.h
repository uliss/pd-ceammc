...
class comp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "dyn_comp");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.1");
		m->declare("name", "dyn_comp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("dyn_comp");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			fRec1[0] = fRec2[0];
			fRec0[0] = ((fSlow1 * fRec0[1]) + (fSlow2 * std::max((((20.0f * std::log10(fRec1[0])) + 100.0f) - fSlow5), 0.0f)));
...
template <class T>
class _comp_UI : public UI {
};
typedef _comp_UI<comp> comp_UI;
...
static bool faust_new_internal(t_faust_comp* x, const std::string& objId = "", bool info_outlet = true)
{
    ...
    x->dsp = new comp();
    x->ui = new PdUI<comp_UI>(sym(comp), objId);
...
static void internal_setup(t_symbol* s, bool soundIn = true)
{
    ...
    ceammc::register_faust_external(comp_faust_class);
}
...
#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ecomp_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".comp~"), false); \
    }
