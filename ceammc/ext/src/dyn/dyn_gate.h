  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			fRec3[0] = ((fSlow3 * fabsf(fTemp0)) + (fSlow2 * fRec3[1]));
			float 	fRec2 = fRec3[0];
			fRec4[0] = (fSlow4 + (0.999f * fRec4[1]));
			int iTemp1 = (fRec2 > powf(10,(0.05f * fRec4[0])));
			iVec0[0] = iTemp1;
			iRec5[0] = max((int)(iSlow5 * (iVec0[0] < iVec0[1])), (int)(iRec5[1] + -1));
			float fTemp2 = fabsf(max(float(iVec0[0]), (float)(iRec5[0] > 0)));
			float fTemp3 = ((int((fRec0[1] > fTemp2)))?fSlow6:fSlow7);
...
static t_class* gate_faust_class;
#define FAUST_EXT t_faust_gate
...
{
    t_faust_gate* x = reinterpret_cast<t_faust_gate*>(w[1]);
...
static void gate_faust_dsp(t_faust_gate* x, t_signal** sp)
{
...
static void gate_dump_to_console(t_faust_gate* x)
{
...
static void gate_faust_any(t_faust_gate* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_gate* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_gate* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_gate* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_gate* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_gate* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void gate_faust_free(t_faust_gate* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_gate* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(gate));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_gate* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(gate));
            return false;
        }
...
static bool faust_new_internal(t_faust_gate* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* gate_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_gate* x = reinterpret_cast<t_faust_gate*>(pd_new(gate_faust_class)); \
