...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn dyn_gate -scn dyn_gate_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
class FAUST_API dyn_gate_dsp {
    ...
    public:
        ...
        dyn_gate_dsp() = default;
        virtual ~dyn_gate_dsp() = default;
...
class dyn_gate : public dyn_gate_dsp {
 ...
 public:
	...
	dyn_gate(const dyn_gate&) = default;
	
	virtual ~dyn_gate() = default;
	
	dyn_gate& operator=(const dyn_gate&) = default;
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn dyn_gate -scn dyn_gate_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = 0.001f * static_cast<float>(fVslider0);
		float fSlow2 = 0.001f * static_cast<float>(fVslider1);
		...
		float fSlow7 = fConst2 * static_cast<float>(fVslider2);
		int iSlow8 = static_cast<int>(fConst4 * static_cast<float>(fVslider3));
		int iSlow9 = std::fabs(fSlow2) < 1.1920929e-07f;
		float fSlow10 = ((iSlow9) ? 0.0f : std::exp(-(fConst1 / ((iSlow9) ? 1.0f : fSlow2))));
		int iSlow11 = std::fabs(fSlow1) < 1.1920929e-07f;
		float fSlow12 = ((iSlow11) ? 0.0f : std::exp(-(fConst1 / ((iSlow11) ? 1.0f : fSlow1))));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec2[0] = std::fabs(fTemp0) * fSlow6 + fRec2[1] * fSlow5;
			fRec3[0] = fSlow7 + fConst3 * fRec3[1];
			int iTemp3 = fRec2[0] > std::pow(1e+01f, 0.05f * fRec3[0]);
			iVec0[0] = iTemp3;
			iRec4[0] = std::max<int>(iSlow8 * (iTemp3 < iVec0[1]), iRec4[1] + -1);
			float fTemp4 = std::fabs(std::max<float>(static_cast<float>(iTemp3), static_cast<float>(iRec4[0] > 0)));
			float fTemp5 = ((fTemp4 > fRec1[1]) ? fSlow12 : fSlow10);
			fRec1[0] = fTemp4 * (1.0f - fTemp5) + fRec1[1] * fTemp5;
			output0[i0] = static_cast<FAUSTFLOAT>(fTemp0 * (fRec0[0] + fRec1[0] * (1.0f - fRec0[0])));
			fRec0[1] = fRec0[0];
			fRec2[1] = fRec2[0];
			fRec3[1] = fRec3[0];
			iVec0[1] = iVec0[0];
...
class faust_dyn_gate_tilde : public FaustExternal<dyn_gate> {
public:
    faust_dyn_gate_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(dyn_gate))
    {
    }
};
