...
#ifndef  __softclip_H__
#define  __softclip_H__
...
static float softclip_faustpower2_f(float value) {
	return (value * value);
}

#ifndef FAUSTCLASS 
#define FAUSTCLASS softclip
#endif
...
class softclip : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("filename", "dyn_softclip.dsp");
		m->declare("name", "dyn_softclip");
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("dyn_softclip");
...
static t_class* softclip_faust_class;
#define FAUST_EXT t_faust_softclip
#define FAUST_EXT_CLASS softclip_faust_class
// clang-format on

template <class T>
class _softclip_UI : public UI {
};
typedef _softclip_UI<softclip> softclip_UI;

struct t_faust_softclip {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_softclip* x = reinterpret_cast<t_faust_softclip*>(w[1]);
...
static void softclip_faust_dsp(t_faust_softclip* x, t_signal** sp)
{
...
static void softclip_dump_to_console(t_faust_softclip* x)
{
...
static void softclip_faust_any(t_faust_softclip* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_softclip* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_softclip* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_softclip* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_softclip* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_softclip* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void softclip_faust_free(t_faust_softclip* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_softclip* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(softclip));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_softclip* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(softclip));
            return false;
        }
...
static bool faust_new_internal(t_faust_softclip* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new softclip();
    x->ui = new PdUI<softclip_UI>(sym(softclip), objId);

    if (!faust_init_inputs(x)) {
        softclip_faust_free(x);
        return false;
    }
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_softclip* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_softclip* pd_obj()
    {
        return this->x_;
    }
};

static void* softclip_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    softclip_faust_class = class_new(s, reinterpret_cast<t_newmethod>(softclip_faust_new),
        reinterpret_cast<t_method>(softclip_faust_free),
        sizeof(t_faust_softclip),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(softclip_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(softclip_faust_class, t_faust_softclip, f);
    }

    class_addmethod(softclip_faust_class, reinterpret_cast<t_method>(softclip_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(softclip_faust_class, reinterpret_cast<t_method>(softclip_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(softclip_faust_class, softclip_faust_any);
    ceammc::register_faust_external(softclip_faust_class);
}

#define EXTERNAL_NEW void* softclip_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* softclip_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_softclip* x = reinterpret_cast<t_faust_softclip*>(pd_new(softclip_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2esoftclip_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".softclip~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2esoftclip_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".softclip~"), false); \
    }
