...
class dyn_comp_up2 : public dyn_comp_up2_dsp {
 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		int iSlow8 = int(std::floor(fConst6 * fSlow3)) % 2;
		int iSlow9 = iSlow7 + 4 * iSlow6;
		int iSlow10 = int(std::floor(fConst7 * fSlow3)) % 2;
		int iSlow11 = iSlow9 + 8 * iSlow8;
		int iSlow12 = int(std::floor(fConst8 * fSlow3)) % 2;
		int iSlow13 = iSlow11 + 16 * iSlow10;
		int iSlow14 = int(std::floor(fConst9 * fSlow3)) % 2;
		int iSlow15 = iSlow13 + 32 * iSlow12;
		int iSlow16 = int(std::floor(fConst10 * fSlow3)) % 2;
		int iSlow17 = iSlow15 + 64 * iSlow14;
		int iSlow18 = int(std::floor(fConst11 * fSlow3)) % 2;
		int iSlow19 = iSlow17 + 128 * iSlow16;
		int iSlow20 = int(std::floor(fConst12 * fSlow3)) % 2;
		int iSlow21 = iSlow19 + 256 * iSlow18;
		int iSlow22 = int(std::floor(fConst13 * fSlow3)) % 2;
		int iSlow23 = iSlow21 + 512 * iSlow20;
		int iSlow24 = int(std::floor(fConst14 * fSlow3)) % 2;
		int iSlow25 = iSlow23 + 1024 * iSlow22;
		int iSlow26 = int(std::floor(fConst15 * fSlow3)) % 2;
		int iSlow27 = iSlow25 + 2048 * iSlow24;
		int iSlow28 = int(std::floor(fConst16 * fSlow3)) % 2;
		int iSlow29 = iSlow27 + 4096 * iSlow26;
		int iSlow30 = int(std::floor(fConst17 * fSlow3)) % 2;
		int iSlow31 = iSlow29 + 8192 * iSlow28;
		int iSlow32 = int(std::floor(fConst18 * fSlow3)) % 2;
		int iSlow33 = iSlow31 + 16384 * iSlow30;
		int iSlow34 = int(std::floor(fConst19 * fSlow3)) % 2;
		int iSlow35 = iSlow33 + 32768 * iSlow32;
		float fSlow36 = float(fHslider3);
		float fSlow37 = float(fHslider4);
		float fSlow38 = 0.5f * fSlow37;
		float fSlow39 = fSlow36 - fSlow38;
		float fSlow40 = fSlow36 + fSlow38;
		float fSlow41 = 1.0f / std::min<float>(1.1920929e-07f, 0.0f - 2.0f * fSlow37);
		float fSlow42 = 0.001f * float(fHslider5);
		float fSlow43 = 0.001f * float(fHslider6);
		float fSlow44 = float(fHslider7);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp50 = 2e+01f * std::log10(std::max<float>(1.1754944e-38f, std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(std::max<float>(((iSlow4) ? fTemp34 : -3.4028235e+38f), ((iSlow5) ? fVec18[iSlow4] : -3.4028235e+38f)), ((iSlow6) ? fVec19[iSlow7] : -3.4028235e+38f)), ((iSlow8) ? fVec20[iSlow9] : -3.4028235e+38f)), ((iSlow10) ? fVec21[(IOTA0 - iSlow11) & 31] : -3.4028235e+38f)), ((iSlow12) ? fVec22[(IOTA0 - iSlow13) & 63] : -3.4028235e+38f)), ((iSlow14) ? fVec23[(IOTA0 - iSlow15) & 127] : -3.4028235e+38f)), ((iSlow16) ? fVec24[(IOTA0 - iSlow17) & 255] : -3.4028235e+38f)), ((iSlow18) ? fVec25[(IOTA0 - iSlow19) & 511] : -3.4028235e+38f)), ((iSlow20) ? fVec26[(IOTA0 - iSlow21) & 1023] : -3.4028235e+38f)), ((iSlow22) ? fVec27[(IOTA0 - iSlow23) & 2047] : -3.4028235e+38f)), ((iSlow24) ? fVec28[(IOTA0 - iSlow25) & 4095] : -3.4028235e+38f)), ((iSlow26) ? fVec29[(IOTA0 - iSlow27) & 8191] : -3.4028235e+38f)), ((iSlow28) ? fVec30[(IOTA0 - iSlow29) & 16383] : -3.4028235e+38f)), ((iSlow30) ? fVec31[(IOTA0 - iSlow31) & 32767] : -3.4028235e+38f)), ((iSlow32) ? fVec32[(IOTA0 - iSlow33) & 65535] : -3.4028235e+38f)), ((iSlow34) ? fVec33[(IOTA0 - iSlow35) & 131071] : -3.4028235e+38f))));
			int iTemp51 = (fTemp50 > fSlow39) + (fTemp50 > fSlow40);
			float fTemp52 = fTemp24 * std::max<float>(fRec2[0], fTemp4 * ((iTemp51 == 0) ? fTemp50 - fSlow36 : ((iTemp51 == 1) ? fSlow41 * dyn_comp_up2_faustpower2_f(fTemp50 - fSlow40) : 0.0f)));
