...
class gate2 : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("dyn_gate2");
...
static bool faust_init_inputs(t_faust_gate2* x)
{
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(gate2));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_gate2* x, bool info_outlet)
{
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(gate2));
            return false;
        }
...
#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2egate2_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".gate2~"), false); \
    }
