  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp3 = ((int((fRec1[1] > fTemp2)))?fSlow3:fSlow4);
...
static t_class* comp2_faust_class;
#define FAUST_EXT t_faust_comp2
...
{
    t_faust_comp2* x = reinterpret_cast<t_faust_comp2*>(w[1]);
...
static void comp2_faust_dsp(t_faust_comp2* x, t_signal** sp)
{
...
static void comp2_dump_to_console(t_faust_comp2* x)
{
...
static void comp2_faust_any(t_faust_comp2* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_comp2* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_comp2* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_comp2* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_comp2* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_comp2* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void comp2_faust_free(t_faust_comp2* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_comp2* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(comp2));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_comp2* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(comp2));
            return false;
        }
...
static bool faust_new_internal(t_faust_comp2* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_comp2* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* comp2_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_comp2* x = reinterpret_cast<t_faust_comp2*>(pd_new(comp2_faust_class)); \
