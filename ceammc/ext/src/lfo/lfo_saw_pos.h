...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn lfo_saw_pos -scn lfo_saw_pos_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __lfo_saw_pos_H__
#define  __lfo_saw_pos_H__
...
class FAUST_API lfo_saw_pos_dsp {

    public:

        lfo_saw_pos_dsp() = default;
        virtual ~lfo_saw_pos_dsp() = default;
         ...
         */
        virtual ::lfo_saw_pos_dsp* clone() = 0;
...
class FAUST_API decorator_dsp : public ::lfo_saw_pos_dsp {
    ...
    protected:
        ...
        ::lfo_saw_pos_dsp* fDSP;

    public:

        decorator_dsp(::lfo_saw_pos_dsp* lfo_saw_pos_dsp = nullptr):fDSP(lfo_saw_pos_dsp) {}
...
class FAUST_API dsp_factory {
    ...
    public:
        ...
        virtual ::lfo_saw_pos_dsp* createDSPInstance() = 0;
...
/**
 The base class of Meta handler to be used in lfo_saw_pos_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
class lfo_saw_pos : public lfo_saw_pos_dsp {
 ...
 public:
	...
	lfo_saw_pos(const lfo_saw_pos&) = default;
	
	virtual ~lfo_saw_pos() = default;
	
	lfo_saw_pos& operator=(const lfo_saw_pos&) = default;
	
	void metadata(Meta* m) { 
		m->declare("ceammc_osc.lib/name", "CEAMMC faust oscillators");
		m->declare("ceammc_osc.lib/version", "0.1");
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn lfo_saw_pos -scn lfo_saw_pos_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual lfo_saw_pos* clone() {
		return new lfo_saw_pos(*this);
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		int iSlow0 = static_cast<float>(fCheckbox0) > 0.5f;
		float fSlow1 = static_cast<float>(iSlow0 == 1);
		float fSlow2 = static_cast<float>(2 * (iSlow0 == 0) + -1);
		float fSlow3 = static_cast<float>(fHslider0);
		float fSlow4 = fConst0 * static_cast<float>((static_cast<float>(fCheckbox1) > 0.5f) == 0);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			fVec0[0] = fSlow3;
			float fTemp0 = fRec0[1] + fSlow4 * static_cast<float>(input0[i0]);
			fRec0[0] = fSlow3 + (fTemp0 - (fVec0[1] + std::floor(fSlow3 + (fTemp0 - fVec0[1]))));
			output0[i0] = static_cast<FAUSTFLOAT>(fSlow1 + fSlow2 * fRec0[0]);
			fVec0[1] = fVec0[0];
			fRec0[1] = fRec0[0];
		}
	}
...
class faust_lfo_saw_pos_tilde : public FaustExternal<lfo_saw_pos> {
public:
    faust_lfo_saw_pos_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(lfo_saw_pos))
    {
    }
};
