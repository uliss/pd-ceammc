...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn lfo_impulse -scn lfo_impulse_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __lfo_impulse_H__
#define  __lfo_impulse_H__
...
class FAUST_API lfo_impulse_dsp {

    public:

        lfo_impulse_dsp() = default;
        virtual ~lfo_impulse_dsp() = default;
         ...
         */
        virtual ::lfo_impulse_dsp* clone() = 0;
...
class FAUST_API decorator_dsp : public ::lfo_impulse_dsp {

    protected:

        ::lfo_impulse_dsp* fDSP;

    public:

        decorator_dsp(::lfo_impulse_dsp* lfo_impulse_dsp = nullptr):fDSP(lfo_impulse_dsp) {}
...
class FAUST_API dsp_factory {
    ...
    public:
        ...
        virtual ::lfo_impulse_dsp* createDSPInstance() = 0;
...
/**
 The base class of Meta handler to be used in lfo_impulse_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
// clang-format off
#ifndef FAUST_MACRO
struct lfo_impulse : public lfo_impulse_dsp {
};
#endif
// clang-format on
...
class lfo_impulse : public lfo_impulse_dsp {
 ...
 public:
	...
	lfo_impulse(const lfo_impulse&) = default;
	
	virtual ~lfo_impulse() = default;
	
	lfo_impulse& operator=(const lfo_impulse&) = default;
	
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn lfo_impulse -scn lfo_impulse_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual lfo_impulse* clone() {
		return new lfo_impulse(*this);
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			iVec0[0] = 1;
			float fTemp0 = ((1 - iVec0[1]) ? 0.0f : fRec0[1] + fConst0 * std::fabs(static_cast<float>(input0[i0])));
			fRec0[0] = fTemp0 - std::floor(fTemp0);
			output0[i0] = static_cast<FAUSTFLOAT>((fRec0[0] - fRec0[1]) < 0.0f);
			iVec0[1] = iVec0[0];
			fRec0[1] = fRec0[0];
		}
	}
