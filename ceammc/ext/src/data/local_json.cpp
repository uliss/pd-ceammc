...
static t_class* json_local_object_class;
...
{
    t_json_object* x = reinterpret_cast<t_json_object*>(pd_new(json_local_object_class));
...
{
    json_local_object_class = class_new(gensym(OBJ_NAME),
    ...
    class_addanything(json_local_object_class, json_object_anything);

    class_addmethod(json_local_object_class, reinterpret_cast<t_method>(json_object_clear), gensym("clear"), A_NULL);
