...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn osc_sin -scn osc_sin_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __osc_sin_H__
#define  __osc_sin_H__
...
class FAUST_API osc_sin_dsp {

    public:

        osc_sin_dsp() = default;
        virtual ~osc_sin_dsp() = default;
...
class FAUST_API decorator_dsp : public ::osc_sin_dsp {
    ...
    public:
        ...
        decorator_dsp(::osc_sin_dsp* osc_sin_dsp = nullptr):fDSP(osc_sin_dsp) {}
...
/**
 The base class of Meta handler to be used in osc_sin_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
// clang-format off
#ifndef FAUST_MACRO
struct osc_sin : public osc_sin_dsp {
};
#endif
// clang-format on
...
class osc_sinSIG0 {
  ...
  public:
	...
	int getNumInputsosc_sinSIG0() {
		return 0;
	}
	int getNumOutputsosc_sinSIG0() {
		return 1;
	}
...
static osc_sinSIG0* newosc_sinSIG0() { return (osc_sinSIG0*)new osc_sinSIG0(); }
static void deleteosc_sinSIG0(osc_sinSIG0* dsp) { delete dsp; }

static float ftbl0osc_sinSIG0[65536];

class osc_sin : public osc_sin_dsp {
 ...
 public:
	...
	osc_sin(const osc_sin&) = default;
	
	virtual ~osc_sin() = default;
	
	osc_sin& operator=(const osc_sin&) = default;
	
	void metadata(Meta* m) { 
		m->declare("basics.lib/name", "Faust Basic Element Library");
		m->declare("basics.lib/version", "1.22.0");
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn osc_sin -scn osc_sin_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	static void classInit(int sample_rate) {
		osc_sinSIG0* sig0 = newosc_sinSIG0();
		sig0->instanceInitosc_sinSIG0(sample_rate);
		sig0->fillosc_sinSIG0(65536, ftbl0osc_sinSIG0);
		deleteosc_sinSIG0(sig0);
	}
	...
	virtual osc_sin* clone() {
		return new osc_sin(*this);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("osc_sin");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			iVec1[0] = 1;
			float fTemp0 = ((1 - iVec1[1]) ? 0.0f : fRec1[1] + fConst0 * static_cast<float>(input0[i0]));
			fRec1[0] = fTemp0 - std::floor(fTemp0);
			output0[i0] = static_cast<FAUSTFLOAT>(ftbl0osc_sinSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec1[0]), 65535))]);
			iVec1[1] = iVec1[0];
			fRec1[1] = fRec1[0];
		}
	}
...
class faust_osc_sin_tilde : public FaustExternal<osc_sin> {
public:
    faust_osc_sin_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(osc_sin))
    {
    }
};
