...
#ifndef FAUSTCLASS 
#define FAUSTCLASS pulse
#endif

class pulse : public dsp {
  private:
	float 	fConst0;
	float 	fConst1;
	float 	fConst2;
  ...
  public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("duty", &fslider0, 0.5f, 0.0f, 1.0f, 0.001f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp6 = max((float)0, min((float)2047, (fConst1 * (fRec1[0] / fTemp1))));
			int iTemp7 = int(fTemp6);
			float fTemp8 = floorf(fTemp6);
			output0[i] = (FAUSTFLOAT)((int((fTemp0 == 0)))?0:(fConst2 * (fVec2[IOTA&4095] - ((fVec2[(IOTA-iTemp7)&4095] * (fTemp8 + (1 - fTemp6))) + ((fTemp6 - fTemp8) * fVec2[(IOTA-int((iTemp7 + 1)))&4095])))));
			// post processing
			fRec1[1] = fRec1[0];
			IOTA = IOTA+1;
			fVec1[1] = fVec1[0];
...
static t_class* pulse_faust_class;
#define FAUST_EXT t_faust_pulse
#define FAUST_EXT_CLASS pulse_faust_class
// clang-format on

struct t_faust_pulse {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_pulse* x = reinterpret_cast<t_faust_pulse*>(w[1]);
...
static void pulse_faust_dsp(t_faust_pulse* x, t_signal** sp)
{
...
static void pulse_dump_to_console(t_faust_pulse* x)
{
...
static void pulse_faust_any(t_faust_pulse* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_pulse* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_pulse* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_pulse* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_pulse* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_pulse* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void pulse_faust_free(t_faust_pulse* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_pulse* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(pulse));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_pulse* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(pulse));
            return false;
        }
...
static bool faust_new_internal(t_faust_pulse* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new pulse();
    x->ui = new PdUI<UI>(sym(pulse), objId);
...
#define EXTERNAL_NEW void* pulse_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* pulse_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_pulse* x = reinterpret_cast<t_faust_pulse*>(pd_new(pulse_faust_class)); \
