...
class sinfbSIG0 {
  ...
  public:
	...
	int getNumInputssinfbSIG0() {
		return 0;
		
	}
	int getNumOutputssinfbSIG0() {
		return 1;
		
	}
	int getInputRatesinfbSIG0(int channel) {
		int rate;
		switch (channel) {
			default: {
				rate = -1;
	...
	int getOutputRatesinfbSIG0(int channel) {
		int rate;
		switch (channel) {
			case 0: {
				rate = 0;
	...
	void instanceInitsinfbSIG0(int samplingFreq) {
...
sinfbSIG0* newsinfbSIG0() { return (sinfbSIG0*)new sinfbSIG0(); }
void deletesinfbSIG0(sinfbSIG0* dsp) { delete dsp; }
...
class sinfbSIG1 {
  ...
  public:
	...
	int getNumInputssinfbSIG1() {
		return 0;
		
	}
	int getNumOutputssinfbSIG1() {
		return 1;
		
	}
	int getInputRatesinfbSIG1(int channel) {
		int rate;
		switch (channel) {
			default: {
				rate = -1;
	...
	int getOutputRatesinfbSIG1(int channel) {
		int rate;
		switch (channel) {
			case 0: {
				rate = 0;
	...
	void instanceInitsinfbSIG1(int samplingFreq) {
...
sinfbSIG1* newsinfbSIG1() { return (sinfbSIG1*)new sinfbSIG1(); }
void deletesinfbSIG1(sinfbSIG1* dsp) { delete dsp; }

static float ftbl0sinfbSIG0[65536];
static float ftbl1sinfbSIG1[65536];

#ifndef FAUSTCLASS 
#define FAUSTCLASS sinfb
#endif
#ifdef __APPLE__ 
...
class sinfb : public dsp {
 ...
 public:
	...
	static void classInit(int samplingFreq) {
		sinfbSIG0* sig0 = newsinfbSIG0();
		sig0->instanceInitsinfbSIG0(samplingFreq);
		sig0->fillsinfbSIG0(65536, ftbl0sinfbSIG0);
		sinfbSIG1* sig1 = newsinfbSIG1();
		sig1->instanceInitsinfbSIG1(samplingFreq);
		sig1->fillsinfbSIG1(65536, ftbl1sinfbSIG1);
		deletesinfbSIG0(sig0);
		deletesinfbSIG1(sig1);
...
static t_class* sinfb_faust_class;
#define FAUST_EXT t_faust_sinfb
#define FAUST_EXT_CLASS sinfb_faust_class
// clang-format on

template<class T>
class _DspUI : public UI {};
typedef _DspUI<sinfb> DspUI;

struct t_faust_sinfb {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_sinfb* x = reinterpret_cast<t_faust_sinfb*>(w[1]);
...
static void sinfb_faust_dsp(t_faust_sinfb* x, t_signal** sp)
{
...
static void sinfb_dump_to_console(t_faust_sinfb* x)
{
...
static void sinfb_faust_any(t_faust_sinfb* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_sinfb* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_sinfb* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_sinfb* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_sinfb* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_sinfb* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void sinfb_faust_free(t_faust_sinfb* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_sinfb* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(sinfb));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_sinfb* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(sinfb));
            return false;
        }
...
static bool faust_new_internal(t_faust_sinfb* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new sinfb();
    x->ui = new PdUI<DspUI>(sym(sinfb), objId);
...
#define EXTERNAL_NEW void* sinfb_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* sinfb_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_sinfb* x = reinterpret_cast<t_faust_sinfb*>(pd_new(sinfb_faust_class)); \
