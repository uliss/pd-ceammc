...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn osc_tri -scn osc_tri_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __osc_tri_H__
#define  __osc_tri_H__
...
class FAUST_API osc_tri_dsp {

    public:

        osc_tri_dsp() = default;
        virtual ~osc_tri_dsp() = default;
...
class FAUST_API decorator_dsp : public ::osc_tri_dsp {
    ...
    public:
        ...
        decorator_dsp(::osc_tri_dsp* osc_tri_dsp = nullptr):fDSP(osc_tri_dsp) {}
...
class FAUST_API dsp_factory {
    ...
    public:
        ...
        virtual ::osc_tri_dsp* createDSPInstance() = 0;
...
/**
 The base class of Meta handler to be used in osc_tri_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
// clang-format off
#ifndef FAUST_MACRO
struct osc_tri : public osc_tri_dsp {
};
#endif
// clang-format on
...
static float osc_tri_faustpower2_f(float value) {
	return value * value;
}

class osc_tri : public osc_tri_dsp {
 ...
 public:
	...
	osc_tri(const osc_tri&) = default;
	
	virtual ~osc_tri() = default;
	
	osc_tri& operator=(const osc_tri&) = default;
	
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn osc_tri -scn osc_tri_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
		m->declare("filename", "osc_tri.dsp");
	...
	virtual osc_tri* clone() {
		return new osc_tri(*this);
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp3 = ((1 - iVec0[1]) ? 0.0f : fRec1[1] + fConst3 * fTemp2);
			fRec1[0] = fTemp3 - std::floor(fTemp3);
			float fTemp4 = osc_tri_faustpower2_f(2.0f * fRec1[0] + -1.0f);
			fVec1[0] = fTemp4;
			float fTemp5 = static_cast<float>(iVec0[1]) * (fTemp4 - fVec1[1]) / fTemp2;
			fVec2[IOTA0 & 4095] = fTemp5;
			float fTemp6 = std::max<float>(0.0f, std::min<float>(2047.0f, fConst4 / fTemp1));
			int iTemp7 = static_cast<int>(fTemp6);
			float fTemp8 = std::floor(fTemp6);
			fRec0[0] = 0.999f * fRec0[1] + fConst2 * (fTemp5 - fVec2[(IOTA0 - iTemp7) & 4095] * (fTemp8 + (1.0f - fTemp6)) - (fTemp6 - fTemp8) * fVec2[(IOTA0 - (iTemp7 + 1)) & 4095]);
			output0[i0] = static_cast<FAUSTFLOAT>(fConst1 * fTemp0 * fRec0[0]);
			iVec0[1] = iVec0[0];
			fRec1[1] = fRec1[0];
			fVec1[1] = fVec1[0];
			IOTA0 = IOTA0 + 1;
...
class faust_osc_tri_tilde : public FaustExternal<osc_tri> {
public:
    faust_osc_tri_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(osc_tri))
    {
    }
};
