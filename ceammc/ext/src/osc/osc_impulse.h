...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn osc_impulse -scn osc_impulse_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __osc_impulse_H__
#define  __osc_impulse_H__
...
class FAUST_API osc_impulse_dsp {

    public:

        osc_impulse_dsp() = default;
        virtual ~osc_impulse_dsp() = default;
         ...
         */
        virtual ::osc_impulse_dsp* clone() = 0;
...
class FAUST_API decorator_dsp : public ::osc_impulse_dsp {

    protected:

        ::osc_impulse_dsp* fDSP;

    public:

        decorator_dsp(::osc_impulse_dsp* osc_impulse_dsp = nullptr):fDSP(osc_impulse_dsp) {}
...
class FAUST_API dsp_factory {
    ...
    public:
        ...
        virtual ::osc_impulse_dsp* createDSPInstance() = 0;
...
/**
 The base class of Meta handler to be used in osc_impulse_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
// clang-format off
#ifndef FAUST_MACRO
struct osc_impulse : public osc_impulse_dsp {
};
#endif
// clang-format on
...
static float osc_impulse_faustpower2_f(float value) {
	return value * value;
}

class osc_impulse : public osc_impulse_dsp {
 ...
 public:
	...
	osc_impulse(const osc_impulse&) = default;
	
	virtual ~osc_impulse() = default;
	
	osc_impulse& operator=(const osc_impulse&) = default;
	
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn osc_impulse -scn osc_impulse_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
		m->declare("filename", "osc_impulse.dsp");
	...
	virtual osc_impulse* clone() {
		return new osc_impulse(*this);
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp1 = std::max<float>(2e+01f, std::fabs(static_cast<float>(input0[i0])));
			float fTemp2 = ((iTemp0) ? 0.0f : fRec0[1] + fConst2 * fTemp1);
			fRec0[0] = fTemp2 - std::floor(fTemp2);
			float fTemp3 = osc_impulse_faustpower2_f(2.0f * fRec0[0] + -1.0f);
			fVec1[0] = fTemp3;
			float fTemp4 = static_cast<float>(iVec0[1]) * (fTemp3 - fVec1[1]) / fTemp1;
			fVec2[0] = fTemp4;
			output0[i0] = static_cast<FAUSTFLOAT>(static_cast<float>(iTemp0) - fConst1 * (fTemp4 - fVec2[1]));
			iVec0[1] = iVec0[0];
			fRec0[1] = fRec0[0];
			fVec1[1] = fVec1[0];
			fVec2[1] = fVec2[0];
...
class faust_osc_impulse_tilde : public FaustExternal<osc_impulse> {
public:
    faust_osc_impulse_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(osc_impulse))
    {
    }
};
