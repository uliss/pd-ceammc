...
static const char* SYM_SINGLE = "single";
static const char* SYM_CLIP = "clip";
static const char* SYM_WRAP = "wrap";
static const char* SYM_FOLD = "fold";
    ...
    , single_done_(false)
{
    ...
    walk_mode_ = new SymbolEnumProperty("@mode", gensym(SYM_SINGLE));
    walk_mode_->appendEnum(gensym(SYM_WRAP));
    walk_mode_->appendEnum(gensym(SYM_CLIP));
    walk_mode_->appendEnum(gensym(SYM_FOLD));
    createProperty(walk_mode_);

    // aliases
    createProperty(new SymbolEnumAlias("@single", walk_mode_, gensym(SYM_SINGLE)));
    createProperty(new SymbolEnumAlias("@loop", walk_mode_, gensym(SYM_WRAP)));
    createProperty(new SymbolEnumAlias("@wrap", walk_mode_, gensym(SYM_WRAP)));
    createProperty(new SymbolEnumAlias("@clip", walk_mode_, gensym(SYM_CLIP)));
    createProperty(new SymbolEnumAlias("@fold", walk_mode_, gensym(SYM_FOLD)));
...
{
    if (walk_mode_->value() != gensym(SYM_FOLD))
...
void ListWalk::toPosition(int pos)
{
    ...
    if (walk_mode_->value() == gensym(SYM_SINGLE)) {
        if (pos < 0) {
            current_pos_ = 0;
            single_done_ = true;
    ...
    } else if (walk_mode_->value() == gensym(SYM_WRAP)) {
        if (calcWrapIndex(pos, lst_.size(), &idx))
            current_pos_ = idx;
    } else if (walk_mode_->value() == gensym(SYM_CLIP)) {
        if (calcClipIndex(pos, lst_.size(), &idx))
            current_pos_ = idx;
    } else if (walk_mode_->value() == gensym(SYM_FOLD)) {
        current_pos_ = pos;
    } else {
