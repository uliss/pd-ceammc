...
class wahwahSIG0 {
  ...
  public:
	...
	int getNumInputswahwahSIG0() {
		return 0;
		
	}
	int getNumOutputswahwahSIG0() {
		return 1;
		
	}
	int getInputRatewahwahSIG0(int channel) {
		int rate;
		switch (channel) {
			default: {
				rate = -1;
	...
	int getOutputRatewahwahSIG0(int channel) {
		int rate;
		switch (channel) {
			case 0: {
				rate = 0;
	...
	void instanceInitwahwahSIG0(int samplingFreq) {
		
	}
	
	void fillwahwahSIG0(int count, float* output) {
...
static t_class* wahwah_faust_class;
#define FAUST_EXT t_faust_wahwah
...
{
    t_faust_wahwah* x = reinterpret_cast<t_faust_wahwah*>(w[1]);
...
static void wahwah_faust_dsp(t_faust_wahwah* x, t_signal** sp)
{
...
static void wahwah_dump_to_console(t_faust_wahwah* x)
{
...
static void wahwah_faust_any(t_faust_wahwah* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_wahwah* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_wahwah* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_wahwah* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_wahwah* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_wahwah* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void wahwah_faust_free(t_faust_wahwah* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_wahwah* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(wahwah));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_wahwah* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(wahwah));
            return false;
        }
...
static bool faust_new_internal(t_faust_wahwah* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
    ...
    t_faust_wahwah* pd_obj()
    {
        return this->x_;
    }
};

static void* wahwah_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    wahwah_faust_class = class_new(s, reinterpret_cast<t_newmethod>(wahwah_faust_new),
        reinterpret_cast<t_method>(wahwah_faust_free),
        sizeof(t_faust_wahwah),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(wahwah_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(wahwah_faust_class, t_faust_wahwah, f);
    }
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* wahwah_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_wahwah* x = reinterpret_cast<t_faust_wahwah*>(pd_new(wahwah_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2ewahwah_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".wahwah~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2ewahwah_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".wahwah~"), false); \
    }
