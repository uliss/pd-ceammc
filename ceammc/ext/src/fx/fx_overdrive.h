...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn fx_overdrive -scn fx_overdrive_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __fx_overdrive_H__
#define  __fx_overdrive_H__
...
class FAUST_API fx_overdrive_dsp {

    public:

        fx_overdrive_dsp() = default;
        virtual ~fx_overdrive_dsp() = default;
         ...
         */
        virtual ::fx_overdrive_dsp* clone() = 0;
...
class FAUST_API decorator_dsp : public ::fx_overdrive_dsp {

    protected:

        ::fx_overdrive_dsp* fDSP;

    public:

        decorator_dsp(::fx_overdrive_dsp* fx_overdrive_dsp = nullptr):fDSP(fx_overdrive_dsp) {}
...
class FAUST_API dsp_factory {
    ...
    public:
        ...
        virtual ::fx_overdrive_dsp* createDSPInstance() = 0;
...
/**
 The base class of Meta handler to be used in fx_overdrive_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
// clang-format off
#ifndef FAUST_MACRO
struct fx_overdrive : public fx_overdrive_dsp {
};
#endif
// clang-format on
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS fx_overdrive
#endif
...
static float fx_overdrive_faustpower2_f(float value) {
	return value * value;
}

class fx_overdrive : public fx_overdrive_dsp {
 ...
 public:
	...
	fx_overdrive(const fx_overdrive&) = default;
	
	virtual ~fx_overdrive() = default;
	
	fx_overdrive& operator=(const fx_overdrive&) = default;
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn fx_overdrive -scn fx_overdrive_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
		m->declare("filename", "fx_overdrive.dsp");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.9.0");
		m->declare("name", "fx.overdrive");
	...
	virtual fx_overdrive* clone() {
		return new fx_overdrive(*this);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("gain", &fVslider0, FAUSTFLOAT(1.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(5.0f), FAUSTFLOAT(0.001f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow2 = fConst0 * static_cast<float>(fVslider0);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			output0[i0] = static_cast<FAUSTFLOAT>(((iSlow0) ? fTemp0 : fTemp1 * (1.0f - fRec0[0]) + fRec0[0] * static_cast<float>((fTemp1 > 0.0f) - (fTemp1 < 0.0f)) * ((fTemp2 < 0.33333334f) ? 2.0f * fTemp2 : ((fTemp2 < 0.6666667f) ? 0.33333334f * (3.0f - fx_overdrive_faustpower2_f(2.0f - 3.0f * fTemp2)) : 1.0f))));
			fRec0[1] = fRec0[0];
			fRec1[1] = fRec1[0];
		}
	}
...
class faust_fx_overdrive_tilde : public FaustExternal<fx_overdrive> {
public:
    faust_fx_overdrive_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(fx_overdrive))
    {
    }
};
