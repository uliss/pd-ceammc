...
class flanger : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->declare(&fslider2, "unit", "ms");
		ui_interface->addHorizontalSlider("delay", &fslider2, 1e+01f, 0.0f, 2e+01f, 0.001f);
		ui_interface->declare(&fslider4, "style", "knob");
		ui_interface->addHorizontalSlider("drywet", &fslider4, 1.0f, 0.0f, 1.0f, 0.001f);
		ui_interface->declare(&fslider0, "style", "knob");
		ui_interface->addHorizontalSlider("feedback", &fslider0, 0.0f, -0.999f, 0.999f, 0.001f);
		ui_interface->addCheckButton("invert", &fcheckbox1);
		ui_interface->declare(&fslider1, "style", "knob");
		ui_interface->declare(&fslider1, "unit", "ms");
		ui_interface->addHorizontalSlider("offset", &fslider1, 1.0f, 0.0f, 2e+01f, 0.001f);
		ui_interface->declare(&fslider3, "style", "knob");
		ui_interface->declare(&fslider3, "unit", "Hz");
		ui_interface->addHorizontalSlider("speed", &fslider3, 0.5f, 0.0f, 1e+01f, 0.01f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow3 = (0.0005f * float(fslider2));
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp2 = ((fRec0[1] * fRec1[0]) - fTemp1);
			fVec1[IOTA&2047] = fTemp2;
			fRec3[0] = ((fSlow5 * fRec4[1]) + (fSlow6 * fRec3[1]));
			fRec4[0] = (((fSlow6 * fRec4[1]) + (fSlow7 * fRec3[1])) + (1 - iVec0[1]));
			fRec2[0] = ((0.999f * fRec2[1]) + (fConst1 * (fSlow2 + (fSlow3 * (fRec3[0] + 1)))));
			int iTemp3 = int(fRec2[0]);
			float fTemp4 = floorf(fRec2[0]);
			fRec0[0] = ((fVec1[(IOTA-int((iTemp3 & 2047)))&2047] * (fTemp4 + (1 - fRec2[0]))) + ((fRec2[0] - fTemp4) * fVec1[(IOTA-int((int((iTemp3 + 1)) & 2047)))&2047]));
			fRec5[0] = (fSlow9 + (0.999f * fRec5[1]));
			output0[i] = (FAUSTFLOAT)((iSlow0)?fTemp0:(0.5f * ((fRec0[0] * ((iSlow8)?(0 - fRec5[0]):fRec5[0])) + fTemp1)));
			// post processing
			fRec5[1] = fRec5[0];
			fRec0[1] = fRec0[0];
			fRec2[1] = fRec2[0];
...
static t_class* flanger_faust_class;
#define FAUST_EXT t_faust_flanger
#define FAUST_EXT_CLASS flanger_faust_class
// clang-format on

struct t_faust_flanger {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_flanger* x = reinterpret_cast<t_faust_flanger*>(w[1]);
...
static void flanger_faust_dsp(t_faust_flanger* x, t_signal** sp)
{
...
static void flanger_dump_to_console(t_faust_flanger* x)
{
...
static void flanger_faust_any(t_faust_flanger* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_flanger* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_flanger* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_flanger* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_flanger* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_flanger* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void flanger_faust_free(t_faust_flanger* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_flanger* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(flanger));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_flanger* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(flanger));
            return false;
        }
...
static bool faust_new_internal(t_faust_flanger* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new flanger();
    x->ui = new PdUI<UI>(sym(flanger), objId);

    if (!faust_init_inputs(x)) {
        flanger_faust_free(x);
        return false;
    }
...
public:
    ...
    t_faust_flanger* pd_obj()
    {
        return this->x_;
    }
};

static void* flanger_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s)
{
    flanger_faust_class = class_new(s, reinterpret_cast<t_newmethod>(flanger_faust_new),
        reinterpret_cast<t_method>(flanger_faust_free),
        sizeof(t_faust_flanger),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);
    class_addmethod(flanger_faust_class, nullfn, &s_signal, A_NULL);
    class_addmethod(flanger_faust_class, reinterpret_cast<t_method>(flanger_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(flanger_faust_class, reinterpret_cast<t_method>(flanger_dump_to_console), gensym("dump"), A_NULL);
    CLASS_MAINSIGNALIN(flanger_faust_class, t_faust_flanger, f);
    class_addanything(flanger_faust_class, flanger_faust_any);
}

#define EXTERNAL_NEW void* flanger_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* flanger_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_flanger* x = reinterpret_cast<t_faust_flanger*>(pd_new(flanger_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2eflanger_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".flanger~"));    \
    }
