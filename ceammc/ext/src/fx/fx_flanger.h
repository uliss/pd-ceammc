...
Compilation options: -a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn fx_flanger -scn fx_flanger_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __fx_flanger_H__
#define  __fx_flanger_H__
...
class FAUST_API fx_flanger_dsp {

    public:

        fx_flanger_dsp() = default;
        virtual ~fx_flanger_dsp() = default;
         ...
         */
        virtual ::fx_flanger_dsp* clone() = 0;
...
class FAUST_API decorator_dsp : public ::fx_flanger_dsp {
    ...
    public:
        ...
        decorator_dsp(::fx_flanger_dsp* fx_flanger_dsp = nullptr):fDSP(fx_flanger_dsp) {}
...
class FAUST_API dsp_factory {
    ...
    public:
        ...
        virtual ::fx_flanger_dsp* createDSPInstance() = 0;
...
/**
 The base class of Meta handler to be used in fx_flanger_dsp::metadata(Meta* m) method to retrieve (key, value) metadata.
 */
struct FAUST_API Meta {
    virtual ~Meta() {}
...
// clang-format off
#ifndef FAUST_MACRO
struct fx_flanger : public fx_flanger_dsp {
};
#endif
// clang-format on
...
class fx_flanger : public fx_flanger_dsp {
 ...
 public:
	...
	fx_flanger(const fx_flanger&) = default;
	
	virtual ~fx_flanger() = default;
	
	fx_flanger& operator=(const fx_flanger&) = default;
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a /Users/serge/work/music/pure-data/ceammc/faust/faust_arch_ceammc.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -cn fx_flanger -scn fx_flanger_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual fx_flanger* clone() {
		return new fx_flanger(*this);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("delay", &fHslider3, FAUSTFLOAT(1e+01f), FAUSTFLOAT(0.0f), FAUSTFLOAT(2e+01f), FAUSTFLOAT(0.001f));
		...
		ui_interface->addHorizontalSlider("offset", &fHslider2, FAUSTFLOAT(1.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(2e+01f), FAUSTFLOAT(0.001f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow3 = 0.001f * static_cast<float>(fHslider2);
		float fSlow4 = 0.0005f * static_cast<float>(fHslider3);
		float fSlow5 = fConst3 * static_cast<float>(fHslider4);
		float fSlow6 = std::sin(fSlow5);
		float fSlow7 = std::cos(fSlow5);
		float fSlow8 = static_cast<float>(((static_cast<int>(static_cast<float>(fCheckbox1))) ? -1 : 1));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp2 = fRec2[0] * fRec1[1] - fTemp1;
			fVec1[IOTA0 & 4095] = fTemp2;
			fRec4[0] = fSlow6 * fRec5[1] + fSlow7 * fRec4[1];
			fRec5[0] = static_cast<float>(1 - iVec0[1]) + fSlow7 * fRec5[1] - fSlow6 * fRec4[1];
			fRec3[0] = 44.1f * (fSlow3 + fSlow4 * (fRec4[0] + 1.0f)) + fConst2 * fRec3[1];
			int iTemp3 = static_cast<int>(fRec3[0]);
			float fTemp4 = std::floor(fRec3[0]);
			fRec1[0] = fVec1[(IOTA0 - std::min<int>(2049, std::max<int>(0, iTemp3))) & 4095] * (fTemp4 + (1.0f - fRec3[0])) + (fRec3[0] - fTemp4) * fVec1[(IOTA0 - std::min<int>(2049, std::max<int>(0, iTemp3 + 1))) & 4095];
			output0[i0] = static_cast<FAUSTFLOAT>(((iSlow0) ? fTemp0 : fTemp1 * (1.0f - fRec0[0]) + 0.5f * fRec0[0] * (fTemp1 + fRec1[0] * fSlow8)));
			iVec0[1] = iVec0[0];
			fRec0[1] = fRec0[0];
			fRec2[1] = fRec2[0];
			IOTA0 = IOTA0 + 1;
...
class faust_fx_flanger_tilde : public FaustExternal<fx_flanger> {
public:
    faust_fx_flanger_tilde(const ceammc::PdArgs& args)
        : FaustExternal(args, sym(fx_flanger))
    {
    }
};
