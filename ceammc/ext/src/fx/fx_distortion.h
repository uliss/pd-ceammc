...
static float distortion_faustpower2_f(float value) {
	return (value * value);
...
class distortion : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "fx_distortion");
		m->declare("filters.lib/name", "Faust Filters Library");
		m->declare("filters.lib/version", "0.0");
		m->declare("misceffects.lib/name", "Faust Math Library");
		m->declare("misceffects.lib/version", "2.0");
		m->declare("name", "fx_distortion");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("fx_distortion");
		ui_interface->addCheckButton("bypass", &fCheckbox0);
		ui_interface->addVerticalSlider("drive", &fVslider0, 0.5f, 0.0f, 1.0f, 0.00100000005f);
		ui_interface->declare(&fHslider0, "style", "knob");
		ui_interface->addHorizontalSlider("drywet", &fHslider0, 1.0f, 0.0f, 1.0f, 0.00999999978f);
		ui_interface->addVerticalSlider("offset", &fVslider1, 0.0f, 0.0f, 0.5f, 0.00100000005f);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp2 = std::max(-1.0f, std::min(1.0f, ((fTemp1 * std::pow(10.0f, (2.0f * fRec2[0]))) + fRec3[0])));
			float fTemp3 = (fTemp2 * (1.0f - (0.333333343f * distortion_faustpower2_f(fTemp2))));
			fVec0[0] = fTemp3;
			fRec1[0] = (((0.995000005f * fRec1[1]) + fTemp3) - fVec0[1]);
...
static t_class* distortion_faust_class;
#define FAUST_EXT t_faust_distortion
#define FAUST_EXT_CLASS distortion_faust_class
// clang-format on

template <class T>
class _distortion_UI : public UI {
};
typedef _distortion_UI<distortion> distortion_UI;

struct t_faust_distortion {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_distortion* x = reinterpret_cast<t_faust_distortion*>(w[1]);
...
static void distortion_faust_dsp(t_faust_distortion* x, t_signal** sp)
{
...
static void distortion_dump_to_console(t_faust_distortion* x)
{
...
static void distortion_faust_any(t_faust_distortion* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_distortion* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_distortion* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_distortion* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_distortion* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_distortion* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void distortion_faust_free(t_faust_distortion* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_distortion* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(distortion));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_distortion* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(distortion));
            return false;
        }
...
static bool faust_new_internal(t_faust_distortion* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new distortion();
    x->ui = new PdUI<distortion_UI>(sym(distortion), objId);

    if (!faust_init_inputs(x)) {
        distortion_faust_free(x);
        return false;
    }
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_distortion* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_distortion* pd_obj()
    {
        return this->x_;
    }
};

static void* distortion_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    distortion_faust_class = class_new(s, reinterpret_cast<t_newmethod>(distortion_faust_new),
        reinterpret_cast<t_method>(distortion_faust_free),
        sizeof(t_faust_distortion),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(distortion_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(distortion_faust_class, t_faust_distortion, f);
    }

    class_addmethod(distortion_faust_class, reinterpret_cast<t_method>(distortion_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(distortion_faust_class, reinterpret_cast<t_method>(distortion_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(distortion_faust_class, distortion_faust_any);
    ceammc::register_faust_external(distortion_faust_class);
}

#define EXTERNAL_NEW void* distortion_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* distortion_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_distortion* x = reinterpret_cast<t_faust_distortion*>(pd_new(distortion_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2edistortion_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".distortion~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2edistortion_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".distortion~"), false); \
    }
