 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			fRec2[0] = fTemp2;
			fRec3[0] = max(0.0f, min(1.0f, (fRec3[1] + fTemp2)));
			...
			fRec1[0] = (((1.0f - fRec3[0]) * fVec0[((IOTA - int(min(float(iConst3), max(0.0f, fRec4[0])))) & 2097151)]) + (fRec3[0] * fVec0[((IOTA - int(min(float(iConst3), max(0.0f, fRec5[0])))) & 2097151)]));
...
static t_class* sdelay_faust_class;
#define FAUST_EXT t_faust_sdelay
...
{
    t_faust_sdelay* x = reinterpret_cast<t_faust_sdelay*>(w[1]);
...
static void sdelay_faust_dsp(t_faust_sdelay* x, t_signal** sp)
{
...
static void sdelay_dump_to_console(t_faust_sdelay* x)
{
...
static void sdelay_faust_any(t_faust_sdelay* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_sdelay* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_sdelay* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_sdelay* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_sdelay* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_sdelay* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void sdelay_faust_free(t_faust_sdelay* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_sdelay* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(sdelay));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_sdelay* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(sdelay));
            return false;
        }
...
static bool faust_new_internal(t_faust_sdelay* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_sdelay* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_sdelay* pd_obj()
    {
        return this->x_;
    }
};

static void* sdelay_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    sdelay_faust_class = class_new(s, reinterpret_cast<t_newmethod>(sdelay_faust_new),
        reinterpret_cast<t_method>(sdelay_faust_free),
        sizeof(t_faust_sdelay),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(sdelay_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(sdelay_faust_class, t_faust_sdelay, f);
    }
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* sdelay_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_sdelay* x = reinterpret_cast<t_faust_sdelay*>(pd_new(sdelay_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2esdelay_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".sdelay~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2esdelay_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".sdelay~"), false); \
    }
