 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			output0[i] = FAUSTFLOAT(((fTemp0 * (1.0f - fRec0[0])) + (fRec0[0] * (fRec1[1] + fRec2))));
			fRec0[1] = fRec0[0];
			fRec10[1] = fRec10[0];
			fRec12[1] = fRec12[0];
			fRec11[1] = fRec11[0];
...
template <class T>
class _freeverb_UI : public UI {
};
typedef _freeverb_UI<freeverb> freeverb_UI;
...
static bool faust_new_internal(t_faust_freeverb* x, const std::string& objId = "", bool info_outlet = true)
{
    ...
    x->dsp = new freeverb();
    x->ui = new PdUI<freeverb_UI>(sym(freeverb), objId);
...
static void internal_setup(t_symbol* s, bool soundIn = true)
{
    ...
    ceammc::register_faust_external(freeverb_faust_class);
}
