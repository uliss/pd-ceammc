...
class echo : public dsp {
 ...
 public:
	...
	virtual void instanceConstants(int samplingFreq) {
		...
		iConst2 = (int((10.0f * fConst0)) + -1);
		iConst3 = ((iConst2 >> 1) | iConst2);
		iConst4 = ((iConst3 >> 2) | iConst3);
		iConst5 = ((iConst4 >> 4) | iConst4);
		iConst6 = ((iConst5 >> 8) | iConst5);
		iConst7 = max(1, (((iConst6 >> 16) | iConst6) + 1));
...
static t_class* echo_faust_class;
#define FAUST_EXT t_faust_echo
...
{
    t_faust_echo* x = reinterpret_cast<t_faust_echo*>(w[1]);
...
static void echo_faust_dsp(t_faust_echo* x, t_signal** sp)
{
...
static void echo_dump_to_console(t_faust_echo* x)
{
...
static void echo_faust_any(t_faust_echo* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_echo* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_echo* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_echo* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_echo* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_echo* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void echo_faust_free(t_faust_echo* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_echo* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(echo));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_echo* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(echo));
            return false;
        }
...
static bool faust_new_internal(t_faust_echo* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_echo* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(argc)
        , argv_(argv)
...
static void* echo_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    echo_faust_class = class_new(s, reinterpret_cast<t_newmethod>(echo_faust_new),
        reinterpret_cast<t_method>(echo_faust_free),
        sizeof(t_faust_echo),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(echo_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(echo_faust_class, t_faust_echo, f);
    }
...
#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* echo_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_echo* x = reinterpret_cast<t_faust_echo*>(pd_new(echo_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2eecho_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".echo~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2eecho_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".echo~"), false); \
    }
