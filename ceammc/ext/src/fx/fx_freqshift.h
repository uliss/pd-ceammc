...
#ifndef  __freqshift_H__
#define  __freqshift_H__
...
class freqshift : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("filename", "fx_freqshift");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("frequency shifter");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			fRec2[0] = ((0.733029008f * (fRec2[2] + fRec3[0])) - fRec3[2]);
			fRec1[0] = ((0.945349991f * (fRec1[2] + fRec2[0])) - fRec2[2]);
			fRec0[0] = ((0.990598023f * (fRec0[2] + fRec1[0])) - fRec1[2]);
			float fTemp1 = float(input1[i]);
			iRec4[0] = (iRec4[1] + 1);
			float fTemp2 = (fConst1 * (fTemp1 * std::fmod(float(iRec4[0]), (fConst0 / fTemp1))));
			float fTemp3 = (fRec0[0] * std::sin(fTemp2));
			fRec8[0] = ((0.479400992f * (fVec0[1] + fRec8[2])) - fVec0[3]);
			fRec7[0] = ((0.876218021f * (fRec7[2] + fRec8[0])) - fRec8[2]);
			fRec6[0] = ((0.976598978f * (fRec6[2] + fRec7[0])) - fRec7[2]);
			fRec5[0] = ((0.997500002f * (fRec5[2] + fRec6[0])) - fRec6[2]);
...
static t_class* freqshift_faust_class;
#define FAUST_EXT t_faust_freqshift
#define FAUST_EXT_CLASS freqshift_faust_class
// clang-format on

template <class T>
class _freqshift_UI : public UI {
};
typedef _freqshift_UI<freqshift> freqshift_UI;

struct t_faust_freqshift {
    t_object x_obj;
#ifdef __MINGW32__
...
{
    t_faust_freqshift* x = reinterpret_cast<t_faust_freqshift*>(w[1]);
...
static void freqshift_faust_dsp(t_faust_freqshift* x, t_signal** sp)
{
...
static void freqshift_dump_to_console(t_faust_freqshift* x)
{
...
static void freqshift_faust_any(t_faust_freqshift* x, t_symbol* s, int argc, t_atom* argv)
{
    if (!x->dsp)
        return;
...
static void faust_free_dsp(t_faust_freqshift* x)
{
    delete x->dsp;
    x->dsp = NULL;
}

static void faust_free_ui(t_faust_freqshift* x)
{
    delete x->ui;
    x->ui = NULL;
}

static void faust_free_inputs(t_faust_freqshift* x)
{
    if (x->inputs)
        free(x->inputs);
    x->inputs = NULL;
...
static void faust_free_outputs(t_faust_freqshift* x)
{
    if (x->outputs)
        free(x->outputs);
    x->outputs = NULL;
...
static void faust_free_buf(t_faust_freqshift* x)
{
    if (x->buf) {
        for (int i = 0; i < x->n_out; i++) {
            if (x->buf[i])
...
static void freqshift_faust_free(t_faust_freqshift* x)
{
    faust_free_dsp(x);
    faust_free_ui(x);
    faust_free_inputs(x);
...
static bool faust_init_inputs(t_faust_freqshift* x)
{
    x->inputs = NULL;
    ...
    if (x->n_in > 0) {
        ...
        if (x->inputs == NULL) {
            error("[%s] faust_init_inputs failed", sym(freqshift));
            return false;
        }
    }
...
static bool faust_init_outputs(t_faust_freqshift* x, bool info_outlet)
{
    x->outputs = NULL;
    x->buf = NULL;
    ...
    if (x->n_out > 0) {
        ...
        if (x->outputs == NULL) {
            error("[%s] faust_init_outputs failed", sym(freqshift));
            return false;
        }
...
static bool faust_new_internal(t_faust_freqshift* x, const std::string& objId = "", bool info_outlet = true)
{
    int sr = 44100;
    x->active = 1;
    x->xfade = 0;
    ...
    x->dsp = new freqshift();
    x->ui = new PdUI<freqshift_UI>(sym(freqshift), objId);

    if (!faust_init_inputs(x)) {
        freqshift_faust_free(x);
        return false;
    }
...
public:
     ...
     * @param argv pointer to argument vector
     */
    PdArgParser(t_faust_freqshift* x, int argc, t_atom* argv, bool info_outlet = true)
        : x_(x)
        , argc_(0)
        , argv_(argv)
    ...
    t_faust_freqshift* pd_obj()
    {
        return this->x_;
    }
};

static void* freqshift_faust_new(t_symbol* s, int argc, t_atom* argv);

static void internal_setup(t_symbol* s, bool soundIn = true)
{
    freqshift_faust_class = class_new(s, reinterpret_cast<t_newmethod>(freqshift_faust_new),
        reinterpret_cast<t_method>(freqshift_faust_free),
        sizeof(t_faust_freqshift),
        CLASS_DEFAULT,
        A_GIMME, A_NULL);

    if (soundIn) {
        class_addmethod(freqshift_faust_class, nullfn, &s_signal, A_NULL);
        CLASS_MAINSIGNALIN(freqshift_faust_class, t_faust_freqshift, f);
    }

    class_addmethod(freqshift_faust_class, reinterpret_cast<t_method>(freqshift_faust_dsp), gensym("dsp"), A_NULL);
    class_addmethod(freqshift_faust_class, reinterpret_cast<t_method>(freqshift_dump_to_console), gensym("dump"), A_NULL);
    class_addanything(freqshift_faust_class, freqshift_faust_any);
    ceammc::register_faust_external(freqshift_faust_class);
}

#define EXTERNAL_NEW void* freqshift_faust_new(t_symbol*, int argc, t_atom* argv)

#define EXTERNAL_SIMPLE_NEW()                                                           \
    static void* freqshift_faust_new(t_symbol*, int argc, t_atom* argv)                     \
    {                                                                                   \
        t_faust_freqshift* x = reinterpret_cast<t_faust_freqshift*>(pd_new(freqshift_faust_class)); \
        PdArgParser p(x, argc, argv, false);                                            \
        return p.pd_obj();                                                              \
    }

#define EXTERNAL_SETUP(MOD)                        \
    extern "C" void setup_##MOD##0x2efreqshift_tilde() \
    {                                              \
        internal_setup(gensym(#MOD ".freqshift~"));    \
    }

#define EXTERNAL_SETUP_NO_IN(MOD)                      \
    extern "C" void setup_##MOD##0x2efreqshift_tilde()     \
    {                                                  \
        internal_setup(gensym(#MOD ".freqshift~"), false); \
    }
