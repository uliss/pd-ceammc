	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (index = 0; index <= fullcount - 64; index += 64) {
			...
			for (int i=0; i<count; i++) {
				fRec0[i] = (fSlow3 + (0.9999f * fRec0[i-1]));
			}
			// post processing
			...
			// exec code
			for (int i=0; i<count; i++) {
				fRec1[i] = (fSlow4 + (0.9999f * fRec1[i-1]));
			}
			// post processing
			...
			// exec code
			for (int i=0; i<count; i++) {
				fRec2[i] = (fSlow5 + (0.9999f * fRec2[i-1]));
			}
			// post processing
			...
			// exec code
			for (int i=0; i<count; i++) {
				fRec3[i] = (fSlow6 + (0.9999f * fRec3[i-1]));
			}
			// post processing
			...
			// exec code
			for (int i=0; i<count; i++) {
				output1[i] = (FAUSTFLOAT)((float)input0[i] * fRec1[i]);
			}
			
			// LOOP 0x7fdeaa426360
			// exec code
			for (int i=0; i<count; i++) {
				output2[i] = (FAUSTFLOAT)((float)input0[i] * fRec2[i]);
			}
			
			// LOOP 0x7fdeaa427b80
			// exec code
			for (int i=0; i<count; i++) {
				output3[i] = (FAUSTFLOAT)((float)input0[i] * fRec3[i]);
			}
...
static void faust_init_label(t_faust* x, const char* obj_id) {
    x->label = new std::string(sym(pan4) "~");
