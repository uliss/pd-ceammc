  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (index = 0; index <= fullcount - 64; index += 64) {
			...
			FAUSTFLOAT* input4 = &input[4][index];
			FAUSTFLOAT* input5 = &input[5][index];
			...
			// exec code
			for (int i=0; i<count; i++) {
				fRec0[i] = ((fRec0[i-2] * (0 - (float)input4[i])) + ((((fYec0[i-2] * (float)input2[i]) + (fYec0[i-1] * (float)input1[i])) + ((float)input0[i] * (float)input5[i])) + (fRec0[i-1] * (0 - (float)input3[i]))));
			}
			// post processing
...
static void faust_init_label(t_faust* x, const char* obj_id) {
    x->label = new std::string(sym(biquad) "~");
...
static bool faust_new_internal(t_faust* x, const char* obj_id = NULL) {
    ...
    x->dsp = new biquad();
    x->ui = new PdUI(sym(biquad), obj_id);
