...
class pitchshift : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("fade", &fslider1, 1e+01f, 1e+04f, 1e+01f, 0.1f);
		ui_interface->addVerticalSlider("window", &fslider0, 1e+01f, 1e+04f, 1e+01f, 0.1f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (index = 0; index <= fullcount - 64; index += 64) {
			...
			for (int i=0; i<count; i++) {
				fRec0[i] = fmodf((fSlow0 + (fRec0[i-1] + (1 - powf(2,(0.083333336f * (float)input1[i]))))),fSlow0);
			}
			// post processing
...
static bool faust_new_internal(t_faust* x, const char* obj_id = NULL) {
    ...
    x->ui = new PdUI(sym(pitchshift), obj_id);
