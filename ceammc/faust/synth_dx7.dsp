...
with{
    ...
    gate = button("gate");
    egR1UI = par(i, 6, nentry("op%i:R1",90,0,99,1));
    egR1(n) = ba.take(n+1,egR1UI);
    egR2UI = par(i, 6, nentry("op%i:R2",90,0,99,1));
    egR2(n) = ba.take(n+1,egR2UI);
    egR3UI = par(i,6,nentry("op%i:R3",90,0,99,1));
    egR3(n) = ba.take(n+1,egR3UI);
    egR4UI = par(i,6,nentry("op%i:R4",90,0,99,1));
    egR4(n) = ba.take(n+1,egR4UI);
    egL1UI = par(i,6,nentry("op%i:L1",0,0,99,1));
    egL1(n) = ba.take(n+1,egL1UI);
    egL2UI = par(i,6,nentry("op%i:L2",90,0,99,1));
    egL2(n) = ba.take(n+1,egL2UI);
    egL3UI = par(i,6,nentry("op%i:L3",90,0,99,1));
    egL3(n) = ba.take(n+1,egL3UI);
    egL4UI = par(i,6,nentry("op%i:L4",0,0,99,1));
    egL4(n) = ba.take(n+1,egL4UI);
    outLevelUI = par(i,6,nentry("op%i:level",95,0,99,1));
    outLevel(n) = ba.take(n+1,outLevelUI);
    keyVelSensUI = par(i,6,nentry("op%i:vel",1,0,8,1));
    keyVelSens(n) = ba.take(n+1,keyVelSensUI);
    ampModSensUI = par(i,6,nentry("op%i:mod",0,0,99,1));
    ...
    opRateScaleUI = par(i,6,nentry("op%i:rate",0,0,10,1));
