...
centroid = hslider("centroid", 60, 20, 120, 0.001);
range    = hslider("range", 120, 1, 120, 0.001) / 2.;
Npartials = 10;
tblsize = 1 << 16;
...
with {
  ...
  partial(pos) = pos : *(2.) : -(1.) : *(range) : +(centroid) : +(7) : cm.clip(20, 120)
        : ba.midikey2hz : os.osc * lookup(pos, thin_env);
};
