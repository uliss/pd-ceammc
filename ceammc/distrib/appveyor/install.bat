...
bash -lc "cd ${APPVEYOR_BUILD_FOLDER}/build && cmake -G 'MSYS Makefiles' -DCMAKE_BUILD_TYPE=Release -DWITH_FFTW=ON -DWITH_EXT_FLEXT=OFF -DWITH_EXT_LYONPOTPOURRI=OFF -DWITHEXT_FFTEASE=OFF -DWITH_BENCHMARK=OFF -DCMAKE_INSTALL_PREFIX=/opt/local/release/pd ${APPVEYOR_BUILD_FOLDER}"
