...
struct _rtext
{
    /** \brief raw byte string, assumed UTF-8 encoded (moo) --*/
    char *x_buf;
    /** \brief byte length --*/
    int x_bufsize;
    /** \brief byte offset --*/
...
/** \brief convert t_text te_type symbol for use as a Tk tag */
...
/** \brief the following routine computes line breaks and carries out
 some action
...
*/
        ...
        /* \briefmoo: check for unicode codepoints beyond 7-bit ASCII --*/
        else if (n > 127)
        {
            ...
            x->x_bufsize = newsize;
            /* \brief moo: assume canvas_key() has encoded keysym as UTF-8 */
