/** \brief Copyright (c) 1997-2001 Miller Puckette and others.
* For information on usage and redistribution, and for a DISCLAIMER OF ALL
* WARRANTIES, see the file, "LICENSE.txt," in this distribution.  */

/** \brief this file defines the "glist" class, also known as "canvas" (the two used
...
#define snprintf sprintf_s
    ...
    /** \brief LATER consider adding font size to this struct (see glist_getfont()) */
struct _canvasenvironment
{
    /** \brief directory patch lives in */
    t_symbol *ce_dir;
...
/** \brief since the window decorations aren't included, open new windows a few
...
/** \defgroup gCc_01 g_canvas.c: variables
 *@{ */
...
/** \defgroup gCc_02 g_canvas.c: forward function declarations
 * @{ */
...
/** \defgroup gCc_03 g_canvas.c: functions to handle the canvas environment
 * @{ */
...
t_glist *glist_reloadingabstraction;
    ...
    /** \brief maintain the list of visible toplevels for the GUI's "windows" menu */
void canvas_updatewindowlist( void)
{
    if (!glist_reloadingabstraction)  /** \brief not if we're in a reload */
        sys_gui("::pd_menus::update_window_menu\n");
}

    /** \brief add a glist the list of "root" canvases (toplevels without parents.) */
static void canvas_addtolist(t_canvas *x)
{
...
static void canvas_takeofflist(t_canvas *x)
{
        /** \brief take it off the window list */
...
{
        /** \brief if there's an old one lying around free it here.  This
...
/** \defgroup gCc_04 g_canvas.c: traversing the set of lines in a canvas
 * @{ */
...
/** @} */
/** \defgroup gCc_05 g_canvas.c: the canvas object
 * @{ */
int glist_valid = 10000;

void glist_init(t_glist *x)
{
        /** \brief zero out everyone except "pd" field */
    ...
    /** \brief make a new glist.  It will either be a "root" canvas or else
...
t_canvas *canvas_new(void *dummy, t_symbol *sel, int argc, t_atom *argv)
{
    ...
    if (!owner)
        canvas_addtolist(x);
    /** \brief post("canvas %lx, owner %lx", x, owner); */

    if (argc == 5)  /** \brief toplevel: x, y, w, h, font */
    {
    ...
    else if (argc == 6)  /** \brief subwindow: x, y, w, h, name, vis */
    {
        xloc = atom_getintarg(0, argc, argv);
        yloc = atom_getintarg(1, argc, argv);
        width = atom_getintarg(2, argc, argv);
        ...
        /** \brief (otherwise assume we're being created from the menu.) */
    ...
    canvas_bind(x);
    x->gl_loading = 1;
    x->gl_goprect = 0;      /** \brief no GOP rectangle unless it's turned on later */
        /** \brief cancel "vis" flag if we're a subpatch of an
...
static void canvas_coords(t_glist *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    /** \brief make a new glist and add it to this glist.  It will appear as
    ...
    t_float px1, t_float py1, t_float px2, t_float py2)
{
    ...
    if (!*sym->s_name)
    {
        ...
        && (zz = atoi(str + 5)) > gcount)
            gcount = zz;
        /** \brief in 0.34 and earlier, the pixel rectangle and the y bounds were
    ...
    /** \brief call glist_addglist from a Pd message */
void glist_glist(t_glist *g, t_symbol *s, int argc, t_atom *argv)
{
    ...
    /** \brief return true if the glist should appear as a graph on parent;
...
int glist_isgraph(t_glist *x)
{
  return (x->gl_isgraph|(x->gl_hidetext<<1));
    ...
    /** \brief This is sent from the GUI to inform a toplevel that its window has been
    moved or resized. */
...
/** \brief this is the internal version using ints */
static void canvas_dosetbounds(t_canvas *x, int x1, int y1, int x2, int y2)
{
    int heightwas = y2 - y1;
    ...
    {
            /** \brief if it's flipped so that y grows upward,
        ...
        x->gl_y2 = x->gl_y1 - diff;
            /** \brief and move text objects accordingly; they should stick
...
t_symbol *canvas_makebindsym(t_symbol *s)
{
    ...
    /** \brief functions to bind and unbind canvases to symbol "pd-blah".  As
...
void canvas_reflecttitle(t_canvas *x)
{
    ...
    /** \brief mark a glist dirty or clean */
...
EXTERN void canvas_drawgrid(t_canvas *x)
{
    ...
    /** \brief the window becomes "mapped" (visible and not miniaturized) or
...
void canvas_map(t_canvas *x, t_floatarg f)
{
    ...
    else
    {
        if (glist_isvisible(x))
        {
                /** \brief just clear out the whole canvas */
...
void canvas_redraw(t_canvas *x)
{
    ...
    /** \brief we call this on a non-toplevel glist to "open" it into its
    own window. */
void glist_menu_open(t_glist *x)
{
    if (glist_isvisible(x) && !glist_istoplevel(x))
    {
        ...
        if (!gl2)
            bug("glist_menu_open");  /** \brief shouldn't happen but not dangerous */
        else
        {
                /** \brief erase ourself in parent window */
            gobj_vis(&x->gl_gobj, gl2, 0);
                    /** \brief get rid of our editor (and subeditors) */
            if (x->gl_editor)
                canvas_destroy_editor(x);
            x->gl_havewindow = 1;
                    /** \brief redraw ourself in parent window (blanked out this time) */
...
{
        /** \brief we consider a graph "toplevel" if it has its own window
...
void canvas_free(t_canvas *x)
{
    ...
    if (x == glist_getcanvas(x))
        canvas_vis(x, 0);
    if (x->gl_editor)
        canvas_destroy_editor(x);   /** \brief bug workaround; should already be gone*/
    canvas_unbind(x);
    ...
    gfxstub_deleteforkey(x);        /** \brief probably unnecessary */
...
/** @} */
/** \defgroup gCc_06 g_canvas.c: lines
 * @{*/
...
void canvas_fixlinesfor(t_canvas *x, t_text *text)
{
    ...
    /** \brief kill all lines for the object */
void canvas_deletelinesfor(t_canvas *x, t_text *text)
{
    ...
    /** \brief kill all lines for one inlet or outlet */
...
void canvas_initbang(t_canvas *x)
{
    t_gobj *y;
    t_symbol *s = gensym("loadbang");
    /** \brief run "initbang" for all subpatches, but NOT for the child abstractions */
    for (y = x->gl_list; y; y = y->g_next)
        if (pd_class(&y->g_pd) == canvas_class &&
            !canvas_isabstraction((t_canvas *)y))
                canvas_initbang((t_canvas *)y);
    /** \brief call the initbang()-method for objects that have one */
    for (y = x->gl_list; y; y = y->g_next)
...
void canvas_closebang(t_canvas *x)
{
    ...
    /** \brief call the closebang()-method for objects that have one
...
/** \brief no longer used by 'pd-gui', but kept here for backwards compatibility.  The
    ...
    t_symbol *topgeom)
{
    ...
    if (sscanf(canvasgeom->s_name, "%dx%d+%d+%d", &cw, &ch, &cxpix, &cypix)
        ...
        bug("canvas_relocate");
            /** \brief for some reason this is initially called with cw=ch=1 so
...
/** @} */
/** \defgroup gCc_07 g_canvas.c: subcanvases
 * @{*/
        ...
        t_floatarg shift, t_floatarg ctrl, t_floatarg alt)
{
    ...
    /** \brief find out from subcanvas contents how much to fatten the box */
void canvas_fattensub(t_canvas *x,
    int *xp1, int *yp1, int *xp2, int *yp2)
{
    t_gobj *y;
    *xp2 += 50;     /** \brief fake for now */
    *yp2 += 50;
}
...
static void canvas_rename_method(t_canvas *x, t_symbol *s, int ac, t_atom *av)
{
    ...
    /** \brief return true if the "canvas" object is an abstraction (so we don't
...
int canvas_isabstraction(t_canvas *x)
{
    ...
    /** \brief return true if the "canvas" object should be treated as a text
    object.  This is true for abstractions but also for "table"s... */
/** \brief JMZ: add a flag to gop-abstractions to hide the title */
int canvas_showtext(t_canvas *x)
{
    ...
    /** \brief get the document containing this canvas */
...
/** @} */
/** \defgroup gCc_08 g_canvas.c: DSP chain handling
 * @{ */
...
void ugen_done_graph(t_dspcontext *dc);
    ...
    /** \brief schedule one canvas for DSP.  This is called below for all "root"
...
void canvas_dodsp(t_canvas *x, int toplevel, t_signal **sp)
{
    ...
    t_dspcontext *dc;
        ...
        /** \brief create a new "DSP graph" object to use in sorting this canvas.
    ...
    dc = ugen_start_graph(toplevel, sp,
        ...
        /** \brief find all the "dsp" boxes and add them to the graph */

    for (y = x->gl_list; y; y = y->g_next)
        if ((ob = pd_checkobject(&y->g_pd)) && zgetfn(&y->g_pd, dspsym))
            ugen_add(dc, ob);

        /** \brief ... and all dsp interconnections */
    linetraverser_start(&t, x);
    while ((oc = linetraverser_next(&t)))
        if (obj_issignaloutlet(t.tr_ob, t.tr_outno))
            ugen_connect(dc, t.tr_ob, t.tr_outno, t.tr_ob2, t.tr_inno);

        /** \brief finally, sort them and add them to the DSP chain */
    ugen_done_graph(dc);
}
...
int canvas_dspstate;
    ...
    /** \brief this routine starts DSP for all root canvases. */
static void canvas_start_dsp(void)
{
    t_canvas *x;
...
static void canvas_stop_dsp(void)
{
    ...
    /** \brief DSP can be suspended before, and resumed after, operations which
...
void canvas_resume_dsp(int oldstate)
{
    ...
    /** \brief this is equivalent to suspending and resuming in one step. */
void canvas_update_dsp(void)
{
    if (pd_this->pd_dspstate) canvas_start_dsp();
}

/** \brief the "dsp" message to pd starts and stops DSP somputation, and, if
...
/******************* redrawing  data *********************/
    ...
    /** \brief redraw all "scalars" (do this if a drawing command is changed.)
...
static void glist_redrawall(t_glist *gl, int action)
{
    ...
    /** \brief public interface for above. */
void canvas_redrawallfortemplate(t_template *template, int action)
{
    t_canvas *x;
        /** \brief find all root canvases */
    for (x = pd_getcanvaslist(); x; x = x->gl_next)
        glist_redrawall(x, action);
}

    /** \brief find the template defined by a canvas, and redraw all elements
    for that */
...
/** @} */
/** \defgroup gCc_09 g_canvas.c: declare
 * @{ */

/** \brief put "declare" objects in a patch to tell it about the environment in
...
static void *declare_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    x->x_canvas = canvas_getcurrent();
        /** \brief LATER update environment and/or load libraries */
    if (!x->x_canvas->gl_loading)
    {
        /** \brief the object is created by the user (not by loading a patch),
...
{
    x->x_useme = 0;
        /** \brief LATER update environment */
}
...
void canvas_savedeclarationsto(t_canvas *x, t_binbuf *b)
{
    ...
    for (y = x->gl_list; y; y = y->g_next)
    {
        if (pd_class(&y->g_pd) == declare_class)
        {
            ...
            /** \brief before 0.47 we also allowed abstractions to write out to the
...
/** \brief maybe we should rename check_exists() to sys_access() and move it to s_path */
#ifdef _WIN32
...
static void canvas_stdpath(t_canvasenvironment *e, char *stdpath)
{
    ...
    /** \brief strip    "extra/"-prefix */
    if (!strncmp("extra/", stdpath, 6))
        stdpath+=6;

        /** \brief prefix full pd-path (including extra) */
    ...
    /** \brief check whether the given subdir is in one of the standard-paths */
...
static void canvas_stdlib(t_canvasenvironment *e, char *stdlib)
{
    ...
    /** \brief check whether the given library is located in one of the standard-paths */
    for (nl=sys_staticpath; nl; nl=nl->nl_next)
    {
...
static int canvas_open_iter(const char *path, t_canvasopen *co)
{
    ...
    /** \brief utility function to read a file, looking first down the canvas's search
    ...
    char *dirresult, char **nameresult, unsigned int size, int bin)
{
    ...
    t_canvasopen co;
        ...
        /** \brief first check if "name" is absolute (and if so, try to open) */
    if (sys_open_absolute(name, ext, dirresult, nameresult, size, bin, &fd))
        return (fd);

        /** \brief otherwise "name" is relative; iterate over all the search-paths */
    co.name = name;
    co.ext = ext;
...
int canvas_path_iterate(t_canvas*x, t_canvas_path_iterator fun, void *user_data)
{
    ...
    if (!fun)
        return 0;
        /** \brief iterate through canvas-local paths */
        ...
        if (y->gl_env)
    {
        ...
        for (nl = y->gl_env->ce_path; nl; nl = nl->nl_next)
        {
            ...
            {   /** \brief if not absolute path, append Pd lib dir */
    ...
    /** \brief try canvas dir */
    if (!fun((x ? canvas_getdir(x)->s_name : "."), user_data))
        return count+1;
    count++;

    /** \brief now iterate through the global paths */
...
extern t_class *array_define_class;     /** \brief LATER datum class too */

    /** \brief check if a pd can be treated as a glist - true if we're of any of
...
/** @} */
/** \defgroup gCc_10 g_canvas.c: setup routine
 * @{ */

    /** \brief why are some of these "glist" and others "canvas"? */
...
{
        /** \brief we prevent the user from typing "canvas" in an object box
    ...
    canvas_class = class_new(gensym("canvas"), 0,
        (t_method)canvas_free, sizeof(t_canvas), CLASS_NOINLET, 0);
            /** \brief here is the real creator function, invoked in patch files
...
/** \brief -------------- IEMGUI: button, toggle, slider, etc.  ------------ */
...
/** \brief -------------- CEAMMC GUI: keyboard, sliders etc.   ------------ */
...
/** \brief ----- subcanvases, which you get by typing "pd" in a box ---- */
...
/*--------------- future message to set formatting  -------------- */
    class_addmethod(canvas_class, (t_method)canvas_f,
        gensym("f"), A_GIMME, 0);
/** \brief -------------- setups from other files for canvas_class ---------------- */
    g_graph_setup();
    g_editor_setup();
    g_readwrite_setup();
}

    /** \brief functions to add basic gui (e.g., clicking but not editing) to things
