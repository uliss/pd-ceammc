...
/** \detail this file defines the structure for "glists" and related structures and
...
/** \defgroup gC1 g_canvas : geometry
 * @{*/
#define IOWIDTH 7       /** \brief width of an inlet/outlet in pixels */
...
#define GLIST_DEFGRAPHHEIGHT 140
    ...
    /** \defgroup gC2 g_canvas : data
     * @{*/
typedef struct _updateheader
{
    struct _updateheader *upd_next;
    /** \brief true if array, false if glist */
    unsigned int upd_array:1;
    /** \brief true if we're queued */
    unsigned int upd_queued:1;
} t_updateheader;

    /** \brief types to support glists grabbing mouse motion or keys from parent */
...
typedef struct _selection
{
    ...
    /** \brief this structure is instantiated whenever a glist becomes visible. */
typedef struct _editor
{
    /** \brief update header structure */
    t_updateheader e_upd;
    /** \brief list of objects to update */
    t_selection *e_updlist;
    /** \brief text responder linked list */
    t_rtext *e_rtext;
    /** \brief head of the selection list */
    t_selection *e_selection;
    /** \brief the rtext if any that we are editing */
    t_rtext *e_textedfor;
    /** \brief object being "dragged" */
    t_gobj *e_grab;
    /** \brief ... motion callback */
    t_glistmotionfn e_motionfn;
    /** \brief ... keypress callback */
    t_glistkeyfn e_keyfn;
    /** \brief connections to deleted objects */
    t_binbuf *e_connectbuf;
    /** \brief last stuff we deleted */
    t_binbuf *e_deleted;
    /** \brief GUI connection for filtering messages */
    t_guiconnect *e_guiconnect;
    /** \brief glist which owns this */
    struct _glist *e_glist;
    /** \brief xpos on last mousedown or motion event */
    int e_xwas;
    /** \brief ypos on last mousedown or motion event  */
    int e_ywas;
    /** \brief indices for the selected line if any */
    int e_selectline_index1;
    /** \brief (only valid if e_selectedline is set) */
    int e_selectline_outno;
    int e_selectline_index2;
    int e_selectline_inno;
    t_outconnect *e_selectline_tag;
    /** \brief action to take on motion */
    unsigned int e_onmotion: 3;
    /** \brief one if mouse has moved since click */
    unsigned int e_lastmoved: 1;
    /** \brief one if e_textedfor has changed */
    unsigned int e_textdirty: 1;
    /** \brief one if a line is selected */
    unsigned int e_selectedline: 1;
    /** \brief clock to filter GUI move messages */
    t_clock *e_clock;
    /** \brief xpos for next move event */
    int e_xnew;
    /** \brief ypos for next move event */
    int e_ynew;
} t_editor;

#define MA_NONE    0    /** \brief e_onmotion: do nothing on mouse motion */
#define MA_MOVE    1    /** \brief drag the selection around */
#define MA_CONNECT 2    /** \brief make a connection */
#define MA_REGION  3    /** \brief selection region */
#define MA_PASSOUT 4    /** \brief send on to e_grab */
#define MA_DRAGTEXT 5   /** \brief drag in text editor to alter selection */
#define MA_RESIZE  6    /** \brief drag to resize */

/** \brief editor structure for "garrays".  We don't bother to delete and regenerate
...
{
    t_updateheader av_upd;          /** \brief update header structure */
    t_garray *av_garray;            /** \brief owning structure */
} t_arrayvis;

/** \brief the t_tick structure describes where to draw x and y "ticks" for a glist */

typedef struct _tick    /** \brief where to put ticks on x or y axes */
{
    t_float k_point;      /** \brief one point to draw a big tick at */
    t_float k_inc;        /** \brief x or y increment per little tick */
    int k_lperb;        /** \brief little ticks per big; 0 if no ticks to draw */
} t_tick;

/** \brief the t_glist structure, which describes a list of elements that live on an
area of a window.
...
struct _glist
{
    /** \brief header in case we're a glist */
    t_object gl_obj;
    /** \brief the actual data */
    t_gobj *gl_list;
    /** \brief safe pointer handler */
    struct _gstub *gl_stub;
    /** \brief incremented when pointers might be stale */
    int gl_valid;
    /** \brief parent glist, supercanvas, or 0 if none */
    struct _glist *gl_owner;
    /** \brief width in pixels (on parent, if a graph) */
    int gl_pixwidth;
    /** \brief bounding rectangle in our own coordinates */
    int gl_pixheight;
    t_float gl_x1;
    t_float gl_y1;
    t_float gl_x2;
    t_float gl_y2;
    /** \brief screen coordinates when toplevel */
    int gl_screenx1;
    /** \brief screen coordinates when toplevel */
    int gl_screeny1;
    /** \brief screen coordinates when toplevel */
    ...
    /** \brief screen coordinates when toplevel */
    int gl_screeny2;
    /** \brief origin for GOP rectangle */
    int gl_xmargin;
    /** \brief origin for GOP rectangle */
    int gl_ymargin;
    /** \brief ticks marking X values */
    t_tick gl_xtick;
    /** \brief number of X coordinate labels */
    int gl_nxlabels;
    /** \brief ... an array to hold X coordinate labels */
    t_symbol **gl_xlabel;
    /** \brief ... brief number of Y coordinate labels */
    t_float gl_xlabely;
    /** \brief an array to hold Y coordinate labels */
    t_tick gl_ytick;
    int gl_nylabels;
    t_symbol **gl_ylabel;
    t_float gl_ylabelx;
    /** \brief editor structure when visible */
    t_editor *gl_editor;
    /** \brief symbol bound here */
    t_symbol *gl_name;
    /** \brief nominal font size in points, e.g., 10 */
    int gl_font;
    /** \brief zoom factor (integer zoom-in only) */
    int gl_zoom;
    /** \brief link in list of toplevels */
    struct _glist *gl_next;
    /** \brief root canvases and abstractions only */
    t_canvasenvironment *gl_env;
    /** \brief true if we own a window */
    unsigned int gl_havewindow:1;
    /** \brief true if, moreover, it's "mapped" */
    unsigned int gl_mapped:1;
    /** \brief (root canvas only:) patch has changed */
    unsigned int gl_dirty:1;
    /** \brief am now loading from file */
    unsigned int gl_loading:1;
    /** \brief make me visible after loading */
    unsigned int gl_willvis:1;
    /** \brief edit mode */
    unsigned int gl_edit:1;
    /** \brief we're inside glist_delete -- hack! */
    unsigned int gl_isdeleting:1;
    /** \brief draw rectangle for graph-on-parent */
    unsigned int gl_goprect:1;
    /** \brief show as graph on parent */
    unsigned int gl_isgraph:1;
    /** \brief hide object-name + args when doing graph on parent */
    unsigned int gl_hidetext:1;
    /** \brief private flag used in x_scalar.c */
    unsigned int gl_private:1;
    /** \brief esists as part of a clone object */
    unsigned int gl_isclone:1;
    /** \brief CEAMMC grid flag*/
...
/** \brief a data structure to describe a field in a pure datum */
...
typedef struct _dataslot
{
    int ds_type;
    t_symbol *ds_name;
    /** \brief filled in for arrays only */
    t_symbol *ds_arraytemplate;
} t_dataslot;

typedef struct _template
{
    t_pd t_pdobj;               /** \brief header */
    struct _gtemplate *t_list;  /** \brief list of "struct"/gtemplate objects */
    t_symbol *t_sym;            /** \brief name */
    int t_n;                    /** \brief number of dataslots (fields) */
    t_dataslot *t_vec;          /** \brief array of dataslots */
} t_template;

struct _array
{
    int a_n;            /** \brief number of elements */
    int a_elemsize;     /** \brief size in bytes; LATER get this from template */
    char *a_vec;        /** \brief array of elements */
    t_symbol *a_templatesym;    /** \brief template for elements */
    int a_valid;        /** \brief protection against stale pointers into array */
    t_gpointer a_gp;    /** \brief pointer to scalar or array element we're in */
    t_gstub *a_stub;    /** \brief stub for pointing into this array */
};

    /** \brief structure for traversing all the connections in a glist */
typedef struct _linetraverser
{
    ...
    int tr_nextoutno;
        ...
        /** \brief Call this to get a gobj's bounding rectangle in pixels */
typedef void (*t_getrectfn)(t_gobj *x, struct _glist *glist,
    int *x1, int *y1, int *x2, int *y2);
        /** \brief and this to displace a gobj: */
typedef void (*t_displacefn)(t_gobj *x, struct _glist *glist, int dx, int dy);
        /** \brief change color to show selection: */
typedef void (*t_selectfn)(t_gobj *x, struct _glist *glist, int state);
        /** \brief change appearance to show activation/deactivation: */
typedef void (*t_activatefn)(t_gobj *x, struct _glist *glist, int state);
        /** \brief warn a gobj it's about to be deleted */
typedef void (*t_deletefn)(t_gobj *x, struct _glist *glist);
        /** \brief  making visible or invisible */
typedef void (*t_visfn)(t_gobj *x, struct _glist *glist, int flag);
        /** \brief field a mouse click (when not in "edit" mode) */
typedef int (*t_clickfn)(t_gobj *x, struct _glist *glist,
    int xpix, int ypix, int shift, int alt, int dbl, int doit);
        /** \brief ... and later, resizing; getting/setting font or color... */
    
    /** \brief function types used to define graphical behavior for gobjs, a bit like X
...
/** \defgroup gC3 g_canvas : behaviors for scalars defined by objects in template
 * @{ */
/** \brief these are set by "drawing commands" in g_template.c which add appearance to
...
struct _parentwidgetbehavior
{
    ...
    /** \brief cursor definitions; used as return value for t_parentclickfn */
...
EXTERN void canvas_setcursor(t_glist *x, unsigned int cursornum);
    ...
    /** \brief last canvas we did a find in */
extern t_canvas *canvas_whichfind;
extern t_class *vinlet_class, *voutlet_class;
    /** \brief incremented when pointers might be stale */
extern int glist_valid;

    /** \brief plotting styles for arrays */
...
/** \defgroup gC4 g_canvas : functions on any gobj
 * @{*/
...
/** \defgroup gC5 g_canvas : functions on glists
 * @{ */
...
void canvas_deletelinesforio(t_canvas *x, t_text *text,
    ...
    /** \brief stop GUI changes while reloading */
extern int glist_amreloadingabstractions;
    /** 
     *@} */
    
/** \defgroup gC7 g_canvas : functions on texts
 * @{*/
...
/** \defgroup gC8 g_canvas : functions on rtexts
 * @{*/
#define RTEXT_DOWN 1
#define RTEXT_DRAG 2
#define RTEXT_DBL 3
...
/** \defgroup gC9 g_canvas : functions on canvases
 * @{*/
...
EXTERN int canvas_setdeleting(t_canvas *x, int flag);
    ...
    /** \brief "loadbang" actions - 0 for original meaning */
#define LB_LOAD 0
    /** \brief loaded but not yet connected to parent patch */
#define LB_INIT 1
    /** \brief about to close */
#define LB_CLOSE 2

    /** \brief Pointer to canvas that was saved necessitating a reload of abstractions
...
EXTERN t_glist *glist_reloadingabstraction;
    ...
    /** \brief a function that does UNDO/REDO */
typedef void (*t_undofn)(t_canvas *canvas, void *buf,
    int action);
    /** \brief free current undo/redo buffer */
#define UNDO_FREE 0
    /** \brief undo */
#define UNDO_UNDO 1
    /** \brief redo */
...
/** \brief a function that gets called for each path by canvas_path_iterate
...
/** \defgroup gC10 g_canvas : functions on canvasses as objects
 * @{*/
...
/** \defgroup gC11 g_canvas : functions on garrays (graphical arrays)
 * @{*/
...
/** \defgroup gC13 g_canvas : gpointers and stubs
 * @{*/
...
/** \defgroup gC14 g_canvas : scalars
 * @{*/
...
/** \defgroup gC15 g_canvas : helper routines for "garrays" and "plots"
 * @{*/
...
/** \defgroup gC16 g_canvas : templates
 * @{*/
...
/** \defgroup gC17 g_canvas : guiconnects, g_guiconnect.c
 * @{*/
...
/** \defgroup gC18 g_canvas : IEMGUI routines used in other g_ files
 * @{*/
...
/** \defgroup gC19 g_canvas : g_clone.c
 * @{*/
extern t_class *clone_class;
    /** 
     *@} */
    
/** \defgroup gC20 g_canvas : CEAMMC grid
 * @{*/
    /** \brief draws grid; added to canvas draw routine
     */
