...
{
    t_eclass* c = eclass_new("ui.colorpanel", (method)colorpanel_new, (method)colorpanel_free,
        sizeof(t_colorpanel), 0L, A_GIMME, 0);
    ...
    CLASS_ATTR_LABEL                (c, "reverse", 0, _("Matrix Reversed"));
    ...
    CLASS_ATTR_LABEL                (c, "saturation", 0, _("Saturation"));
    ...
    CLASS_ATTR_LABEL                (c, "hue", 0, _("First Hue"));
    ...
    CLASS_ATTR_LABEL                (c, "lightness", 0, _("First Lightness"));
...
void* colorpanel_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    x->f_out_all = outlet_new((t_object*)x, &s_list);
...
{
    return -256 * 256 * int(round(c.red * 255.0))
        - 256 * int(round(c.green * 255.0))
        - 1 * int(round(c.blue * 255.0)) - 1;
}
...
void colorpanel_output(t_colorpanel* x)
{
    ...
    if (ebox_isdrawable((t_ebox*)x) && x->f_color_picked.x >= 0 && x->f_color_picked.y >= 0) {
        ...
        // output pd
        atom_setfloat(av, rgb_to_pd(color_rgb));
        outlet_anything(x->f_out_all, SYM_PROP_PD, 1, av);

        // output rgb
        atom_setfloat(av, color_rgb.red);
        atom_setfloat(av + 1, color_rgb.green);
        atom_setfloat(av + 2, color_rgb.blue);
        outlet_anything(x->f_out_all, SYM_PROP_RGB, 3, av);

        // hsl
        atom_setfloat(av, color_hls.hue);
        atom_setfloat(av + 1, color_hls.saturation);
        atom_setfloat(av + 2, color_hls.lightness);
        outlet_anything(x->f_out_all, SYM_PROP_HSL, 3, av);

        // hex
        atom_setsym(av, color_hex);
        outlet_anything(x->f_out_all, SYM_PROP_HEX, 1, av);
    }
}
...
void draw_background(t_colorpanel* x, t_object* view, t_rect* rect)
{
    ...
    if (g) {
        for (int incx = 0, i = 0; i < x->f_matrix_sizes.x; i++, incx += block_width) {
            for (int incY = 0, j = 0; j < x->f_matrix_sizes.y; j++, incY += block_height) {
