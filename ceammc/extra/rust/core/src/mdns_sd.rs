...
impl mdns {
    fn new(on_err: mdns_err_cb, on_srv: mdns_srv_cb) -> Self {
        mdns {
...
pub extern "C" fn ceammc_rs_mdns_create(on_err: mdns_err_cb, on_srv: mdns_srv_cb) -> *mut mdns {
    let srv = mdns::new(on_err, on_srv);
    if !srv.is_ok() {
        std::ptr::null_mut()
    } else {
...
/// @return mdns_rc
pub extern "C" fn ceammc_rs_mdns_subscribe(mdns: *mut mdns, service: *const c_char) -> mdns_rc {
    if mdns.is_null() {
        return mdns_rc::NullService;
    }
    ...
    match service {
        ...
        Ok(service) => match mdns.as_ref().browse(service.as_str()) {
            Ok(rcv) => {
                mdns.add_service(service, rcv);
                mdns_rc::Ok
            }
            Err(err) => {
...
/// @param service name
/// @return mdns_rc
pub extern "C" fn ceammc_rs_mdns_unsubscribe(mdns: *mut mdns, service: *const c_char) -> mdns_rc {
    if mdns.is_null() {
        return mdns_rc::NullService;
    }
...
/// @param mdns - pointer to mdns
/// @return mdns_rc
pub extern "C" fn ceammc_rs_mdns_process_events(mdns: *mut mdns, timeout_ms: u64) -> mdns_rc {
    if mdns.is_null() {
        return mdns_rc::NullService;
    }
