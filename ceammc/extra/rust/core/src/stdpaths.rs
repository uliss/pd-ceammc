...
fn path_to_string(path: Option<PathBuf>, cb: path_str_cb) -> bool {
    match path {
        Some(path) => {
            if !path.exists() {
...
pub extern "C" fn ceammc_path_home(cb: path_str_cb) -> bool {
    path_to_string(dirs::home_dir(), cb)
}

#[no_mangle]
/// cross-platform pd user directory path with '/' slashes
pub extern "C" fn ceammc_path_pd_user(cb: path_str_cb) -> bool {
...
pub extern "C" fn ceammc_path_pd_doc(cb: path_str_cb) -> bool {
...
pub extern "C" fn ceammc_path_ceammc_doc(cb: path_str_cb) -> bool {
