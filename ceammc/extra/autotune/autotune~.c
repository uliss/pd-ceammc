...
t_int *autotune_perform(t_int *w)
{
	...
	for (lSampleIndex = 0; lSampleIndex < SampleCount; lSampleIndex++)  
	{
		...
		if ((x->cbiwr)%(N/x->noverlap) == 0) {
			...
			if (conf>=x->vthresh) {
				...
				for (ti=0; ti<12; ti++) {
					...
					// 	  }
					tf2 = tf2 - (float)fffabs( (pitch-(float)ti)/6 - 2*floorf(((pitch-(float)ti)/12 + 0.5)) ); // like a likelihood function
		...
		*/
			...
			// when fPull is 1 (legacy behavior which picks fabsolute pitch in respect to A intonation)
			if (fPull <= 1)
			{
			...
			else
			{
				...
				float result = a;
				if (fabs(b) < fabs(result)) result = b;
				if (fabs(c) < fabs(result)) result = c;
